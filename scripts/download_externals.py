...
def _pick_github_asset_match(owner: str, repo: str, matcher, label: str) -> tuple[Optional[str], Optional[str]]:
    """Return (download_url, asset_name) for the newest matching GitHub release asset."""
    endpoints = [
    ...
    for url in endpoints:
        try:
            ...
            log(f"{label}: GitHub release lookup failed at {url}: {e}")
            continue
    return None, None


def _zip_has_member(zip_path: Path, member_name: str) -> bool:
    try:
        needle = member_name.lower().replace("\\", "/")
        with zipfile.ZipFile(zip_path, "r") as zf:
            return any(n.lower().replace("\\", "/").endswith("/" + needle) or n.lower().replace("\\", "/") == needle for n in zf.namelist())
    except Exception:
        return False


def _safe_extract_zip_overwrite(zip_path: Path, target_dir: Path, strip_single_root: bool = False) -> int:
    """Extract zip into target_dir, overwriting existing files and blocking path traversal."""
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        if strip_single_root:
            roots = {n.replace("\\", "/").split("/", 1)[0] for n in names if n and not n.endswith("/")}
            if len(roots) == 1:
        ...
        for raw in names:
            ...
            if not str(out).lower().startswith(str(target_root).lower()):
                raise RuntimeError(f"unsafe zip path blocked: {raw}")
            out.parent.mkdir(parents=True, exist_ok=True)
            with zf.open(raw) as src, open(out, "wb") as dst:
                shutil.copyfileobj(src, dst)
            extracted += 1
    return extracted


def _download_extract_latest_github_zip(
    owner: str,
    repo: str,
    matcher,
    label: str,
    ...
    """Download selected newest GitHub release ZIP, extract to target_dir, then delete the ZIP."""
    url = manual_url
    asset_name = Path(manual_url.split("?", 1)[0]).name if manual_url else None
    if not url:
        url, asset_name = _pick_github_asset_match(owner, repo, matcher, label)
    if not url:
        log(f"{label}: no matching GitHub release asset found; skipping")
        return False

    CACHE.mkdir(parents=True, exist_ok=True)
    safe_name = re.sub(r"[^A-Za-z0-9._-]+", "_", asset_name or Path(url.split("?", 1)[0]).name or "download.zip")
    zpath = CACHE / f"{cache_prefix}-{safe_name}"
    try:
        log(f"{label}: downloading {asset_name or url}")
        http_fetch(url, zpath, timeout=600)
        if required_member and not _zip_has_member(zpath, required_member):
            raise RuntimeError(f"required file not found in zip: {required_member}")
        count = _safe_extract_zip_overwrite(zpath, target_dir, strip_single_root=strip_single_root)
        log(f"{label}: extracted {count} file(s) to {target_dir}")
        return True
    except Exception as e:
        ...
        try:
            if zpath.exists():
                zpath.unlink()
                log(f"{label}: removed downloaded zip {zpath.name}")
        except Exception as e:
            log(f"{label}: could not remove downloaded zip: {e}")


def ensure_sd_cli_cuda(manual_url: Optional[str] = None) -> None:
    """Install newest stable-diffusion.cpp Windows CUDA 12 sd-cli build into presets/bin."""
    bin_dir = ROOT / "presets" / "bin"
    ok = _download_extract_latest_github_zip(
        "leejet",
        "stable-diffusion.cpp",
        lambda n: n.endswith(".zip") and "bin-win-cuda12-x64" in n and n.startswith("sd-") and "cudart" not in n,
        "stable-diffusion.cpp sd-cli CUDA12",
        bin_dir,
        "sd-cli-cuda12",
        required_member="sd-cli.exe",
        manual_url=manual_url,
        strip_single_root=True,
    )
    if ok:
        log("stable-diffusion.cpp sd-cli CUDA12 ready")


def ensure_llamacpp_cuda(manual_url: Optional[str] = None, cudart_url: Optional[str] = None) -> None:
    """Install newest llama.cpp Windows x64 CUDA 12 build + CUDA 12.x DLL pack into presets/bin/llama."""
    llama_dir = ROOT / "presets" / "bin" / "llama"

    # Main executable package, for example: llama-b####-bin-win-cuda-12.4-x64.zip
    main_ok = _download_extract_latest_github_zip(
        "ggml-org",
        "llama.cpp",
        lambda n: n.endswith(".zip") and n.startswith("llama-") and "bin-win-cuda" in n and "x64" in n and "vulkan" not in n and "cudart" not in n,
        "llama.cpp CUDA12",
        llama_dir,
        "llamacpp-cuda12",
        required_member="llama-cli.exe",
        manual_url=manual_url,
        strip_single_root=True,
    )

    # Runtime DLL package, for example: cudart-llama-bin-win-cuda-12.4-x64.zip
    dll_ok = _download_extract_latest_github_zip(
        "ggml-org",
        "llama.cpp",
        lambda n: n.endswith(".zip") and n.startswith("cudart-llama-") and "bin-win-cuda" in n and "x64" in n,
        "llama.cpp CUDA DLLs",
        llama_dir,
        "llamacpp-cudart",
        required_member=None,
        manual_url=cudart_url,
        strip_single_root=True,
    )

    if main_ok:
        if dll_ok:
            log("llama.cpp CUDA12 ready")
        else:
            log("llama.cpp main CUDA12 package installed, but CUDA DLL pack was not found/downloaded")




def _download_plain_file(url: str, out: Path, retries: int = 3, timeout: int = 240) -> bool:
    """Download a normal file (not ZIP-validated), overwriting partial files safely."""
    out.parent.mkdir(parents=True, exist_ok=True)
    token = os.environ.get("GITHUB_TOKEN") or os.environ.get("GH_TOKEN")
    last = None
    for i in range(retries):
        ...
        try:
            headers = {"User-Agent": "FrameVision external downloader"}
            ...
            try:
                ...
                pass
            time.sleep(1.5 * (i + 1))
    log(f"download failed: {last} @ {url}")
    return False


def pull_video_timeline_transitions() -> None:
    """
    Download FrameVision Video Timeline Editor transition PNG files into:
        assets/transitions

    Source:
        Koongrizzly/FrameVision-Video-Timeline-Editor/assets/transitions
    """
    repo_api = "https://api.github.com/repos/Koongrizzly/FrameVision-Video-Timeline-Editor/contents/assets/transitions?ref=main"
    dest_root = ROOT / "assets" / "transitions"
    ...
    def _walk(api_url: str, rel_dir: Path) -> None:
        nonlocal downloaded, skipped, failed
        try:
            data = http_json(api_url, retries=3, timeout=30)
        except Exception as e:
            log(f"timeline transitions: GitHub listing failed: {e}")
            failed += 1
            return
        ...
        if not isinstance(data, list):
            log("timeline transitions: unexpected GitHub API response; skipping")
            failed += 1
            return
        ...
        for item in data:
            ...
            if typ == "dir":
                ...
                if child_url:
                    _walk(str(child_url), rel_dir / name)
                continue

            if typ != "file" or not name.lower().endswith(".png"):
                continue

            url = item.get("download_url")
            if not url:
                failed += 1
                log(f"timeline transitions: missing download_url for {name}")
                continue

            out = dest_root / rel_dir / name
            expected_size = item.get("size")
            try:
                if out.exists() and out.is_file():
                    if expected_size is None or int(out.stat().st_size) == int(expected_size):
                        skipped += 1
                        continue
            except Exception:
                pass

            log(f"timeline transitions: fetching {rel_dir / name}")
            if _download_plain_file(str(url), out, retries=3, timeout=240):
                downloaded += 1
            else:
                failed += 1

    _walk(repo_api, Path(""))
    log(f"timeline transitions ready: {downloaded} downloaded, {skipped} kept, {failed} failed -> {dest_root}")
...
def pull_qwen3_vl_2b(base: Path) -> None:
    """
    Download Huihui-Qwen3-VL-2B-Instruct-abliterated into:
        models/describe/default/qwen3vl2b

    This keeps the old FrameVision folder name/path so existing Qwen3-VL users do
    not need code changes.

    If the destination already contains the old original Qwen files, it is cleared
    first. The Huihui build is detected by either:
        - added_tokens.json
        - chat_template.jinja
    """
    repo_id = "huihui-ai/Huihui-Qwen3-VL-2B-Instruct-abliterated"
    dest = base / "describe" / "default" / "qwen3vl2b"
    marker_files = ("added_tokens.json", "chat_template.jinja")

    log(f"pulling {repo_id} -> {dest}")
    ...
    # If this folder already has files but none of the Huihui marker files, treat it
    # as the old original Qwen folder and clear it before downloading the replacement.
    try:
        has_marker = any((dest / name).exists() for name in marker_files)
        has_any_existing = dest.exists() and any(dest.iterdir())
        if has_any_existing and not has_marker:
            log("Qwen3-VL folder looks like the old original build; clearing it before downloading Huihui replacement.")
            for item in list(dest.iterdir()):
                try:
                    ...
                    log(f"Qwen3 cleanup warning: could not remove {item}: {e}")
    except Exception as e:
        log(f"Qwen3 cleanup check warning: {e}")
    ...
    try:
        if snapshot_download is not None:
            snapshot_download(
                ...
                allow_patterns=[
                    "*.json", "*.jinja", "*.safetensors", "*.bin", "*.model",
                    "*.txt", "*.tiktoken", "*.py", "LICENSE*", "*.md"
                ],
    ...
    if used is None:
        # Fallback to huggingface-cli / hf.
        ...
        cmd = [cli, "download", repo_id, "--local-dir", str(dest), "--resume-download"]
        try:
...
def pull_bg_models(dest_folder: str | None = None, pro: bool = False, extra_urls: list[str] | None = None, manual_url: str | None = None) -> None:
    ...
    Download background-removal models into root \\models\\ (default subfolder: 'bg').
      ...
      - With --all or --bg-pro: also fetches BiRefNet (ONNX) — large, high-quality general SOD.
    ...
    # BiRefNet ONNX (large) — included for --all/normal repair, and also when --bg-pro is used
    if pro:
...
def main(argv=None):
    ...
    ap.add_argument('--transitions-only', action='store_true', help='Download Video Timeline Editor transition PNG files into assets/transitions')
    ap.add_argument('--sdcli-only', action='store_true', help='Download latest stable-diffusion.cpp CUDA12 sd-cli into presets/bin')
    ap.add_argument('--sdcli-url', type=str, help='Manual stable-diffusion.cpp CUDA12 ZIP URL override')
    ap.add_argument('--llamacpp-only', action='store_true', help='Download latest llama.cpp Windows CUDA12 build into presets/bin/llama')
    ap.add_argument('--llamacpp-url', type=str, help='Manual llama.cpp CUDA12 ZIP URL override')
    ap.add_argument('--llamacpp-cudart-url', type=str, help='Manual llama.cpp CUDA DLL ZIP URL override')
    ...
    do_all = args.all or not any([
        ...
        args.bg_only, args.bg_pro, args.sdcli_only, args.llamacpp_only, args.transitions_only
    ])

    if do_all or args.transitions_only:
        try:
            pull_video_timeline_transitions()
        except Exception as e:
            log(f"timeline transitions error: {e}")
    ...
    if do_all or args.realesrgan_only:
        ...
        try:
            pull_realesr_general_v3()
        except Exception as e:
            log(f"realesr-general extra models error: {e}")
    ...
    if do_all or args.bg_only or args.bg_pro:
        try:
            # --all should repair/install every bundled external model except Whisper.
            # --bg-only remains the small/default BG install unless --bg-pro is also passed.
            pull_bg_models(dest_folder=args.bg_dest, pro=(do_all or args.bg_pro), extra_urls=args.bg_url)
        except Exception as e:
            print("[externals] BG models error:", e)

    # Stable-diffusion.cpp sd-cli + llama.cpp CUDA builds (overwrite-safe; zips removed after extraction)
    if do_all or args.sdcli_only:
        try:
            ensure_sd_cli_cuda(args.sdcli_url)
        except Exception as e:
            log(f"stable-diffusion.cpp sd-cli error: {e}")

    if do_all or args.llamacpp_only:
        try:
            ensure_llamacpp_cuda(args.llamacpp_url, args.llamacpp_cudart_url)
        except Exception as e:
            log(f"llama.cpp error: {e}")
...
def _run_embedded_downloadbg() -> int:
    ...
    try:
        # Download remaining upscaler packs/extras by default, but keep Whisper opt-in only.
        # Embedded downloadbg.py treats no extra args as all supported downloads except Faster-Whisper.
