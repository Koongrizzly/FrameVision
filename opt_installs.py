...
def _default_installs() -> List[OptionalInstall]:
    ...
    return [
        ...
        OptionalInstall(
            ...
            description="VRAM: 16GB recommended. Disk: ~30GB download (diffusion fp16 + text encoder + VAE). Tip: try bf16 if fp16 shows black images.",
...
class OptionalInstallsDialog(QtWidgets.QDialog):
    ...
    def __init__(self, parent: Optional[QtWidgets.QWidget] = None, root_dir: Optional[Path] = None) -> None:
        ...
        subtitle = QtWidgets.QLabel(
            "Select extra installs. When you press Start, installs run one-by-one and the log appears below. Restart the app when done"
        )
