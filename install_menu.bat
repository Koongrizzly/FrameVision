...
echo.
echo %ITEM% 2^)  Core install/update %ORANGE%(resets the app, this isn't a real install)%RST%
echo %RST%      Creates/updates .venv and installs FrameVision or removes old models that are no longer used.
echo %BLUE%      Good for a quick repair when the app no longer starts or a model was removed by the developper.
echo. 
echo %ITEM% 3^)  Full install %ORANGE%(CUDA GPU)
echo %RST%      Installs the app plus CUDA-enabled dependencies and the smaller models
echo %BLUE%      Built for RTX 30XX and above GPUs. 8-12 gig Vram or more is advised 
echo.
echo %ITEM% 4^)  Exit
echo. %RST%
...
:ensure_conda
rem Ensure Conda is available for optional AI model installers. This does NOT replace the main .venv app install.
call :_detect_conda && exit /b 0

echo.
echo Conda was not found.
echo Some optional AI model installers use Conda environments.
choice /C YN /N /M "Install Miniconda now? [Y/N]: "
if errorlevel 2 (
  echo Conda skipped. Some optional installers may not work until Miniconda is installed.
  exit /b 0
)

call :_install_miniconda
call :_detect_conda && (
  echo Conda is now available.
  exit /b 0
)

echo Failed to install Miniconda automatically.
echo Some optional installers may not work until Miniconda is installed manually.
exit /b 1

:_detect_conda
rem Detect local FrameVision Miniconda first, then common user/system installs, then PATH.
set "CONDA_BAT="
set "CONDA_EXE="

if exist "%ROOT%tools\miniconda3\condabin\conda.bat" (
  set "CONDA_BAT=%ROOT%tools\miniconda3\condabin\conda.bat"
  set "PATH=%ROOT%tools\miniconda3\condabin;%ROOT%tools\miniconda3\Scripts;%ROOT%tools\miniconda3;%PATH%"
  exit /b 0
)

if exist "%UserProfile%\miniconda3\condabin\conda.bat" (
  set "CONDA_BAT=%UserProfile%\miniconda3\condabin\conda.bat"
  set "PATH=%UserProfile%\miniconda3\condabin;%UserProfile%\miniconda3\Scripts;%UserProfile%\miniconda3;%PATH%"
  exit /b 0
)

if exist "%LocalAppData%\miniconda3\condabin\conda.bat" (
  set "CONDA_BAT=%LocalAppData%\miniconda3\condabin\conda.bat"
  set "PATH=%LocalAppData%\miniconda3\condabin;%LocalAppData%\miniconda3\Scripts;%LocalAppData%\miniconda3;%PATH%"
  exit /b 0
)

if exist "%ProgramData%\miniconda3\condabin\conda.bat" (
  set "CONDA_BAT=%ProgramData%\miniconda3\condabin\conda.bat"
  set "PATH=%ProgramData%\miniconda3\condabin;%ProgramData%\miniconda3\Scripts;%ProgramData%\miniconda3;%PATH%"
  exit /b 0
)
...
:_install_miniconda
rem Installs Miniconda locally inside FrameVision tools\miniconda3 with minimal clicks.
set "CONDA_DIR=%ROOT%tools\miniconda3"
set "CONDA_TMP=%TEMP%\Miniconda3-latest-Windows-x86_64.exe"
set "CONDA_URL=https://repo.anaconda.com/miniconda/Miniconda3-latest-Windows-x86_64.exe"

if /I not "%PROCESSOR_ARCHITECTURE%"=="AMD64" (
  echo Automatic Miniconda install currently supports 64-bit Windows x86_64 only.
  echo Please install Miniconda manually for this CPU architecture.
  exit /b 1
)

if exist "%CONDA_DIR%\condabin\conda.bat" exit /b 0

if not exist "%ROOT%tools" mkdir "%ROOT%tools" >nul 2>nul

echo Downloading Miniconda...
powershell -NoProfile -ExecutionPolicy Bypass -Command "try{Invoke-WebRequest -UseBasicParsing '%CONDA_URL%' -OutFile '%CONDA_TMP%';exit 0}catch{Write-Host 'Download failed';exit 1}"
if errorlevel 1 exit /b 1

echo Installing Miniconda locally to: %CONDA_DIR%
echo This can take a few minutes...
powershell -NoProfile -ExecutionPolicy Bypass -Command "$p='%CONDA_TMP%'; $d='%CONDA_DIR%'; $args=@('/InstallationType=JustMe','/RegisterPython=0','/AddToPath=0','/S',('/D=' + $d)); $proc=Start-Process -FilePath $p -ArgumentList $args -Wait -PassThru; exit $proc.ExitCode"
set "CONDA_INSTALL_RC=%ERRORLEVEL%"
del /q "%CONDA_TMP%" >nul 2>nul
if not "%CONDA_INSTALL_RC%"=="0" exit /b %CONDA_INSTALL_RC%

exit /b 0

:ensure_venv
rem If a usable .venv already exists, do not try to create it again.
rem Important: use exit /b 0 instead of goto :eof so stale ERRORLEVEL values
rem cannot make callers think venv creation failed.
if exist ".venv\Scripts\python.exe" (
  echo .venv already present.
  exit /b 0
)

rem If the folder exists already, leave it alone. This avoids repair/reset
rem failing because Python tries to recreate an existing/locked environment.
if exist ".venv" (
...
if exist ".venv\Scripts\python.exe" call ".venv\Scripts\python.exe" -m pip install --prefer-binary pymunk==6.10.* || goto pip_fail
...
:ensure_core_tools
rem Ensures FrameVision command-line tools exist in presets\bin.
set "FV_BIN=%ROOT%presets\bin"
if not exist "%FV_BIN%" mkdir "%FV_BIN%" >nul 2>nul

echo(
echo %TITLE% Tool check: presets\bin %RST%

if exist "%FV_BIN%\sd-cli.exe" (
  echo( sd-cli.exe: OK
) else (
  echo( sd-cli.exe: MISSING - downloading latest stable-diffusion.cpp CUDA12 Windows build...
  call :download_sd_cli
  if exist "%FV_BIN%\sd-cli.exe" (echo( sd-cli.exe: OK) else echo( sd-cli.exe: STILL MISSING
)

set "FV_FFMPEG_MISSING="
for %%F in (ffmpeg.exe ffprobe.exe ffplay.exe) do if not exist "%FV_BIN%\%%F" set "FV_FFMPEG_MISSING=1"
if defined FV_FFMPEG_MISSING (
  echo( FFmpeg bundle: MISSING - downloading latest BtbN Windows GPL build...
  call :download_ffmpeg_bundle
) else (
  echo( FFmpeg bundle: OK
)

for %%F in (ffmpeg.exe ffprobe.exe ffplay.exe) do (
  if exist "%FV_BIN%\%%F" (echo( %%F: OK) else echo( %%F: MISSING)
)
exit /b 0

:download_sd_cli
powershell -NoProfile -ExecutionPolicy Bypass -Command "$ErrorActionPreference='Stop'; $bin=(Resolve-Path 'presets\bin').Path; $tmp=Join-Path $env:TEMP ('fv_sdcli_' + [guid]::NewGuid().ToString('N')); New-Item -ItemType Directory -Force -Path $tmp | Out-Null; try { [Net.ServicePointManager]::SecurityProtocol=[Net.SecurityProtocolType]::Tls12; $headers=@{'User-Agent'='FrameVisionInstaller'}; $rel=Invoke-RestMethod -Uri 'https://api.github.com/repos/leejet/stable-diffusion.cpp/releases/latest' -Headers $headers; $asset=$rel.assets | Where-Object { $_.name -like 'sd-master-*-bin-win-cuda12-x64.zip' } | Select-Object -First 1; if(-not $asset){ throw 'No sd-master-*-bin-win-cuda12-x64.zip asset found in latest stable-diffusion.cpp release.' }; $zip=Join-Path $tmp $asset.name; Write-Host ('Downloading ' + $asset.name); Invoke-WebRequest -Uri $asset.browser_download_url -OutFile $zip -Headers $headers; Expand-Archive -LiteralPath $zip -DestinationPath $tmp -Force; $exe=Get-ChildItem -LiteralPath $tmp -Recurse -File | Where-Object { $_.Name -eq 'sd-cli.exe' -or $_.Name -eq 'sd.exe' } | Select-Object -First 1; if(-not $exe){ throw 'sd-cli.exe/sd.exe was not found inside the downloaded archive.' }; Copy-Item -Path (Join-Path $exe.DirectoryName '*') -Destination $bin -Force -ErrorAction SilentlyContinue; if(Test-Path (Join-Path $bin 'sd.exe')){ Copy-Item -LiteralPath (Join-Path $bin 'sd.exe') -Destination (Join-Path $bin 'sd-cli.exe') -Force }; if(-not (Test-Path (Join-Path $bin 'sd-cli.exe'))){ Copy-Item -LiteralPath $exe.FullName -Destination (Join-Path $bin 'sd-cli.exe') -Force }; } finally { Remove-Item -LiteralPath $tmp -Recurse -Force -ErrorAction SilentlyContinue }"
if errorlevel 1 echo %NOTE%[WARN] sd-cli download/extract failed. Check internet connection or GitHub availability.%RST%
exit /b 0

:download_ffmpeg_bundle
powershell -NoProfile -ExecutionPolicy Bypass -Command "$ErrorActionPreference='Stop'; $bin=(Resolve-Path 'presets\bin').Path; $tmp=Join-Path $env:TEMP ('fv_ffmpeg_' + [guid]::NewGuid().ToString('N')); New-Item -ItemType Directory -Force -Path $tmp | Out-Null; try { [Net.ServicePointManager]::SecurityProtocol=[Net.SecurityProtocolType]::Tls12; $url='https://github.com/BtbN/FFmpeg-Builds/releases/download/latest/ffmpeg-master-latest-win64-gpl.zip'; $zip=Join-Path $tmp 'ffmpeg-master-latest-win64-gpl.zip'; Write-Host 'Downloading ffmpeg-master-latest-win64-gpl.zip'; Invoke-WebRequest -Uri $url -OutFile $zip -Headers @{'User-Agent'='FrameVisionInstaller'}; Expand-Archive -LiteralPath $zip -DestinationPath $tmp -Force; $ffmpeg=Get-ChildItem -LiteralPath $tmp -Recurse -File -Filter 'ffmpeg.exe' | Select-Object -First 1; if(-not $ffmpeg){ throw 'ffmpeg.exe was not found inside the downloaded archive.' }; $src=$ffmpeg.DirectoryName; foreach($name in @('ffmpeg.exe','ffprobe.exe','ffplay.exe')){ $f=Join-Path $src $name; if(Test-Path $f){ Copy-Item -LiteralPath $f -Destination (Join-Path $bin $name) -Force } }; } finally { Remove-Item -LiteralPath $tmp -Recurse -Force -ErrorAction SilentlyContinue }"
if errorlevel 1 echo %NOTE%[WARN] FFmpeg download/extract failed. If this keeps failing, provide a preferred FFmpeg bundle URL.%RST%
exit /b 0
...
call :ensure_conda
if errorlevel 1 echo %NOTE%[WARN] Conda could not be installed automatically. Optional Conda-based model installers may not work.%RST%
...
if defined CONDA_BAT (
  echo( Conda: OK ^(!CONDA_BAT!^)
) else if defined CONDA_EXE (
  echo( Conda: OK ^(!CONDA_EXE!^)
) else (
  echo( Conda: NOT FOUND  ^(optional AI model installers may need Miniconda^)
)
...
rem === Download tools/models for Core repair/reset ===
rem Keep this directly before the app launch so option 2 also restores the default describe model.
if exist "scripts\download_externals.py" (
  if exist ".venv\Scripts\python.exe" (
    echo Running download_externals.py for Core repair/reset...
    call ".venv\Scripts\python.exe" -u "scripts\download_externals.py" --all
    if errorlevel 1 echo %BRIGHT_YELLOW%[WARN] download_externals.py had issues; continuing to launch FrameVision.%RST%
  ) else (
    echo %BRIGHT_YELLOW%[WARN] .venv Python was not found, so download_externals.py could not run.%RST%
  )
) else (
  echo %BRIGHT_YELLOW%[WARN] scripts\download_externals.py was not found; skipping external downloads.%RST%
)
