...
_nodes = []
@register_visualizer
class VoltageArcs(BaseVisualizer):
    display_name = "Voltage Arcs"
    def paint(self, p:QPainter, r, bands, rms, t):
        ...
        if not _nodes:
            for i in range(16):
                _nodes.append([_rng.random()*w, _rng.random()*h])
        p.fillRect(r,QBrush(QColor(6,6,10)))
        env, gate, boom, punch, bc = beat_drive(bands, rms, t)
        p.setPen(QPen(QColor(80,100,160,160),2))
        for x,y in _nodes:
            p.drawEllipse(QPointF(x,y), 4,4)
        p.setCompositionMode(QPainter.CompositionMode_Plus)
        for i in range(len(_nodes)-1):
            x1,y1=_nodes[i]; x2,y2=_nodes[(i+1)%len(_nodes)]
            jitter = 6 + 30*(env + punch)
            col = QColor.fromHsv(int((t*90+i*10)%360),230,255,200)
            p.setPen(QPen(col,3))
            p.drawLine(QPointF(x1+_rng.random()*jitter-jitter/2, y1+_rng.random()*jitter-jitter/2),
                       QPointF(x2+_rng.random()*jitter-jitter/2, y2+_rng.random()*jitter-jitter/2))
