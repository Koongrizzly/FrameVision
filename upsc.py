...
class UpscPane(QtWidgets.QWidget):
    ...
    def _enqueue_or_sequential(self, files, out_dir_override: Path = None):
        """
        Try to enqueue each file to the GPU queue; if queue integration isn't available,
        fall back to running all jobs sequentially in a single _RunThread (no parallel spawns).
        If out_dir_override is provided, use it for all items regardless of type.
        """
        # Prefer queue path (helpers.queue_adapter or main.enqueue)
        try:
        ...
        # For Waifu2x, the worker may not implement that engine — run locally but sequentially.
        if isinstance(eng_label, str) and "Waifu2x" in eng_label:
            enq = None

        # Attempt to enqueue per file if enqueue callable exists
        if callable(enq):
            queued = 0
            for p in files:
                try:
                    ...
                    # Out dir: override > UI > per-type default
                    if out_dir_override:
                        out_dir = Path(out_dir_override)
                    else:
                        try:
                            out_dir_ui = (self.edit_outdir.text() or "").strip()
                        except Exception:
                            out_dir_ui = ""
                        out_dir = Path(out_dir_ui) if out_dir_ui else (OUT_VIDEOS if is_video else OUT_SHOTS)

                    # Derive model from UI
                    try:
                        if "Waifu2x" in eng_label:
                            model_name = self.combo_model_w2x.currentText()
                        else:
                            model_name = self.combo_model_realsr.currentText()
                    except Exception:
                        model_name = ""

                    # Try kwargs signature first
                    try:
                        enq(job_type=job_type, input_path=str(p), out_dir=str(out_dir), factor=int(factor), model=str(model_name))
                        queued += 1
                        try:
                            self._append_log(f"Queued: {p.name} → {out_dir}  (via {where})")
                        except Exception:
                            pass
                        continue
                    except TypeError:
                        # Fall back to job-dict signature
                        try:
                            job = {
                                "type": job_type,
                                "input": str(p),
                                "out_dir": str(out_dir),
                                "args": {"factor": int(factor), "model": str(model_name)},
                            }
                            enq(job)  # if queue_adapter.enqueue(job) style
                            queued += 1
                            try:
                                self._append_log(f"Queued: {p.name} → {out_dir}  (via {where} [job])")
                            except Exception:
                                pass
                            continue
                        except Exception:
                            # drop to local sequential
                            pass
                except Exception:
                    pass
            if queued > 0:
        ...
        # ---- Sequential local fallback (no queue available) ----
        all_cmds = []
        last_out = None
        for p in files:
            try:
