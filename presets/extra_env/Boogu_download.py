...
BOOGU_TURBO_GGUF_REPO = "realrebelai/Boogu-Image-Turbo_GGUFs"
BOOGU_EDIT_GGUF_REPO = "realrebelai/Boogu-Image-Edit_GGUFs"
BOOGU_EDIT_TURBO_GGUF_REPO = "realrebelai/Boogu-Image-Edit-Turbo_GGUFs"
...
GGUF_FILE_CHOICES = {
    "Q4": {
        "turbo": (BOOGU_TURBO_GGUF_REPO, "boogu-turbo-dit-Q4_1.gguf"),
        "edit": (BOOGU_EDIT_GGUF_REPO, "boogu-edit-dit-Q4_1.gguf"),
        "edit-turbo": (BOOGU_EDIT_TURBO_GGUF_REPO, "boogu-edit-turbo-dit-Q4_1.gguf"),
    },
    "Q5": {
        "turbo": (BOOGU_TURBO_GGUF_REPO, "boogu-turbo-dit-Q5_1.gguf"),
        "edit": (BOOGU_EDIT_GGUF_REPO, "boogu-edit-dit-Q5_1.gguf"),
        "edit-turbo": (BOOGU_EDIT_TURBO_GGUF_REPO, "boogu-edit-turbo-dit-Q5_1.gguf"),
    },
    "Q8": {
        "turbo": (BOOGU_TURBO_GGUF_REPO, "boogu-turbo-dit-Q8_0.gguf"),
        "edit": (BOOGU_EDIT_GGUF_REPO, "boogu-edit-dit-Q8_0.gguf"),
        "edit-turbo": (BOOGU_EDIT_TURBO_GGUF_REPO, "boogu-edit-turbo-dit-Q8_0.gguf"),
    },
}
...
def planned_gguf_downloads(quant: str, model_names: List[str]) -> List[Tuple[str, str, Path, str]]:
    """Build a GGUF download plan for Turbo, Edit, or Edit-Turbo."""
    quant = quant.upper()
    if quant not in GGUF_FILE_CHOICES:
        raise ValueError(f"Unsupported GGUF quant: {quant}")

    downloads: List[Tuple[str, str, Path, str]] = []
    for model_name in model_names:
        if model_name not in GGUF_FILE_CHOICES[quant]:
            raise ValueError(f"Unsupported Boogu GGUF model: {model_name}")
        repo, filename = GGUF_FILE_CHOICES[quant][model_name]
        target = BOOGU_MODEL_DIR / filename
        label_name = model_name.replace("-", " ").title()
        downloads.append((repo, filename, target, f"Boogu Image {label_name} {quant}"))
    ...
    if any(name in {"edit", "edit-turbo"} for name in model_names):
        mm_repo, mm_path, mm_target = EXTRA_FILES["qwen_mmproj"]
        downloads.append((mm_repo, mm_path, mm_target, "Qwen3-VL vision projector"))
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Download Boogu Image models for FrameVision.")
    parser.add_argument("--install-sd-cli", action="store_true", help="Download/update sd-cli before models.")
    parser.add_argument("--precision", choices=("fp8", "fp16"), help="Safetensors precision.")
    parser.add_argument("--model", choices=("turbo", "edit", "both", "edit-turbo"), help="Model family to download.")
    parser.add_argument("--gguf-quant", choices=("Q4", "Q5", "Q8"), help="Download GGUF instead of safetensors.")
    parser.add_argument("--yes", action="store_true", help="Run non-interactively and accept the plan.")
...
def main() -> int:
    ...
    non_interactive = bool(args.yes or args.model or args.gguf_quant or args.precision or args.install_sd_cli)

    if args.install_sd_cli:
        install_latest_sd_cli()
    elif non_interactive:
        log("Keeping existing sd-cli (use --install-sd-cli to update it)")
    elif ask_yes_no("Download or update sd-cli first", True):
        install_latest_sd_cli()
    else:
    ...
    if args.model:
        ...
        chosen_model = MODEL_CHOICES[model_key]

    models = ["turbo", "edit"] if chosen_model == "both" else [chosen_model]

    if args.gguf_quant:
        precision_label = args.gguf_quant.lower()
        downloads = planned_gguf_downloads(args.gguf_quant, models)
    else:
        if "edit-turbo" in models:
            raise ValueError("Edit-Turbo is currently available through GGUF selections only.")
        if args.precision:
    ...
    if not args.yes and not ask_yes_no("Start download", True):
    ...
    write_manifest(precision_label, models, completed_targets)
