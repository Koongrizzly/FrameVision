...
class ACEStepPipeline:
    ...
    def save_wav_file(
        ...
        """
        Patched: always save as plain WAV using soundfile, bypassing torchaudio / torchcodec.
        The 'format' argument is kept for compatibility with callers, but the file on disk
        is always written as .wav via soundfile.
        """
        # Decide base output folder and final path
        if save_path is None:
            ...
            output_path_wav = os.path.join(
                base_path,
                f"output_{time.strftime('%Y%m%d%H%M%S')}_{idx}.wav",
            )
        else:
            # If a directory is passed, create it and put a timestamped file inside
            if os.path.isdir(save_path):
                ...
                # A full filename was passed
                dir_name = os.path.dirname(save_path)
                if dir_name:
                    ensure_directory_exists(dir_name)
                # Ensure it has .wav extension
                if not save_path.lower().endswith(".wav"):
                    output_path_wav = save_path + ".wav"
                else:
                    output_path_wav = save_path

        # Ensure tensor is on CPU float32 and convert to NumPy
        if torch.is_tensor(target_wav):
            wav = target_wav.detach().cpu().float().numpy()
        else:
            wav = target_wav
        ...
        if wav.ndim == 1:
            # mono
            sf.write(output_path_wav, wav, sample_rate)
        else:
            # [channels, samples] -> [samples, channels]
            sf.write(output_path_wav, wav.T, sample_rate)

        logger.info(
            f"Saving audio (forced WAV) to {output_path_wav} using soundfile backend"
        )
