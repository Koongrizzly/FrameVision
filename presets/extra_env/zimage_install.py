...
def _env_dir(root: Path) -> Path:
    """Dedicated Z-Image environment location used by FrameVision."""
    return (root / "environments" / ".zimage_env").resolve()
...
def _install_python_env(root: Path, args: argparse.Namespace) -> Path:
    """Create/update root/environments/.zimage_env and install runtime deps."""
    env = _env_dir(root)
    py = _env_python(root)

    old_env = (root / ".zimage_env").resolve()
    if old_env.exists() and old_env != env:
        log(f"[zimage-env] note: old root env exists and is no longer used: {old_env}")
        log(f"[zimage-env] using new env location: {env}")
    ...
    if py is None:
        ...
        if py is None:
            ...
            raise RuntimeError(f"Z-Image env was created but python.exe was not found. Tried: {tried}")
    else:
    ...
    if not getattr(args, "skip_deps", False):
        ...
        if getattr(args, "skip_torch", False):
            ...
            torch_index = getattr(args, "torch_index_url", "") or "https://download.pytorch.org/whl/cu121"
            log(f"[zimage-env] installing CUDA PyTorch from: {torch_index}")
            _run_cmd([str(py), "-m", "pip", "install", "torch", "torchvision", "torchaudio", "--index-url", torch_index], cwd=root)
    ...
    try:
        marker.write_text(
            "FrameVision Z-Image environment\n"
            f"location={env}\n"
...
def _parse_args(argv) -> argparse.Namespace:
    ...
    p.add_argument("--torch-index-url", default="https://download.pytorch.org/whl/cu121",
