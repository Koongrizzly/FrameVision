...
TORCH_PIN = "2.9.0"
TORCHVISION_PIN = "0.24.0"
TORCHAUDIO_PIN = "2.9.0"
...
def run_check(cmd: list[str], *, cwd: Path | None = None, env: dict[str, str] | None = None) -> subprocess.CompletedProcess:
    """Run a command without raising. Used for install-time validation gates."""
    log(">>> " + " ".join(cmd))
    return subprocess.run(cmd, cwd=str(cwd) if cwd else None, env=env, check=False)


def torch_stack_marker(env_dir: Path) -> Path:
    return env_dir / ".framevision_torch_stack_2_9_0_cu128_ok"


def torch_metadata_ok(vpy: Path, env: dict[str, str]) -> bool:
    """Check installed package versions without importing torchaudio binary modules.

    Do NOT import torchaudio here. A broken _torchaudio.pyd can trigger a blocking
    Windows dialog before Python can recover. Metadata-only checks are safe.
    """
    code = f"""
...
for name, want in expected.items():
    ...
    if got != want:
        raise SystemExit(21)
"""
    result = run_check([str(vpy), "-c", code], env=env)
    return result.returncode == 0


def torch_binary_import_ok(vpy: Path, env: dict[str, str]) -> bool:
    """Import-test the full Torch stack after install/repair."""
    code = f"""
import sys
import torch
import torchvision
...
if clean(torch.__version__) != "{TORCH_PIN}":
    raise SystemExit(11)
if str(torch.version.cuda) != "12.8":
    raise SystemExit(12)
if not torch.cuda.is_available():
    raise SystemExit(13)
if clean(torchvision.__version__) != "{TORCHVISION_PIN}":
    raise SystemExit(14)
if clean(torchaudio.__version__) != "{TORCHAUDIO_PIN}":
    raise SystemExit(15)
"""
    result = run_check([str(vpy), "-c", code], env=env)
    return result.returncode == 0
...
def ensure_torch_stack(vpy: Path, vpip: list[str], env: dict[str, str]) -> None:
    """Install/repair the Torch CUDA stack safely.

    Rules:
    - Never import torchaudio before repair; that can trigger Windows popup dialogs.
    - Use a marker file only after the binary import test succeeds.
    - If this is a fresh/rebuilt env or no marker exists, force-reinstall the pinned trio once.
    - If marker exists and metadata is still correct, skip the heavy install.
    """
    env_dir = vpy.parents[1]
    marker = torch_stack_marker(env_dir)

    if marker.exists() and torch_metadata_ok(vpy, env):
        log("[OK] Pinned Torch stack marker + metadata are valid. Skipping heavy Torch install.")
        return
    ...
    log("[STEP] Installing/repairing pinned Torch CUDA stack:")
    log(f"       torch=={TORCH_PIN}+cu128")
    log(f"       torchvision=={TORCHVISION_PIN}+cu128")
    log(f"       torchaudio=={TORCHAUDIO_PIN}+cu128")
    ...
    run(vpip + [
        ...
        "--force-reinstall",
        "--no-cache-dir",
        "--index-url", TORCH_INDEX_CU128,
        f"torch=={TORCH_PIN}+cu128",
        f"torchvision=={TORCHVISION_PIN}+cu128",
        f"torchaudio=={TORCHAUDIO_PIN}+cu128",
    ], env=env)

    log("[STEP] Verifying Torch/TorchVision/Torchaudio binary compatibility...")
    if not torch_binary_import_ok(vpy, env):
        raise RuntimeError("Torch/TorchVision/Torchaudio binary compatibility check failed after pinned install.")

    try:
        marker.write_text("ok\n", encoding="utf-8")
    except Exception:
        pass
    log("[OK] Torch stack verified.")
...
def main() -> int:
    ...
    log("[STEP] Ensuring Torch CUDA stack (safe pinned trio) + torchsde + triton-windows...")
    ensure_torch_stack(vpy, vpip, env)
    ...
    try:
        ...
        # Re-validate/repair the pinned compatible Torch stack, then retry ACE install.
        ensure_torch_stack(vpy, vpip, env)
        run(vpip + ["install", "--extra-index-url", TORCH_INDEX_CU128, "-e", "."], cwd=repo_dir, env=env)
    ...
    # Final sanity check. This happens only after the pinned Torch stack has been
    # installed/repaired, so it will not trigger the old pre-repair torchaudio popup.
    log("[STEP] Final Torch stack binary check...")
    if not torch_binary_import_ok(vpy, env):
        raise RuntimeError("Final Torch stack binary check failed.")
