...
def ensure_ltx23_split_parts(py: Path, root: Path, split_dir: Path, *, skip_downloads: bool) -> None:
    """Download/repair the default LTX 2.3 split component.

    Production default is intentionally conservative: only vocoder.safetensors
    is downloaded because it is the split file that proved useful for stability.
    Other extracted split parts stay optional/dev and are not pulled by the
    installer unless this function is deliberately extended later.
    """
    split_dir.mkdir(parents=True, exist_ok=True)
    target_name = "vocoder.safetensors"
    target = split_dir / target_name

    if target.exists() and target.stat().st_size > 1024:
        status("FOUND", f"LTX split vocoder: {target} ({target.stat().st_size / (1024 ** 2):.2f} MB)")
        status("SKIPPED", "LTX split vocoder already exists")
        return

    status("MISSING", f"LTX split vocoder: {target}")
    if skip_downloads:
        status("SKIPPED", "LTX split vocoder download skipped")
        return
...
print(p)
"""
    status("DOWNLOADING", f"LTX split vocoder from Hugging Face repo {HF_LTX23_SPLIT_REPO}")
    run([py, "-c", code], cwd=root, env=portable_env(root), check=True)

    if target.exists() and target.stat().st_size > 1024:
        status("FOUND", f"Split part: {target.name} ({target.stat().st_size / (1024 ** 2):.2f} MB)")
    else:
        raise RuntimeError(f"Vocoder split download finished but expected file was not found: {target}")
