...
What this installer does by default:
  ...
  2) Installs the Python deps used by the working WAN/Turbo tests, including
     Triton-Windows, FlashAttention and SageAttention in that order.
...
TRITON_PACKAGE = "triton-windows==3.2.0.post21"
...
def run_with_note(cmd: list[str], note: str, cwd: Path | None = None, env: dict[str, str] | None = None) -> None:
    """Run a command and add a focused note when it fails."""
    log("[RUN] " + " ".join(f'"{x}"' if " " in str(x) else str(x) for x in cmd))
    p = subprocess.run(cmd, cwd=str(cwd) if cwd else None, env=env)
    if p.returncode != 0:
        fail(f"Command failed with exit code {p.returncode}: {' '.join(cmd)}\n{note}")
...
def install_env_packages(py: Path, skip_flash: bool = False) -> None:
    ...
    log("\n[INFO] Installing Triton-Windows, FlashAttention and SageAttention...")
    log("       Order matters: Triton-Windows first, then FlashAttention, then SageAttention.")
    run_with_note(
        [str(py), "-m", "pip", "install", "--upgrade", "--no-warn-script-location", TRITON_PACKAGE],
        "Triton-Windows install failed. For this Torch 2.6 / CUDA 12.6 env, use a compatible triton-windows 3.2.x release from: "
        + TRITON_WINDOWS_RELEASES_URL,
    )
    run([str(py), "-m", "pip", "install", "--upgrade", "--no-warn-script-location", FLASH_ATTN_WHEEL])
    run([str(py), "-m", "pip", "install", "--upgrade", "--force-reinstall", "--no-warn-script-location", SAGE_ATTN_WHEEL])
    run([str(py), "-c", "import triton; print('triton', getattr(triton, '__version__', 'unknown'))"])
    run([str(py), "-c", "import flash_attn; print('flash_attn', getattr(flash_attn, '__version__', 'unknown'))"])
    run([str(py), "-c", "import sageattention; print('sageattention import: ok')"])
    verify_runtime_imports(py)
...
def verify_install(py: Path, repo_dir: Path, model_dir: Path, turbo_model_dir: Path, full_base: bool) -> None:
    ...
    run([str(py), "-c", "import torch, transformers, diffusers, omegaconf, huggingface_hub; print('torch', torch.__version__, 'cuda', torch.version.cuda, 'cuda_available', torch.cuda.is_available()); import triton; print('triton', getattr(triton, '__version__', 'unknown')); import flash_attn; print('flash_attn', getattr(flash_attn, '__version__', 'unknown')); import sageattention; print('sageattention import: ok')"])
