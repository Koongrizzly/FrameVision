...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    ap.add_argument("--bin-dir", default="", help="Destination folder for the Qwen-specific sd-cli.exe + DLLs (optional). Default: <root>/presets/bin/extra_sdcli")
    ...
    if not args.no_cli:
        # Default bin dir: <root>/presets/bin/extra_sdcli (Qwen-specific special sd-cli build)
        bin_dir = Path(args.bin_dir).resolve() if args.bin_dir else (root / "presets" / "bin" / "extra_sdcli")
        print(f"[QWEN2512] Extra sd-cli dir: {bin_dir}")
