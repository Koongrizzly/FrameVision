...
REM - Pins CUDA 12.6 PyTorch 2.6 and prevents it from being overwritten
...
REM    We install torch/vision/audio explicitly from the cu126 index
REM    so they do NOT get replaced by CPU wheels.
REM ------------------------------------------------

echo.
echo [INFO] Installing PyTorch (CUDA 12.6, Torch 2.6.0, pinned)...
"%VENV_DIR%\Scripts\python.exe" -m pip install --upgrade --force-reinstall ^
  torch==2.6.0 torchvision==0.21.0 torchaudio==2.6.0 ^
  --index-url https://download.pytorch.org/whl/cu126
if errorlevel 1 goto :fatal_error

echo.
echo [INFO] Verifying CUDA PyTorch install...
"%VENV_DIR%\Scripts\python.exe" -c "import torch; print('torch', torch.__version__); print('cuda runtime', torch.version.cuda); print('cuda available', torch.cuda.is_available()); print('gpu', torch.cuda.get_device_name(0) if torch.cuda.is_available() else 'NONE')"
if errorlevel 1 goto :fatal_error
...
REM ------------------------------------------------
REM 7) Install optional attention backends LAST
REM    Triton-Windows 3.2 is the PyTorch 2.6-compatible line.
REM    FlashAttention wheel must match: Windows + Python 3.11 + CUDA 12.6 + Torch 2.6.0.
REM ------------------------------------------------

echo.
echo [INFO] Installing Triton-Windows for PyTorch 2.6...
"%VENV_DIR%\Scripts\python.exe" -m pip install --upgrade "triton-windows>=3.2,<3.3"
if errorlevel 1 goto :fatal_error

echo.
echo [INFO] Installing FlashAttention wheel for CUDA 12.6 / Torch 2.6.0 / Python 3.11...
"%VENV_DIR%\Scripts\python.exe" -m pip install --upgrade "https://huggingface.co/lldacing/flash-attention-windows-wheel/resolve/main/flash_attn-2.7.4+cu126torch2.6.0cxx11abiFALSE-cp311-cp311-win_amd64.whl?download=true"
if errorlevel 1 goto :fatal_error

echo.
echo [INFO] Verifying FlashAttention import...
"%VENV_DIR%\Scripts\python.exe" -c "import flash_attn; print('flash_attn', getattr(flash_attn, '__version__', 'unknown'))"
if errorlevel 1 goto :fatal_error

REM ------------------------------------------------
REM 8) Download WAN 2.2-5B weights from Hugging Face via Python
...
REM ------------------------------------------------
REM 9) Download + merge GitHub repo (Wan2.2 main)
REM ------------------------------------------------
