...
# Keep HiDream runtime stable. Text-only can survive newer packages, but
# edit/reference-image conditioning is sensitive to the Qwen3VL/processor stack.
HIDREAM_DEPS = [
    "transformers==4.57.1",
    "tokenizers==0.22.2",
    "diffusers==0.35.1",
...
def pip_install(env_python: Path, packages: list[str], extra_args: list[str] | None = None, check: bool = True) -> subprocess.CompletedProcess:
    cmd = [str(env_python), "-m", "pip", "install", "--no-warn-script-location"]
    if extra_args:
...
if not torch.cuda.is_available():
    raise SystemExit(10)
if str(torch.__version__) != "2.8.0+cu128":
    raise SystemExit(11)
...
def project_root() -> Path:
    """Resolve FrameVision root from this installer location.

    Expected location:
        <root>/presets/extra_env/hidream_install.py
    """
...
def write_hidream_constraints(root_dir: Path) -> Path:
    """Write constraints that prevent PyPI from replacing CUDA Torch with CPU Torch."""
    p = Path(root_dir) / "presets" / "extra_env" / "_hidream_constraints.txt"
...
def verify_hidream_torch_stack(env_python: Path) -> None:
    """Hard guard: HiDream must stay on torch 2.8.0+cu128 / torchvision 0.23.0+cu128."""
    code = r"""
import torch
import torchvision
print("torch:", torch.__version__)
...
if torch.__version__ != "2.8.0+cu128":
    raise SystemExit("Wrong torch version; expected 2.8.0+cu128")
if str(torch.version.cuda) != "12.8":
    raise SystemExit("Wrong torch CUDA build; expected 12.8")
if not torch.cuda.is_available():
    raise SystemExit("CUDA is not available in torch")
if torchvision.__version__ != "0.23.0+cu128":
    raise SystemExit("Wrong torchvision version; expected 0.23.0+cu128")
""".strip()
    run([str(env_python), "-c", code])
...
def ensure_python_packages(env_python: Path) -> None:
    ...
    run([str(env_python), "-m", "pip", "install", "--no-warn-script-location", "--upgrade", "pip", "setuptools", "wheel"])
    install_torch_stack(env_python)
    log("Installing pinned HiDream dependencies without allowing Torch drift.")

    constraints = write_hidream_constraints(project_root())

    pip_install(env_python, HIDREAM_DEPS, extra_args=[
        ...
        "--extra-index-url", TORCH_INDEX_CU128,

        "-c", str(constraints),

    ])

    verify_hidream_torch_stack(env_python)
...
def patch_pipeline_for_attention(env_python: Path) -> None:
    """Patch the official pipeline for FrameVision's stable HiDream path.

    FlashAttention can import successfully while still producing bad reference/edit
    conditioning through the Qwen3VL vision path. Text-to-image may look fine, but
    edit/reference runs can produce flat green/blank results. Therefore FrameVision
    uses the safer non-flash Qwen3VL path by default.

    Set HIDREAM_KEEP_FLASH_ATTN=1 before running this installer only for manual
    experiments.
    """
    ...
    if os.environ.get("HIDREAM_KEEP_FLASH_ATTN", "").strip().lower() in {"1", "true", "yes", "on"}:
        log("HIDREAM_KEEP_FLASH_ATTN=1 set. Leaving official pipeline use_flash_attn setting unchanged.")
        return

    if '"use_flash_attn": False' in text:
        log("Pipeline already patched to use_flash_attn=False for stable edit/reference generation.")
        return
    if '"use_flash_attn": True' not in text:
        log("Could not find exact use_flash_attn=True text. Skipping no-flash patch.")
        return

    backup = pipeline.with_suffix(".py.bak_before_framevision_no_flash_patch")
    if not backup.exists():
        backup.write_text(text, encoding="utf-8")
    pipeline.write_text(text.replace('"use_flash_attn": True', '"use_flash_attn": False'), encoding="utf-8")
    log("Patched official pipeline to use_flash_attn=False for stable edit/reference generation.")
...
import importlib.metadata as md
for pkg in ["tokenizers", "diffusers", "accelerate", "pillow", "numpy", "safetensors", "huggingface_hub"]:
    try:
...
for name in ["triton", "flash_attn", "sageattention", "PySide6"]:
    try:
        ...
        # Flash/Sage are optional for FrameVision's stable edit path because the
        # pipeline is patched to use_flash_attn=False. Triton and PySide6 should
        # still import cleanly.
        if name in ("triton", "PySide6"):
            raise
if not torch.cuda.is_available():
    raise SystemExit("CUDA is not available in torch.")
'''
