...
class MusicDCAE(ModelMixin, ConfigMixin, FromOriginalModelMixin):
    ...
    def load_audio(self, audio_path):
        """
        Patched: use soundfile instead of torchaudio.load to avoid torchcodec dependency.
        Always returns a 2-channel tensor shaped [channels, samples].
        """
        # Read with soundfile -> numpy array (samples, channels)
        audio, sr = sf.read(audio_path, always_2d=True)
        # Convert to torch tensor [channels, samples]
        audio_tensor = torch.as_tensor(audio, dtype=torch.float32).T
