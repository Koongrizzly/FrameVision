...
REM Uses an existing FrameVision/shared Python and downloads:
...
set "BIN_DIR=%ROOT%\presets\bin\extra_sdcli"


set "DL_SCRIPT=%ROOT%\presets\extra_env\qwen2512_download.py"


set "MARKER=%ROOT%\presets\setsave\installed_qwen2512.txt"
...
if exist "%ROOT%\environments\.images_models\python.exe" set "PY_EXE=%ROOT%\environments\.images_models\python.exe"
if not defined PY_EXE if exist "%ROOT%\python\python.exe" set "PY_EXE=%ROOT%\python\python.exe"
...
if not exist "%ENV_DIR%\Scripts\python.exe" (
  ...
  echo [QWEN2512] Using existing Python (no private venv)...
