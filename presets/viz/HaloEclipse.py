...
# Halo Eclipse
@register_visualizer
class HaloEclipse(BaseVisualizer):
    display_name = "Halo Eclipse"
    def paint(self, p:QPainter, r, bands, rms, t):
        ...
        amp = spring_to("eclipse", 1.0+0.7*env + TUNE_KICK*punch + 1.0*boom, t, hi=4.2)
        R = min(w,h)*0.24*amp
        p.setCompositionMode(QPainter.CompositionMode_Plus)
        for i in range(120):
            th = i*2*pi/120
            lenr = R*0.4*(1+0.8*env) + (20 if gate>0.6 and i%8==0 else 0)
            col = QColor.fromHsv(int((t*50+i*3)%360), 230, 255, 180)
            p.setPen(QPen(col,2))
            p.drawLine(QPointF(cx+R*cos(th), cy+R*sin(th)),
                       QPointF(cx+(R+lenr)*cos(th), cy+(R+lenr)*sin(th)))
        p.setBrush(QBrush(QColor(10,10,14))); p.setPen(QPen(QColor(255,255,255,150),3))
