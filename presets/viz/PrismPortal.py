...
@register_visualizer
class PrismPortal(BaseVisualizer):
    display_name = "Prism Portal"
    def paint(self, p:QPainter, r, bands, rms, t):
        ...
        amp = spring_to("portal", 1.0+0.7*env + TUNE_KICK*punch + 1.0*boom, t, hi=4.2)
        R = min(w,h)*0.26*amp
        # lens/portal body
        grad = QRadialGradient(QPointF(cx,cy), R*1.2)
        grad.setColorAt(0.0, QColor.fromHsv(int((t*80)%360), 200, 255, 160))
        grad.setColorAt(1.0, QColor(0,0,0,0))
        p.setBrush(QBrush(grad)); p.setPen(QPen(QColor(255,255,255,120),3))
        p.drawEllipse(QPointF(cx,cy), R*1.0, R*0.7)
        # caustic lines
        p.setCompositionMode(QPainter.CompositionMode_Plus)
        for i in range(50):
            th = t*2 + i*0.12
            col = QColor.fromHsv((int(t*90 + i*8)%360), 230, 255, 160)
            p.setPen(QPen(col,2))
            p.drawLine(QPointF(cx-R, cy+10*sin(th)), QPointF(cx+R, cy+10*sin(th+1.2)))
