...
_paths = []
@register_visualizer
class CircuitPulse(BaseVisualizer):
    display_name = "Circuit Pulse"
    def paint(self, p:QPainter, r, bands, rms, t):
        ...
        if not _paths:
            for i in range(12):
                _paths.append([(int(_rng.random()*w), int(_rng.random()*h))])
        p.fillRect(r,QBrush(QColor(2,4,8)))
        env, gate, boom, punch, bc = beat_drive(bands, rms, t)
        p.setCompositionMode(QPainter.CompositionMode_Plus)
        for path in _paths:
            if gate>0.6 or _rng.random()<0.1*env:
                x,y=path[-1]
                nx = max(0,min(w,x + _rng.choice([-60,0,60])))
                ny = max(0,min(h,y + _rng.choice([-40,0,40])))
                path.append((nx,ny))
            hue=int((t*80)%360)
            p.setPen(QPen(QColor.fromHsv(hue,230,255,200),3))
            for i in range(1,len(path)):
                p.drawLine(QPointF(*path[i-1]), QPointF(*path[i]))
