...
@register_visualizer
class HologramSpin(BaseVisualizer):
    display_name = "Hologram Spin"
    def paint(self, p:QPainter, r, bands, rms, t):
        ...
        amp = spring_to("holo", 1.0+0.7*env + TUNE_KICK*punch + 0.8*boom, t, hi=3.6)
        R = min(w,h)*0.22*amp
        p.setCompositionMode(QPainter.CompositionMode_Plus)
        for i in range(6):
            ang = t*1.2 + i*pi/3
            x = cx + R*cos(ang); y = cy + R*sin(ang)
            col = QColor.fromHsv(int((t*80 + i*30)%360),230,255,200)
            p.setPen(QPen(col,3)); p.drawLine(QPointF(cx,cy), QPointF(x,y))
        # glitch peel
        if gate>0.6:
            p.setPen(QPen(QColor(255,255,255,200),2))
            p.drawEllipse(QPointF(cx,cy), R*1.1, R*0.8)
