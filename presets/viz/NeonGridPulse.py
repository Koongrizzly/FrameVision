...
@register_visualizer
class NeonGridPulse(BaseVisualizer):
    display_name = "Neon Grid Pulse"
    def paint(self, p:QPainter, r, bands, rms, t):
        ...
        cx, cy = w*0.5, h*0.5
        waveR = min(w,h)*0.08 * spring_to("grid", 1.0+0.8*env + TUNE_KICK*punch + 1.0*boom, t, hi=3.5)
        glitch = (gate>0.6)
        for iy in range(rows):
            for ix in range(cols):
                x = ix*cellw + cellw*0.5; y = iy*cellh + cellh*0.5
                d = ((x-cx)**2 + (y-cy)**2)**0.5
                k = max(0.0, 1.0 - abs(d - waveR)/(waveR*0.8+1))
                hue = int((t*60 + 200*boom + ix*5 + iy*7)%360)
                val = int(60 + 195*k)
                if glitch and (ix+iy+int(t*60))%11==0:
                    # glitch flip
                    hue = (hue+180)%360; val = 255
                p.setBrush(QBrush(QColor.fromHsv(hue, 230, val, 220)))
                p.setPen(QPen(QColor(20,30,40,120),1))
                p.drawRect(QRectF(ix*cellw+2, iy*cellh+2, cellw-4, cellh-4))
