...
_chars = list("ΩΨΣΦ∆λµπ★✦✧✩☼☯☮☄♬♩♪❤")
@register_visualizer
class NimbusGlyphs(BaseVisualizer):
    display_name = "Nimbus Glyphs"
    def paint(self, p:QPainter, r, bands, rms, t):
        ...
        amp = spring_to("nimbus", 1.0+0.8*env + TUNE_KICK*punch + 0.8*boom, t, hi=3.8)
        R = min(w,h)*0.30*amp
        p.setFont(QFont("DejaVu Sans", int(20+10*amp)))
        p.setCompositionMode(QPainter.CompositionMode_Plus)
        for i,ch in enumerate(_chars):
            th = i*2*pi/len(_chars) + t*(0.5+1.2*env)
            x = cx + R*cos(th); y = cy + R*sin(th)
            col = QColor.fromHsv(int((t*60 + i*20)%360), 230, 255, 200)
            p.setPen(QPen(col, 2))
            p.drawText(QPointF(x,y), ch)
        if gate>0.6:
            # one-frame sigil
            p.setPen(QPen(QColor.fromHsv(int(t*90)%360,230,255,255),4))
