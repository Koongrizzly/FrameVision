...
@register_visualizer
class MirrorLabyrinth(BaseVisualizer):
    display_name = "Mirror Labyrinth"
    def paint(self, p:QPainter, r, bands, rms, t):
        ...
        for y in range(rows):
            for x in range(cols):
                rot = (x+y)*0.2 + t*(0.4+0.8*env)
                alpha = int(130 + 100*rms)
                p.setPen(QPen(QColor(160,200,255,alpha),2))
                cx,cy = x*cellw+cellw/2, y*cellh+cellh/2
                rx = center.x() + (cx-center.x())*cos(0.02*(bc%10)) - (cy-center.y())*sin(0.02*(bc%10))
                ry = center.y() + (cx-center.x())*sin(0.02*(bc%10)) + (cy-center.y())*cos(0.02*(bc%10))
                p.drawRect(QRectF(rx-cellw*0.35, ry-cellh*0.35, cellw*0.7, cellh*0.7))
