...
@register_visualizer
class FabricWeave(BaseVisualizer):
    ...
    def paint(self, p:QPainter, r, bands, rms, t):
        ...
        tight = 1.0 + 0.8*env + 0.6*punch
        for y in range(rows):
            for x in range(cols):
                hue=int((t*50 + (x+y)*6)%360)
                p.setPen(QPen(QColor.fromHsv(hue,230,255,160),2))
                # warp threads
                y0 = y*cellh + cellh*0.5 + 6*sin(0.5*x + t*2)*tight
                p.drawLine(QPointF(x*cellw, y0), QPointF((x+1)*cellw, y0))
                # weft threads
                x0 = x*cellw + cellw*0.5 + 6*sin(0.5*y + t*2.3)*tight
                p.drawLine(QPointF(x0, y*cellh), QPointF(x0, (y+1)*cellh))
