...
@register_visualizer
class HologramCubeHead(BaseVisualizer):
    display_name = "Hologram Cube Head"
    def paint(self, p:QPainter, r, bands, rms, t):
        ...
        p.drawRect(QRectF(cx-size/2+off, cy-size/2-off, size, size))
        for dx,dy in ((-size/2,-size/2),(size/2,-size/2),(-size/2,size/2),(size/2,size/2)):
            p.drawLine(QPointF(cx+dx,cy+dy), QPointF(cx+dx+off,cy+dy-off))

        # scanline eyes (state→y offset)
        for side,state in ((-1,_eye_state_L),(1,_eye_state_R)):
            y = cy + {0:-size*0.12, 1:0.0, 2:size*0.12}[state]
            p.setPen(QPen(QColor(200,255,240,220), max(4,int(size*0.04))))
            p.drawLine(QPointF(cx-size*0.30, y), QPointF(cx+size*0.30, y))

        # mouth panel
        ms=_mouth_state; width=size*0.6; base_y=cy+size*0.22
        if ms==0:
            height=size*0.18*(1.0+0.5*(env+punch))
            p.setBrush(QBrush(QColor(30,40,60,220))); p.setPen(QPen(QColor(120,220,255,220),4))
            p.drawRoundedRect(QRectF(cx-width/2, base_y-height/2, width, height), 8,8)
        elif ms==1:
            p.setPen(QPen(QColor(120,220,255,220), max(4,int(size*0.05)))); p.drawLine(QPointF(cx-width/2, base_y), QPointF(cx+width/2, base_y))
        else:
            path=QPainterPath(QPointF(cx-width/2, base_y))
            curve=size*0.12*(1.0+0.6*(env+punch))
            path.cubicTo(QPointF(cx-width*0.2, base_y-curve), QPointF(cx+width*0.2, base_y-curve), QPointF(cx+width/2, base_y))
            p.setPen(QPen(QColor(120,220,255,220), max(4,int(size*0.05)))); p.setBrush(QBrush(QColor(0,0,0,0)))
            p.drawPath(path)
