
from math import sin, cos, pi
from random import random
from PySide6.QtGui import QPainter, QPen, QColor, QBrush, QPainterPath
from PySide6.QtCore import QRectF, QPointF, Qt
from helpers.music import register_visualizer, BaseVisualizer


def _split(bands):
    if not bands:
        return 0.0, 0.0, 0.0
    n = len(bands)
    a = max(1, n // 6)
    b = max(a + 1, n // 2)
    lo = sum(bands[:a]) / a
    mid = sum(bands[a:b]) / max(1, (b - a))
    hi = sum(bands[b:]) / max(1, (n - b))
    return lo, mid, hi


def _env_step(env, target, up=0.5, down=0.2):
    if target > env:
        return (1 - up) * env + up * target
    else:
        return (1 - down) * env + down * target



@register_visualizer
class WireCube(BaseVisualizer):
    display_name = "Wire Cube"

    def __init__(self):
        super().__init__()
        self._env_lo = self._env_mid = self._env_hi = 0.0
        self._angle = 0.0

    def paint(self, p: QPainter, r, bands, rms, t):
        w, h = int(r.width()), int(r.height())
        if w <= 0 or h <= 0:
            return

        p.setRenderHint(QPainter.Antialiasing, True)

        lo, mid, hi = _split(bands)
        self._env_lo = _env_step(self._env_lo, lo + 0.4 * rms, 0.6, 0.24)
        self._env_mid = _env_step(self._env_mid, mid, 0.55, 0.23)
        self._env_hi = _env_step(self._env_hi, hi, 0.6, 0.28)

        p.fillRect(r, QColor(2, 2, 8))

        cx, cy = w * 0.5, h * 0.5
        size = min(w, h) * (0.25 + 0.1 * self._env_lo)

        self._angle += (25 + 70 * self._env_mid) * (1.0 / 60.0)
        ax = self._angle * pi / 180.0
        ay = ax * 0.8
        az = ax * 0.4

        # cube vertices in 3D
        pts3 = []
        for dz in (-1, 1):
            for dy in (-1, 1):
                for dx in (-1, 1):
                    x, y, z = dx, dy, dz

                    # rotation around X
                    y2 = y * cos(ax) - z * sin(ax)
                    z2 = y * sin(ax) + z * cos(ax)

                    # rotation around Y
                    x3 = x * cos(ay) + z2 * sin(ay)
                    z3 = -x * sin(ay) + z2 * cos(ay)

                    # rotation around Z
                    x4 = x3 * cos(az) - y2 * sin(az)
                    y4 = x3 * sin(az) + y2 * cos(az)

                    pts3.append((x4, y4, z3))

        # project to 2D
        pts2 = []
        for x, y, z in pts3:
            depth = 2.2 + z * 0.8
            k = size / depth
            sx = cx + x * k
            sy = cy + y * k
            pts2.append((sx, sy, depth))

        # edges of cube by index
        indices = [
            (0, 1), (0, 2), (1, 3), (2, 3),
            (4, 5), (4, 6), (5, 7), (6, 7),
            (0, 4), (1, 5), (2, 6), (3, 7),
        ]

        p.setCompositionMode(QPainter.CompositionMode_Plus)
        for a, b in indices:
            x1, y1, d1 = pts2[a]
            x2, y2, d2 = pts2[b]
            depth = max(0.0, min(1.0, 1.2 - 0.5 * (d1 + d2) * 0.5))
            hue = int((200 + 120 * depth + 120 * self._env_hi) % 360)
            sat = int(170 + 60 * self._env_mid)
            val = int(150 + 80 * self._env_lo)
            alpha = int(80 + 150 * depth)

            col = QColor.fromHsv(hue, sat, val, alpha)
            width = 2.0 + 2.5 * depth
            p.setPen(QPen(col, width, Qt.SolidLine, Qt.RoundCap))
            p.drawLine(x1, y1, x2, y2)

        p.setCompositionMode(QPainter.CompositionMode_SourceOver)
