...
def gear(p, cx, cy, r, teeth, rot, col):
    p.setPen(QPen(col,3))
    for k in range(teeth):
        th = rot + k*2*pi/teeth
        x1 = cx + r*cos(th); y1 = cy + r*sin(th)
        x2 = cx + (r+10)*cos(th+0.05); y2 = cy + (r+10)*sin(th+0.05)
        p.drawLine(QPointF(x1,y1), QPointF(x2,y2))

@register_visualizer
class RhythmicGears(BaseVisualizer):
    display_name = "Rhythmic Gears"
    def paint(self, p:QPainter, r, bands, rms, t):
        ...
        rot1 = t*(0.6+1.6*env); rot2 = -t*(0.7+1.2*env)
        col = QColor.fromHsv(int((t*80)%360), 230, 255, 220)
        gear(p, cx-80, cy, 50, 14, rot1, col)
        gear(p, cx+80, cy, 60, 18, rot2, col)
        if punch>0.6:
            p.setPen(QPen(QColor(255,220,160,220),3))
            p.drawLine(QPointF(cx,cy), QPointF(cx+_rng.random()*40-20, cy-40))
