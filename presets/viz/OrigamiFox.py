...
class OrigamiFox(BaseVisualizer):
    ...
    def paint(self, p:QPainter, r, bands, rms, t):
        ...
        # head polygon
        p.setBrush(QBrush(QColor(255,160,90,230))); p.setPen(QPen(QColor(255,220,180,220),5))
        pts=[QPointF(cx,cy-size*0.45), QPointF(cx+size*0.45, cy), QPointF(cx, cy+size*0.38), QPointF(cx-size*0.45, cy)]
        p.drawPolygon(pts)

        # ears twitch (highs)
        earY = cy-size*0.55 - 6*(env+gate)
        p.drawPolygon([QPointF(cx-size*0.25, earY), QPointF(cx-size*0.05, cy-size*0.30), QPointF(cx-size*0.40, cy-size*0.10)])
        p.drawPolygon([QPointF(cx+size*0.25, earY), QPointF(cx+size*0.05, cy-size*0.30), QPointF(cx+size*0.40, cy-size*0.10)])

        # triangular eyes (state→triangle orientation)
        for side,state in ((-1,_eye_state_L),(1,_eye_state_R)):
            x=cx+side*size*0.18; y=cy-size*0.10
            d=size*0.10
            if state==0: pts=[QPointF(x-d,y),QPointF(x+d,y),QPointF(x,y+d)]
            elif state==1: pts=[QPointF(x,y-d),QPointF(x+d,y+d),QPointF(x-d,y+d)]
            else: pts=[QPointF(x-d,y-d),QPointF(x+d,y-d),QPointF(x,y+d)]
            p.setBrush(QBrush(QColor(40,20,60,240))); p.setPen(QPen(QColor(255,220,200,220),3))
            p.drawPolygon(pts)

        # mouth/muzzle
        ms=_mouth_state; width=size*0.35; base_y=cy+size*0.10
        if ms==0:
            height=size*0.18*(1.0+0.5*(env+punch))
            p.setBrush(QBrush(QColor(60,30,30,220))); p.setPen(QPen(QColor(255,220,200,220),4))
            p.drawRoundedRect(QRectF(cx-width/2, base_y-height/2, width, height), 6,6)
        elif ms==1:
            p.setPen(QPen(QColor(255,220,200,220), max(3,int(size*0.04)))); p.drawLine(QPointF(cx-width/2, base_y), QPointF(cx+width/2, base_y))
        else:
            path=QPainterPath(QPointF(cx-width/2, base_y))
            curve=size*0.10*(1.0+0.6*(env+punch))
            path.cubicTo(QPointF(cx-width*0.2, base_y-curve), QPointF(cx+width*0.2, base_y-curve), QPointF(cx+width/2, base_y))
            p.setPen(QPen(QColor(255,220,200,220), max(3,int(size*0.04)))); p.setBrush(QBrush(QColor(0,0,0,0)))
            p.drawPath(path)
