...
@register_visualizer
class KaraokeMicrophone(BaseVisualizer):
    display_name = "Karaoke Microphone"
    def paint(self, p:QPainter, r, bands, rms, t):
        ...
        # mic body
        body_w=min(w,h)*0.18*s; body_h=min(w,h)*0.55*s
        p.setBrush(QBrush(QColor(40,40,60,230))); p.setPen(QPen(QColor(160,200,255,220),5))
        p.drawRoundedRect(QRectF(cx-body_w/2, cy-body_h*0.1, body_w, body_h*0.6), 12,12)
        # grill
        gr=body_w*0.9
        p.setBrush(QBrush(QColor(80,80,110,230))); p.setPen(QPen(QColor(200,220,255,220),5))
        p.drawEllipse(QPointF(cx, cy-body_h*0.1), gr*0.6, gr*0.6)

        # LED eyes on body (state→x offset)
        ex_off = {0:-body_w*0.18,1:0,2:body_w*0.18}
        for side,state in ((-1,_eye_state_L),(1,_eye_state_R)):
            x=cx+ex_off[state]; y=cy+body_h*0.05
            p.setBrush(QBrush(QColor(120,220,255,230))); p.setPen(QPen(QColor(0,0,0,0),0))
            p.drawEllipse(QPointF(x,y), body_w*0.10, body_w*0.10)

        # mouth across grill
        ms=_mouth_state; width=gr*0.9; base_y=cy-body_h*0.02
        if ms==0:
            height=gr*0.35*(1.0+0.5*(env+punch))
            p.setBrush(QBrush(QColor(30,20,50,220))); p.setPen(QPen(QColor(200,180,255,220),4))
            p.drawRoundedRect(QRectF(cx-width/2, base_y-height/2, width, height), 10,10)
        elif ms==1:
            p.setPen(QPen(QColor(200,180,255,220), max(4,int(body_w*0.20)))); p.drawLine(QPointF(cx-width/2, base_y), QPointF(cx+width/2, base_y))
        else:
            path=QPainterPath(QPointF(cx-width/2, base_y))
            curve=gr*0.20*(1.0+0.6*(env+punch))
            path.cubicTo(QPointF(cx-width*0.2, base_y-curve), QPointF(cx+width*0.2, base_y-curve), QPointF(cx+width/2, base_y))
            p.setPen(QPen(QColor(200,180,255,220), max(4,int(body_w*0.18)))); p.setBrush(QBrush(QColor(0,0,0,0)))
            p.drawPath(path)
