...
_blobs = []
@register_visualizer
class LavaLampFlow(BaseVisualizer):
    display_name = "Lava Lamp Flow"
    def paint(self, p:QPainter, r, bands, rms, t):
        ...
        if not _blobs:
            for i in range(6):
                _blobs.append([_rng.random()*w, _rng.random()*h, 30+50*_rng.random()])
        p.fillRect(r,QBrush(QColor(6,4,8)))
        env, gate, boom, punch, bc = beat_drive(bands, rms, t)
        dt=0.016; p.setCompositionMode(QPainter.CompositionMode_Plus)
        for i in range(len(_blobs)):
            x,y,r=_blobs[i]
            y += (-40 + 80*env)*dt
            if gate>0.6 and _rng.random()<0.3:
                # split
                _blobs.append([x+_rng.random()*20-10, y, r*0.6])
                _blobs[i][2] = r*0.6
            col = QColor.fromHsv(int((t*70+i*40)%360),230,255,180)
            p.setPen(QPen(col,3)); p.setBrush(QBrush(QColor(col.red(),col.green(),col.blue(),80)))
            p.drawEllipse(QPointF(x%w,y%h), r*(1+0.6*boom), r*(0.8+0.4*boom))
