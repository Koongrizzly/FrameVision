...
_petals = []
@register_visualizer
class PetalStorm(BaseVisualizer):
    display_name = "Petal Storm"
    def paint(self, p:QPainter, r, bands, rms, t):
        ...
        if not _petals:
            for i in range(160):
                _petals.append([_rng.random()*w, _rng.random()*h, _rng.random()*2*pi])
        ...
        for i in range(len(_petals)):
            x,y,ang=_petals[i]
            vx = wind + 60*sin(ang+t) + (cx-x)*0.01*boom
            vy = 40*cos(ang*1.3+t*0.6) + (cy-y)*0.01*boom
            if gate>0.6: ang += 0.8  # bloom flip
            x+=vx*dt; y+=vy*dt; _petals[i]=[x%w,y%h,ang]
            col = QColor.fromHsv(int((t*80+i*2)%360),230,255,180)
