...
# --- Singing / Talking Smiley (v3.2) ---
# Eyes: desynced. Left switches every 3 beats, right every 4 beats (random L/C/R).
# Mouth: only three states (OPEN / CLOSE / SMILE) switching every 2 beats.
...
EYE_PERIOD_RIGHT  = 4
MOUTH_PERIOD      = 2
EYE_INTERVAL_LEFT_SEC  = 0.9   # time fallback if gate misses
...
# spring for general scalar (with wide clamp range)
_sdict = {}
...
@register_visualizer
class SingingSmileyV32(BaseVisualizer):
    display_name = "Singing Smiley v3.2"
    def paint(self, p:QPainter, r, bands, rms, t):
        ...
        # beat-driven switches with different periods
        ...
        # eyes (independent states, smooth horizontal slides)
        ...
        if ms == 0:  # OPEN
        ...
        elif ms == 2:  # SMILE (always upwards)
            ...
            # safety: default to SMILE if any other value gets through
            curve = R*(0.22 + 0.18*(env + 0.6*punch))
            path = QPainterPath(QPointF(cx - width*0.55, base_y))
            path.cubicTo(QPointF(cx - width*0.20, base_y - curve),
                         QPointF(cx + width*0.20, base_y - curve),
