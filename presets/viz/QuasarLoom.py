...
@register_visualizer
class QuasarLoom(BaseVisualizer):
    display_name = "Quasar Loom"
    def paint(self, p:QPainter, r, bands, rms, t):
        ...
        amp = spring_to("loom", 1.0+0.8*env + TUNE_KICK*punch + 0.9*boom, t, hi=4.0)
        width = min(w,h)*0.42*amp
        strands = 8
        p.setCompositionMode(QPainter.CompositionMode_Plus)
        for s in range(strands):
            hue = int((t*70 + s*25)%360)
            p.setPen(QPen(QColor.fromHsv(hue,230,255,200), 3))
            prev=None
            for i in range(140):
                f = i/139.0
                y = cy + (f-0.5)*width
                x = cx + 0.5*width*sin( (f*8 + s*0.3)*pi + t*2 + (0.5 if (s==int(t*2)%strands and gate>0.6) else 0.0))
