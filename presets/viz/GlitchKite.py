...
_pose = 0.0
@register_visualizer
class GlitchKite(BaseVisualizer):
    display_name = "Glitch Kite"
    def paint(self, p:QPainter, r, bands, rms, t):
        ...
        if gate>0.6: _pose = (_pose + 1.0) % 2.0
        cx, cy = r.center().x(), r.center().y() - 40*sin(t*0.8)
        amp = spring_to("kite", 1.0+0.7*env + TUNE_KICK*punch + 0.9*boom, t, hi=4.0)
        size = min(w,h)*0.20*amp
        # kite
        p.setPen(QPen(QColor(200,240,255,220),3)); p.setBrush(QBrush(QColor(50,70,110,160)))
        pts = [QPointF(cx,cy-size), QPointF(cx+size,cy), QPointF(cx,cy+size), QPointF(cx-size,cy)]
        path = QPainterPath(pts[0]); [path.lineTo(pt) for pt in pts[1:]]; path.closeSubpath()
        p.drawPath(path)
        # string
        p.setPen(QPen(QColor(180,200,255,180),2))
        for i in range(1,15):
            y = cy + size + i*18
            x = cx + 30*sin(0.15*y + t*3 + _pose*pi)
