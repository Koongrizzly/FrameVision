...
_lines = []
@register_visualizer
class NeonCircuitry(BaseVisualizer):
    display_name = "Neon Circuitry"
    def paint(self, p:QPainter, r, bands, rms, t):
        ...
        if not _lines:
            for i in range(40):
                _lines.append([_rng.random()*w, _rng.random()*h, _rng.random()*w, _rng.random()*h])
        p.fillRect(r,QBrush(QColor(4,4,8)))
        env, gate, boom, punch, bc = beat_drive(bands, rms, t)
        p.setCompositionMode(QPainter.CompositionMode_Plus)
        for i in range(len(_lines)):
            x1,y1,x2,y2=_lines[i]
            if gate>0.6: x2+=(_rng.random()-0.5)*80; y2+=(_rng.random()-0.5)*80
            hue=int((t*90 + i*7)%360)
