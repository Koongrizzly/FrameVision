...
@register_visualizer
class SpectralWaves(BaseVisualizer):
    display_name = "Spectral Waves"
    def paint(self, p:QPainter, r, bands, rms, t):
        ...
        A = 16 + 60*(env + 0.6*punch)
        for k in range(6):
            hue = int((t*60 + k*50)%360)
            p.setPen(QPen(QColor.fromHsv(hue,230,255,200),2))
            for x in range(0, w, 6):
                y = h/2 + A*sin(0.012*x + t*(1.0+k*0.15)) + A*0.5*sin(0.023*x + t*1.6 + k)
                y += 12*boom*sin(0.004*x + t*3)
