...
def install_queue_toggle_play_last(pane_widget, grid_layout, config: dict, save_config_callable, done_dir: Path):
    ...
    try:
        ...
        cb_cleanup.setToolTip("When enabled, the app will archive old Done/Failed jobs before hitting the limit by moving their job JSONs to: jobs\\done\\old_jobs")
        cb_cleanup.setChecked(bool(config.get("queue_auto_cleanup", False)))
