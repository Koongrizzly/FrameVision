...
from PySide6.QtCore import Qt, QTimer, QObject, QThread, Signal, QSettings
...
def _fv_startup_lazy_load_tools_enabled() -> bool:
    ...
    False = full startup / allow eager Describer import for ready-to-use tools.
    True  = fast startup / avoid heavy Describer imports during startup.
    """
    try:
...
# Lightweight model metadata used by Prompt Enhancement.
# Keep this tiny and dependency-free: importing helpers.describer here pulls in
# transformers/torch/torchvision and costs multiple seconds during FrameVision startup.
_LIGHTWEIGHT_QWEN_VL_CATALOG: Dict[str, Dict[str, str]] = {
    "qwen2-vl-2b": {
...
def _lightweight_models_root() -> Path:
    try:
        return (ROOT / "models").resolve()
    except Exception:
        return (Path(".").resolve() / "models").resolve()

def _lightweight_catalog_folder(model_key: str) -> Optional[Path]:
    try:
        meta = _LIGHTWEIGHT_QWEN_VL_CATALOG.get(str(model_key), {})
        folder = meta.get("folder", "")
        if not folder:
            return None
        return (_lightweight_models_root() / folder).resolve()
    except Exception:
        return None
...
# Small optional style spices (applied sometimes) to add gentle variety without changing meaning
STYLE_SPICE_VARIANTS = [
...
def _list_qwen_vl_models() -> List[Tuple[str,str]]:
    """
    Discover available Qwen-VL models.

    Fast startup mode keeps this filesystem-only so helpers.describer does not
    import transformers/torch/torchvision during app startup.

    Full startup mode intentionally restores the older eager behavior: import
    helpers.describer and use its catalog so supported tools are ready sooner.
    """
    ...
    if not _fv_startup_lazy_load_tools_enabled():
        try:
            ...
            catalog = getattr(D, "ENGINE_CATALOG", {}) or {}
            try:
                root = Path(getattr(D, "models_root")())
            except Exception:
                root = _lightweight_models_root()
            for k, meta in catalog.items():
                try:
                    folder = root / str(meta.get("folder", ""))
                    if not (folder.exists() and any(folder.iterdir())):
                        continue
                    fkey = str(folder.resolve()).lower()
                    if fkey in seen_folders:
                        continue
                    label = str(meta.get("label", k))
                    out.append((str(k), label))
                    seen_folders.add(fkey)
                except Exception:
                    continue
        except Exception:
            # Fall back to the lightweight scan below.
            pass
    ...
    try:
        ...
        if q3.exists() and any(q3.iterdir()):
            fkey = str(q3.resolve()).lower()
            if fkey not in seen_folders:
                out.append((QWEN3_LOCAL_KEY, "Qwen3-VL 2B (local)"))
                seen_folders.add(fkey)
    except Exception:
        pass

    # Lightweight catalog mirror for known describer Qwen/VL engines.
    # Do not import helpers.describer here in fast startup mode.
    try:
        for k, meta in _LIGHTWEIGHT_QWEN_VL_CATALOG.items():
            folder = _lightweight_catalog_folder(k)
            if folder is None or not (folder.exists() and any(folder.iterdir())):
                continue
            fkey = str(folder.resolve()).lower()
            if fkey in seen_folders:
                continue
...
def _models_root_path()->Optional[Path]:
    # Lightweight default first. Do not import helpers.describer during startup.
    try:
        return _lightweight_models_root()
    except Exception:
        pass

    # Fallback for legacy/custom describer setups. This should only run when an
    # actual generation path needs a non-lightweight model key.
    try:
        try:
            import helpers.describer as D  # type: ignore
        except Exception:
...
def _folder_for_model_key(model_key:str)->Optional[Path]:
    ...
    # Known Qwen/VL catalog entries can resolve without importing Describer.
    try:
        folder = _lightweight_catalog_folder(model_key)
        if folder is not None:
            return folder
    except Exception:
        pass

    # Fallback for legacy/custom describer catalog keys. This is intentionally
    # delayed until the user actually generates with such a model.
    try:
        try:
            import helpers.describer as D  # type: ignore
        except Exception:
...
# ---------- Collapsible UI helper ----------
class CollapsibleBox(QWidget):
    """A simple arrow-header collapsible container (no checkbox toggle UI)."""
    def __init__(self, title: str = "Advanced", parent=None, start_collapsed: bool = True):
...
class PromptToolPane(QWidget):
    ...
    def __init__(self, main=None, parent=None):
        ...
        self.advanced_box = CollapsibleBox("Advanced settings", self, start_collapsed=True)
        ...
        # Variations / rewrite tools moved to their own row (under Generate, above Result)
        var_row = QHBoxLayout()
        var_row.addWidget(QLabel("Variations"))
        var_row.addWidget(self.var_count)
        var_row.addWidget(self.btn_rephrase)
        var_row.addWidget(self.btn_shuffle)
    ...
    def _compose_prompts(self, seed:str)->tuple[str,str,str]:
        ...
        # Optional small style spice to increase variety (applied sometimes)
        try:
            import random as _r
            if _r.random() < 0.35:
                spice = _r.choice(STYLE_SPICE_VARIANTS)
                if spice:
                    style = (f"{spice}, {style}" if style else spice)
        except Exception:
            pass
