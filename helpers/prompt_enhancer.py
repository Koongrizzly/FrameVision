...
# --------------------------------------------------------------------------- #
# FrameVision local Qwen3-VL backend
# --------------------------------------------------------------------------- #
# FRAMEVISION_LOCAL_QWEN_PE_V1

_LOCAL_QWEN_CACHE: Dict[str, Tuple[Any, Any, Any, Any]] = {}


def _fv_find_root() -> Path:
    ...
    for c in candidates:
        try:
            if (c / "presets").exists() or (c / "models").exists() or (c / "helpers").exists():
                return c.resolve()
        except Exception:
            pass
...
def _fv_qwen3_folder() -> Path:
    override = os.environ.get("BERNINI_PE_QWEN_MODEL_DIR", "").strip()
    if override:
        return Path(override).expanduser().resolve()
    return (_fv_find_root() / "models" / "describe" / "default" / "qwen3vl2b").resolve()
...
def _fv_load_qwen3(model_dir: Optional[Path] = None):
    folder = Path(model_dir or _fv_qwen3_folder())
    if not (folder.exists() and any(folder.iterdir())):
        raise RuntimeError(f"Qwen3-VL local model folder not found: {folder}")
    key = str(folder.resolve())
    cached = _LOCAL_QWEN_CACHE.get(key)
    if cached is not None:
        return cached

    device, dtype = _fv_choose_device()
    ...
    try:
        ...
        try:
            ...
            raise RuntimeError(f"Could not import a compatible Qwen VLM class: {exc}") from exc
    ...
    try:
        ...
        pass
    packed = (processor, model, device, dtype)
    _LOCAL_QWEN_CACHE[key] = packed
    return packed


def _fv_b64_to_image(b64_text: str):
    raw = base64.b64decode(str(b64_text))
    return Image.open(BytesIO(raw)).convert("RGB")


def _fv_clean_text(text: str) -> str:
    s = str(text or "").strip()
    s = re.sub(r"^```(?:json|JSON|text)?\s*", "", s)
    s = re.sub(r"\s*```$", "", s)
    return s.strip()


def _fv_first_json_block(text: str) -> Optional[str]:
    s = _fv_clean_text(text)
    if not s:
        return None
...
def _fv_normalize_pe_response(text: str, json_mode: bool = False) -> str:
    s = _fv_clean_text(text)
    block = _fv_first_json_block(s)
    if block:
        try:
            data = json.loads(block)
            if isinstance(data, dict):
                for key in ("rewritten_text", "prompt", "enhanced_prompt", "text", "result"):
                    val = data.get(key)
            ...
            pass
    if json_mode:
        m = re.search(r"rewritten_text\s*[:=]\s*(.+)", s, flags=re.IGNORECASE | re.DOTALL)
        if m:
            return m.group(1).strip().strip('"')
    return s.strip()


def _fv_qwen_chat(system_prompt: str, user_text: str, images_b64: Sequence[str], json_mode: bool = False) -> str:
    processor, model, device, _dtype = _fv_load_qwen3()
    pil_images = []
    for b64 in images_b64 or []:
        try:
            pil_images.append(_fv_b64_to_image(b64))
        except Exception as exc:
            logger.warning("Could not decode PE image: %s", exc)
    ...
    proc_kwargs: Dict[str, Any] = {"text": [chat_text], "return_tensors": "pt", "padding": True}
    if pil_images:
    ...
    import torch
    max_tokens = int(os.environ.get("BERNINI_PE_MAX_NEW_TOKENS", "1400" if json_mode else "1000"))
    ...
    raw = processor.batch_decode(new_ids, skip_special_tokens=True)[0].strip()
    return _fv_normalize_pe_response(raw, json_mode=json_mode)


class PromptEnhancer:
    """Task-aware prompt enhancer.

    FrameVision patch: by default this uses the local bundled Qwen3-VL model
    instead of requiring an OpenAI-compatible API endpoint. Set
    BERNINI_PE_BACKEND=api to force the original API backend.
    """
    ...
    def __init__(
        ...
        requested_backend = (os.environ.get("BERNINI_PE_BACKEND") or "local_qwen").strip().lower()
        self.backend = requested_backend
        self.client = None
        if requested_backend in ("api", "openai", "endpoint"):
    ...
    def _chat(self, system_prompt, user_text, images_b64, json_mode=False) -> Optional[str]:
        if self.backend == "local_qwen":
            try:
                return _fv_qwen_chat(system_prompt, user_text, images_b64, json_mode=json_mode)
            except Exception as e:  # noqa: BLE001
                logger.error("Local Qwen PE failed: %s", e)
                return None
