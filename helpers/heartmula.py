...
"""
    ...
    def _rebuild_preset_combo(self) -> None:
        ...
        try:
            ...
            def add_preset_item(genre_name: str, preset: dict, prefix_genre: bool) -> None:
                ...
                # Optional per-preset params (older presets may not have these).
                for k in ("cfg_scale", "temperature", "topk", "max_audio_length_ms"):
                    if k in preset:
                        payload[k] = preset.get(k)
    ...
    def _on_preset_changed(self) -> None:
        ...
        # Optional: load per-preset settings (custom presets can store these).
        try:
            if "topk" in data and data.get("topk") is not None:
                self.topk.setValue(int(data.get("topk")))
            if "temperature" in data and data.get("temperature") is not None:
                self.temperature.setValue(float(data.get("temperature")))
            if "cfg_scale" in data and data.get("cfg_scale") is not None:
                self.cfg_scale.setValue(float(data.get("cfg_scale")))
            if "max_audio_length_ms" in data and data.get("max_audio_length_ms") is not None:
                self.max_len.setTime(_ms_to_time(int(data.get("max_audio_length_ms"))))
        except Exception:
            # Never block preset loading on bad values.
            pass
    ...
    def _add_preset(self) -> None:
        ...
        # If a preset with the same name already exists, ask whether to replace or cancel.
        # (User expectation: keep the name, but overwrite the old preset.)
        name_l = name.strip().lower()
        existing_idxs = [
            i
            for i, p in enumerate(presets)
            if isinstance(p, dict) and str(p.get("name", "")).strip().lower() == name_l
        ]

        if existing_idxs:
            msg = QMessageBox(self)
            msg.setIcon(QMessageBox.Question)
            msg.setWindowTitle("Preset exists")
            msg.setText(f"A preset named '{name}' already exists in '{genre}'.")
            msg.setInformativeText("Do you want to remove the old preset and replace it with the new one?")
            btn_replace = msg.addButton("Replace", QMessageBox.AcceptRole)
            msg.addButton("Cancel", QMessageBox.RejectRole)
            msg.setDefaultButton(btn_replace)
            msg.exec()
            if msg.clickedButton() is not btn_replace:
                return

        new_preset = {
            "name": name,
            "tags": tags,
            "cfg_scale": float(self.cfg_scale.value()),
            "temperature": float(self.temperature.value()),
            "topk": int(self.topk.value()),
            "max_audio_length_ms": _time_to_ms(self.max_len.time()),
        }

        if existing_idxs:
            # Replace first match, remove any additional duplicates.
            first = existing_idxs[0]
            presets[first] = new_preset
            for i in reversed(existing_idxs[1:]):
                try:
                    del presets[i]
                except Exception:
                    pass
            self._append_log(f"Preset replaced: {genre} / {name}")
        else:
    ...
    def _validate_install(self) -> tuple[bool, str]:
        ...
        return True, ""
    def _write_triton_probe_wrapper(self, path: Path) -> None:
        """Write a tiny wrapper that logs Triton/SDPA status before/after generation.

        This is the most reliable way to answer:
          - "Is Triton installed / importable?"
          - "Did Triton actually JIT/emit cache files during this run?"
        """
        code = r"""# Auto-generated by helpers/heartmula.py (FrameVision)
# Purpose: log whether Triton is present AND whether it appears to be used (cache/JIT activity).
import os
import sys
import time
import runpy
...
def _cache_roots():
    ...
    te = os.environ.get("TEMP")
    if la:
        roots.append(Path(la) / "triton")
    if te:
        roots.append(Path(te) / "triton")
    # De-dup
    out = []
    seen = set()
    for r in roots:
    ...
    for r in roots:
        if r.exists():
            ...
            try:
                ...
                pass
    return {"roots": existing, "files": files, "newest_mtime": newest}
...
def _torch_sdpa_flags(torch):
    # Best-effort across torch versions
    out = {}
    try:
        bc = getattr(torch, "backends", None)
        cuda = getattr(bc, "cuda", None) if bc else None
        if cuda:
...
def _torch_sdpa_kernel(torch):
    # torch.backends.cuda.sdpa_kernel() changed across versions; keep it safe.
    try:
        cuda = getattr(getattr(torch, "backends", None), "cuda", None)
        fn = getattr(cuda, "sdpa_kernel", None)
        if callable(fn):
            try:
                return fn()
...
def _inspect_torchtune():
    info = {}
    try:
        import torchtune.modules.attention_utils as au  # type: ignore
        # Grab any obvious "has triton / backend" flags if they exist
        for k in dir(au):
            lk = k.lower()
            if "triton" in lk or "flash" in lk or "sdp" in lk or "xform" in lk:
                try:
                    v = getattr(au, k)
        ...
        info["torchtune_import_error"] = str(e)
    return info

def main():
    if len(sys.argv) < 2:
        _log("[TRITON] Probe wrapper: missing target script argument.")
        return 2

    target = sys.argv[1]
    forwarded = sys.argv[2:]

    _log("---- TRITON / ATTENTION PROBE (pre) ----")
    pre = _cache_stats()
    _log(f"[TRITON] Cache roots found: {pre['roots'] if pre['roots'] else '(none)'}")
    _log(f"[TRITON] Cache file count (pre): {pre['files']}")
    ...
    try:
        ...
        _log(f"[TRITON] Import: FAIL ({e})")
    if triton_ok:
        _log(f"[TRITON] Import: OK (version {triton_ver})")

    # Torch / CUDA info
    try:
        import torch  # type: ignore
        _log(f"[TORCH] Version: {getattr(torch, '__version__', '?')}")
        try:
            _log(f"[TORCH] CUDA available: {_bool_str(torch.cuda.is_available())}")
            if torch.cuda.is_available():
                try:
                    _log(f"[TORCH] GPU: {torch.cuda.get_device_name(0)}")
                except Exception:
                    pass
        except Exception:
            pass

        # SDPA flags + kernel state
        flags = _torch_sdpa_flags(torch)
        if flags:
            _log("[TORCH] SDPA flags: " + ", ".join(f"{k}={v}" for k, v in flags.items()))
        kstate = _torch_sdpa_kernel(torch)
        if kstate is not None:
            _log(f"[TORCH] torch.backends.cuda.sdpa_kernel(): {kstate}")
    except Exception as e:
        _log(f"[TORCH] Import failed: {e}")

    # Torchtune (HeartMuLa stack) hints
    tt = _inspect_torchtune()
    if tt:
        _log("[TORCHTUNE] Attention-related flags (best-effort):")
        for k in sorted(tt.keys()):
            _log(f"  - {k}: {tt[k]}")

    # Run the target script in-process so we can check cache activity after it finishes.
    exit_code = 0
    sys.argv = [target] + forwarded
    try:
        runpy.run_path(target, run_name="__main__")
    except SystemExit as e:
        # Many scripts use sys.exit(); preserve it.
        try:
            exit_code = int(getattr(e, "code", 0) or 0)
        except Exception:
            exit_code = 0
    except Exception as e:
        _log(f"[TRITON] Target script raised: {e}")
        exit_code = 1

    _log("---- TRITON / ATTENTION PROBE (post) ----")
    post = _cache_stats()
    _log(f"[TRITON] Cache file count (post): {post['files']}")
    delta = int(post["files"]) - int(pre["files"])
    used = (delta > 0) or (float(post["newest_mtime"]) > float(pre["newest_mtime"]))
    _log(f"[TRITON] Cache delta: {delta}  -> Triton activity detected: {_bool_str(used)}")
    if not used and triton_ok:
        _log("[TRITON] Note: Triton can be installed but unused (fallback path).")
    return exit_code
...
"""
    ...
    def _generate(self) -> None:
        ...
        wrapper = tmp_dir / "run_with_triton_probe.py"
        try:
            self._write_triton_probe_wrapper(wrapper)
        except Exception as e:
            self._append_log(f"[TRITON] Probe wrapper write failed: {e}")
            wrapper = None
