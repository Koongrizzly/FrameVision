...
ACE_ENV_DIR = ROOT_DIR / "presets" / "extra_env" / ".ace_env"
ACE_REPO_DIR = ACE_ENV_DIR / "ACE-Step"  # ACE-Step repo inside the env
...
class AceLastManager:
    """Keeps a small 'last results' list for ACE-Step.

    Data is stored in:
        ROOT_DIR / "presets" / "setsave" / "acelast.json"

    Each entry has:
        {
            "wav": "relative/or/absolute/path.wav",
            "json": "relative/or/absolute/path.json" or None,
            "mtime": float timestamp,
            "label": display label,
            "info": optional metadata dict (e.g. prompt, duration, seed)
        }
    """
    ...
    def load(self):
        """Load the last-results list from disk (if present)."""
        self.entries = []
        ...
        for raw in items:
            if not isinstance(raw, dict):
                continue
            wav = raw.get("wav") or raw.get("wav_path")
            if not wav:
                continue
            entry = {
                "wav": wav,
                "json": raw.get("json") or raw.get("json_path"),
                "mtime": float(raw.get("mtime", 0.0) or 0.0),
                "label": raw.get("label") or os.path.basename(str(wav)),
                "info": raw.get("info") if isinstance(raw.get("info"), dict) else {},
            }
            self.entries.append(entry)

    def save(self):
        """Persist the last-results list to disk.

        Failures are swallowed so the UI never crashes just because saving fails.
        """
        try:
            self.path.parent.mkdir(parents=True, exist_ok=True)
            data = {"items": self.entries}
    ...
    def rebuild_from_output_dir(self, output_dir: Path, limit: int = 25):
        """Scan an output folder for .wav + sidecar .json files.

        We keep up to 'limit' most recent items (capped at 25).
        """
        entries = []
        try:
                ...
                for wav_path in output_dir.glob("*.wav"):
                    ...
                    if json_path.exists():
                        try:
                            ...
                            meta = {}
                    wav_rel = self._relative_to_root(wav_path)
                    json_rel = self._relative_to_root(json_path) if json_path.exists() else None
                    info = {}
                    if isinstance(meta, dict):
                        prompt = meta.get("prompt") or meta.get("tags") or meta.get("description")
                        if prompt:
                            info["prompt"] = str(prompt)
                        if "audio_duration" in meta:
                            info["audio_duration"] = meta.get("audio_duration")
                        if "seed" in meta:
        ...
        # Cap to the last 'limit' items (but never more than 25)
        limit = max(1, min(int(limit or 25), 25))
        self.entries = entries[:limit]
        self.save()

    def remove_entry(self, entry: dict):
        """Remove a single entry (by wav/json path) and save."""
        key_wav = entry.get("wav")
        key_json = entry.get("json")
        new_entries = []
        for e in self.entries:
            if e.get("wav") == key_wav and e.get("json") == key_json:
                continue
...
class AceUI(QtWidgets.QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.last_manager = AceLastManager(self.root_dir)
    ...
    #    prompt_label = QtWidgets.QLabel("Prompt / Tags (style, genre, mood):")
     #   layout.addWidget(prompt_label)
        ...
        # --- Fixed bottom area (buttons + status) ---
        ...
        self.btn_open_folder.setToolTip("Open the current ACE output folder in your file browser.")
        ...
        # Last results section (collapsible list above the buttons)
        self._build_last_results_section(main_layout)
    ...
    def _build_last_results_section(self, main_layout: QtWidgets.QVBoxLayout):
        """Create the collapsible 'last results' list just above the bottom buttons."""
        # Container frame so we can style/pack this as a separate box
        frame = QtWidgets.QFrame()
        frame.setFrameShape(QtWidgets.QFrame.StyledPanel)
        frame.setObjectName("aceLastResultsFrame")
        ...
        header.setText("Last results")
        header.setToolButtonStyle(QtCore.Qt.ToolButtonTextBesideIcon)
        header.setArrowType(QtCore.Qt.DownArrow)
        header.setCheckable(True)
        header.setChecked(True)
        header.setToolTip(
            "Latest ACE renders from the current output folder (max 25).\n"
            "Right-click an item to delete both the .wav and its JSON sidecar."
        )
        header.clicked.connect(self._toggle_last_results)
        frame_layout.addWidget(header)

        # Inner container holds the actual list widget
        container = QtWidgets.QWidget()
        container_layout = QtWidgets.QVBoxLayout(container)
        container_layout.setContentsMargins(0, 0, 0, 0)
        container_layout.setSpacing(2)

        self.last_results_list = QtWidgets.QListWidget()
        self.last_results_list.setSelectionMode(QtWidgets.QAbstractItemView.SingleSelection)
        self.last_results_list.setContextMenuPolicy(QtCore.Qt.CustomContextMenu)
        self.last_results_list.customContextMenuRequested.connect(self._on_last_results_context_menu)
        self.last_results_list.itemClicked.connect(self._on_last_results_item_clicked)
        self.last_results_list.itemActivated.connect(self._on_last_results_item_clicked)
        self.last_results_list.setMinimumHeight(90)
        container_layout.addWidget(self.last_results_list)

        frame_layout.addWidget(container)

        self.last_results_frame = frame
        self.last_results_header = header
        self.last_results_container = container

        # Start with last results collapsed by default on each launch
        self.last_results_header.setChecked(False)
        self._toggle_last_results(False)

        main_layout.addWidget(frame)

    def _toggle_last_results(self, checked: bool):
        """Expand/collapse the last-results list section."""
        if not hasattr(self, "last_results_container"):
            return
        if checked:
            self.last_results_header.setArrowType(QtCore.Qt.DownArrow)
            self.last_results_container.show()
        else:
            self.last_results_header.setArrowType(QtCore.Qt.RightArrow)
            self.last_results_container.hide()

    def _reload_last_results(self):
        """Scan the current output folder and refresh the last-results UI list."""
        if not hasattr(self, "last_results_list"):
            return

        # Decide which folder to scan (relative to FrameVision root)
        try:
        ...
        # Let the manager rebuild from disk and persist to presets/setsave/acelast.json
        try:
            self.last_manager.rebuild_from_output_dir(out_dir, limit=25)
        except Exception:
            # As a fallback, just load whatever is on disk already
            try:
                self.last_manager.load()
            except Exception:
                return

        from datetime import datetime

        self.last_results_list.blockSignals(True)
        try:
            self.last_results_list.clear()
            for entry in self.last_manager.iter_entries():
                label = entry.get("label") or os.path.basename(str(entry.get("wav", "")))
                if not label:
                    continue
                item = QtWidgets.QListWidgetItem(label)
                # Tooltip assembled from stored info + timestamp
                info = entry.get("info") or {}
                tooltip_parts = []
                prompt = info.get("prompt")
                if prompt:
                    tooltip_parts.append(str(prompt))
                duration = info.get("audio_duration")
                if duration is not None:
                    tooltip_parts.append(f"Duration: {duration}")
                seed = info.get("seed")
                if seed is not None:
                    tooltip_parts.append(f"Seed: {seed}")
                mtime = entry.get("mtime")
                if mtime:
                    try:
                        dt = datetime.fromtimestamp(float(mtime))
                        tooltip_parts.append(f"Saved: {dt.strftime('%Y-%m-%d %H:%M')}")
                    except Exception:
                        pass
                if tooltip_parts:
                    item.setToolTip("\n".join(str(p) for p in tooltip_parts))

                item.setData(QtCore.Qt.UserRole, entry)
                self.last_results_list.addItem(item)
        finally:
            self.last_results_list.blockSignals(False)

    def _on_last_results_context_menu(self, pos):
        """Context menu for the last-results list (right-click)."""
        if not hasattr(self, "last_results_list"):
            return
        item = self.last_results_list.itemAt(pos)
        if item is None:
            return
        ...
        chosen = menu.exec_(self.last_results_list.mapToGlobal(pos))
        if chosen == delete_action:
            self._delete_last_result_entry(item, entry)

    def _delete_last_result_entry(self, item: QtWidgets.QListWidgetItem, entry: dict):
        """Delete the .wav and .json files for a last-result entry, then remove it from the list."""
        wav_path, json_path = self._resolve_last_result_paths(entry)
        errors = []

        # If this file is currently playing in the main player, stop it first so Windows releases the handle.
        try:
            main = self.window() if hasattr(self, "window") else None
            if main is not None and wav_path is not None:
                from pathlib import Path as _P
                try:
                    current = getattr(main, "current_path", None)
                    wp = _P(str(wav_path)).resolve()
                    if current is not None:
                        cp = _P(str(current)).resolve()
                    else:
                        cp = None
                except Exception:
                    cp = None
                    wp = wav_path
                if cp is not None and wp is not None and cp == wp:
                    # Prefer a dedicated helper on MainWindow, fall back to VideoPane method.
                    try:
                        if hasattr(main, "stop_current_media"):
                            main.stop_current_media()
                        elif hasattr(main, "video") and hasattr(main.video, "stop_and_clear"):
                            main.video.stop_and_clear()
                    except Exception:
                        pass
        except Exception:
            pass

        # Delete WAV (with a short retry window for lingering locks)
        if wav_path is not None and wav_path.exists():
            ...
            for attempt in range(3):
                try:
                    ...
                    # On Windows, errno 32 means "file in use by another process".
                    if err_no == 32 and time is not None and attempt < 2:
                        try:
                            time.sleep(0.15)
                        except Exception:
                            pass
        ...
        # If JSON path isn't known, infer it from the WAV path
        if json_path is None and wav_path is not None:
            candidate = wav_path.with_suffix(".json")
            if candidate.exists():
                json_path = candidate
        ...
        # Remove the entry from the in-memory list + persisted acelast.json
        try:
            self.last_manager.remove_entry(entry)
        except Exception:
            pass

        # Remove from the visible list
        row = self.last_results_list.row(item)
        self.last_results_list.takeItem(row)

        # Optionally reload from disk to stay in sync (especially for external/queued jobs)
        try:
            self._reload_last_results()
        except Exception:
            pass
        ...
        if errors:
            try:
                self.status_label.setText(
                    "Status: deleted item, but some files could not be removed (" + "; ".join(errors) + ")."
                )
            except Exception:
                pass
        else:
            try:
                self.status_label.setText("Status: deleted item from disk.")
            except Exception:
                pass

    def _resolve_last_result_paths(self, entry: dict):
        """Turn stored relative/absolute paths into Path objects relative to ROOT_DIR."""
        from pathlib import Path as _P

        wav_rel = entry.get("wav") or entry.get("wav_path")
        json_rel = entry.get("json") or entry.get("json_path")
    ...
    def _on_last_results_item_clicked(self, item: QtWidgets.QListWidgetItem):
        """Play the clicked last-result item in the main FrameVision player."""
        if item is None:
            return
        entry = item.data(QtCore.Qt.UserRole)
        if not isinstance(entry, dict):
        ...
        if wav_path is None:
            try:
                self.status_label.setText("Status: could not resolve audio file for this item.")
            except Exception:
                pass
            return
        try:
            # Reuse the existing 'play last result' pipeline by overriding last_output_path
            self.last_output_path = str(wav_path)
            self._on_play_last_clicked()
        except Exception:
    ...
    def _load_config_into_ui(self):
        ...
        # Restore the last-results list (if the widgets exist)
        try:
            self.last_manager.load()
        except Exception:
            pass
    ...
    def _on_generation_success(self, out_path: str):
        ...
        # Also refresh the "last results" list from the output folder
        try:
            self._reload_last_results()
        except Exception:
            pass
    ...
    def _on_open_folder_clicked(self):
        """Open the current ACE output folder in the OS file browser."""
        try:
            out_rel = self.output_edit.text().strip() or "output/ace"
        except Exception:
            out_rel = "output/ace"
        ...
        try:
            ...
            # Fallback: just show the path in the status label
            try:
