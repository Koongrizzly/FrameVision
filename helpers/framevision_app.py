...
# AskPopup is imported lazily in _open_ask_popup so the chatbot does not load AI libraries during startup.
...
def _migrate_legacy_tree():
    ...
    for p in ["output/video","output/trims","output/screenshots","output/descriptions","output/_temp",
              "jobs/pending","jobs/running","jobs/done","jobs/failed","jobs/stale","logs","models","helpers"]:
...
class VideoPane(QWidget):
            ...
            try:
                ...
                try:
                    QMessageBox.warning(
                        self.window(),
                        "Ask Framie",
                        "Ask Framie could not be loaded.\n\n" + str(exc),
                    )
                except Exception:
                    pass
                return
    ...
    def _apply_compact_label(self, btn, compact: bool, full_text: str, compact_text: str):
        ...
        try:
            ...
            # Keep the custom blue/orange/repeat styling in sync with compact/full mode.
            try:
                obj = str(btn.objectName() or "")
                if obj == "btn_upscale_quick":
                    btn.setStyleSheet(getattr(
                        self,
                        "_btn_upscale_style_compact" if want else "_btn_upscale_style_full",
                        btn.styleSheet(),
                    ))
                elif obj == "btn_ask_chat":
                    btn.setStyleSheet(getattr(
                        self,
                        "_btn_ask_style_compact" if want else "_btn_ask_style_full",
                        btn.styleSheet(),
                    ))
                elif obj == "btn_repeat":
                    self._update_repeat_style()
    ...
    def _update_compact_button_labels(self, allow_restore: bool = True, live_resize: bool = False):
        """Switch Upscale / Ask Framie / Repeat to one-letter labels when space is tight.

        The three action buttons are treated as one group, so they change together:
        Upscale / Ask Framie / Repeat  <->  U / A / R.

        During live splitter/window resizing we avoid changing back and forth on every pixel.
        Full labels are restored only after resize settles, and compacting is delayed until the
        full labels are genuinely close to clipping.
        """
        try:
            bu = getattr(self, "btn_upscale", None)
            ba = getattr(self, "btn_ask", None)
            br = getattr(self, "btn_repeat", None)
            bar = getattr(self, "_controls_bar_layout", None)
            if bu is None or ba is None or bar is None:
                return
            ...
            actions = []
            if bu is not None and bu.isVisible():
                actions.append((bu, full_u, "U"))
            if ba is not None and ba.isVisible():
                actions.append((ba, full_a, "A"))
            if br is not None and br.isVisible():
                actions.append((br, full_r, "R"))
            if not actions:
                return
            ...
            def _need_w(btn, label: str, bold: bool) -> int:
                try:
                    if QFontMetrics is None:
                        return int(btn.sizeHint().width())
                    f = btn.font()
                    f.setBold(bool(bold))
                    fm = QFontMetrics(f)
                    # Padding matches the custom styles. Keep this realistic rather than huge,
                    # otherwise the buttons compact while there is still plenty of room.
                    pad = 32 if not bold else 24
                    return int(fm.horizontalAdvance(str(label))) + pad
                except Exception:
                    return 80 if not bold else 42
            ...
            try:
                ...
                bar_w = max(0, int(self.width()) - int(margins.left()) - int(margins.right()))
            except Exception:
                bar_w = int(self.width())

            other_w = 0
            visible_widgets = []
            action_widgets = {a[0] for a in actions}
            try:
                for i in range(bar.count()):
                    ...
                    if w in action_widgets:
                        continue
                    other_w += int(max(w.width(), w.sizeHint().width()))
            except Exception:
                pass

            try:
                if len(visible_widgets) > 1:
                    other_w += spacing * (len(visible_widgets) - 1)
            except Exception:
                pass

            avail = max(0, bar_w - other_w - 8)
            full_need = sum(_need_w(btn, full, False) for btn, full, letter in actions)
            compact_need = sum(_need_w(btn, letter, True) for btn, full, letter in actions)
            if len(actions) > 1:
                full_need += spacing * (len(actions) - 1)
                compact_need += spacing * (len(actions) - 1)

            currently_compact = any(bool(getattr(btn, "_fv_compact_label", False)) for btn, _full, _letter in actions)

            # Real clipping check, only while labels are currently full. Keep it conservative;
            # this is a safety net, not the main trigger.
            clipped_now = False
            if not currently_compact:
                try:
                    for btn, full, letter in actions:
                        real_w = int(btn.width() or 0)
                        need_w = int(_need_w(btn, full, False))
                        if real_w > 0 and real_w < (need_w - 10):
                            clipped_now = True
                            break
                except Exception:
                    clipped_now = False
            ...
            if bool(live_resize):
                # While dragging, never restore full labels and do not hover around the boundary.
                # Compact only when the full labels are truly too tight. This removes the
                # yes/no/yes/no flicker when shrinking.
                if currently_compact:
                    want_compact = True
                else:
                    want_compact = (avail < (full_need - 10)) or clipped_now
            else:
                # Normal/settled state. Compact slightly before clipping, restore with a smaller
                # margin so it returns faster once there is room again.
                compact_trigger = (avail < (full_need - 4)) or clipped_now
                full_restore = (avail >= (full_need + 22))
                if currently_compact:
                    want_compact = True if not bool(allow_restore) else (not full_restore)
                else:
                    want_compact = compact_trigger

            if avail < compact_need:
                want_compact = True
...
# --- zoom/pan state + mode flag ---
    ...
    def resizeEvent(self, ev):
        try:
            ...
            pass
        try:
            # While dragging the main splitter/window, only allow the buttons to shrink.
            # Restoring to full labels is debounced until resizing has stopped, preventing
            # rapid U/A/R <-> full-name flicker around the threshold.
            self._update_compact_button_labels(allow_restore=False, live_resize=True)
        except Exception:
            pass
        try:
            from PySide6.QtCore import QTimer as _QTimer
            t = getattr(self, "_fv_compact_restore_timer", None)
            if t is None:
                t = _QTimer(self)
                t.setSingleShot(True)
                def _fv_restore_compact_labels():
                    try:
                        self._update_compact_button_labels(allow_restore=True)
                    except Exception:
                        pass
                t.timeout.connect(_fv_restore_compact_labels)
                self._fv_compact_restore_timer = t
            # Restart on every resize event. The full label check runs only after
            # the splitter/window has been still for a short moment.
            t.start(140)
        except Exception:
            pass
...
class MainWindow(QMainWindow):
    ...
    def _tab_order_store_path(self):
        """Small JSON fallback for main-tab order.

        QSettings can return lists differently across PySide/Windows setups. The JSON
        file gives us a boring, readable source of truth and prevents startup from
        silently falling back to the default order after the user reorders tabs.
        """
        try:
            root = ROOT if 'ROOT' in globals() else Path(__file__).resolve().parents[1]
        except Exception:
            root = Path(os.getcwd())
        return Path(root) / "presets" / "setsave" / "main_tabs_order.json"

    def _coerce_tab_order_ids(self, val) -> list[str]:
        """Convert QSettings/JSON variants into a clean list of tab ids."""
        out: list[str] = []
        try:
            if val is None:
                return []
            if isinstance(val, (list, tuple)):
                ...
                if not txt:
                    return []
                raw = None
                # JSON list or dict from our fallback.
                try:
                    import json as _json
                    obj = _json.loads(txt)
                    if isinstance(obj, dict):
                        raw = obj.get("order") or obj.get("ids") or obj.get("main_tabs_order")
                    elif isinstance(obj, list):
                        raw = obj
                except Exception:
                    raw = None
                # Python-list-ish text from some settings backends.
                if raw is None and txt.startswith("[") and txt.endswith("]"):
                    try:
                        import ast as _ast
                        obj = _ast.literal_eval(txt)
                        if isinstance(obj, (list, tuple)):
                            raw = list(obj)
                    except Exception:
                        raw = None
                # Last fallback: comma/semicolon separated ids.
                if raw is None:
                    raw = [x.strip() for x in txt.replace(";", ",").split(",") if x.strip()]
            elif isinstance(val, dict):
                raw = val.get("order") or val.get("ids") or val.get("main_tabs_order") or []
            else:
                raw = []
    ...
    def _load_saved_tab_order_ids(self) -> list[str]:
        """Load saved tab order, preferring the JSON fallback over QSettings."""
        try:
            p = self._tab_order_store_path()
            if p.exists():
                import json as _json
                data = _json.loads(p.read_text(encoding="utf-8", errors="replace"))
                ids = self._coerce_tab_order_ids(data)
                if ids:
                    return ids
        except Exception:
            pass
        try:
            ...
            return self._coerce_tab_order_ids(s.value("main_tabs_order", None))
        except Exception:
            return []
    ...
    def _save_tab_order_ids(self):
        ...
        try:
            s = QSettings("FrameVision", "FrameVision")
            s.setValue("main_tabs_order", ids)
            try:
                s.sync()
            except Exception:
                pass
    ...
    def _restore_saved_tab_order_ids(self):
        try:
            desired = self._load_saved_tab_order_ids()
            if not desired:
                return False

            # Existing installs may have an older saved tab order that does not know
            # about newly added tabs. Inject missing tabs once, but keep all existing
            # saved positions untouched. LLM Chat is inserted before Queue/Settings
            # only when it was missing from the saved order.
            changed = False
            try:
                current_ids = self._current_tab_order_ids()
                for tid in current_ids:
                    ...
                    if tid == "tab_llm_chat":
                        anchors = [
                            desired.index(a)
                            for a in ("tab_queue", "tab_settings")
                            if a in desired
                        ]
                        insert_at = min(anchors) if anchors else len(desired)
                        desired.insert(insert_at, tid)
                    else:
                        desired.append(tid)
            except Exception:
                pass
    ...
    def _post_init_tab_order_setup(self):
        # Do not run this while tabs are still being inserted. Some pane constructors
        # process events, so a QTimer(0) posted early can otherwise save an incomplete
        # or default order and wipe the user's custom layout.
        try:
            if not getattr(self, "_main_tabs_ready_for_order", False):
                QTimer.singleShot(80, self._post_init_tab_order_setup)
                return
        except Exception:
            pass
        ...
        restored = False
        try:
            restored = bool(self._restore_saved_tab_order_ids())
        except Exception:
            restored = False

        # On a true first install there is no saved order yet; persist the default.
        # On existing installs, _restore_saved_tab_order_ids() already saves only when
        # it had to inject a newly added tab.
        try:
            if not restored:
                if not self._load_saved_tab_order_ids():
                    self._save_tab_order_ids()
        except Exception:
            pass
    ...
    def _install_main_tab_tooltips(self):
        """Legacy no-op: main tab hover tooltip was removed because it appeared across the app."""
        try:
            bar = self.tabs.tabBar()
            bar.setToolTip("")
        except Exception:
    ...
    def _install_tab_reorder_behavior(self):
        ...
        try:
            ...
            pass
        # Tab-order restore is triggered after all startup tab insert/remove work is done.
        # Do not post a QTimer here: some pane constructors process events before all
        # tabs exist, which can save the default/incomplete order over the user's order.
        try:
            self._main_tabs_ready_for_order = False
        except Exception:
            pass
    ...
    def __init__(self):
        super().__init__()
        self.setWindowTitle(APP_NAME + " V2.5.2 " + TAGLINE)
        ...
        # Models Manager tab was an older placeholder/legacy tab.
        # Keep the attribute for compatibility, but do not construct/add the old Models UI.
        self.models = None
        self.queue = QueuePane(self, {'BASE': BASE, 'JOBS_DIRS': JOBS_DIRS, 'config': config, 'save_config': save_config})
        try:
            self.queue.setObjectName("tab_queue")
        except Exception:
            pass
...
# <<< FRAMEVISION_EDITOR_INIT_END
        ...
        _main_tabs = [("Edit", self.edit),("Background", self.background),("Media Explorer", self.media_explorer),("Timeline Editor", getattr(self, "timeline_editor", None)),("Tools", self.tools),("Describe", self.describe),("Planner", self.planner),("LLM Chat", getattr(self, "llm_chat", None)),("Queue", self.queue),("Presets", self.presets_tab),("Settings", self.settings)]
        try:
        ...
        # Remove any legacy Models tab that may have been inserted by older saved code/patches.
        try:
            for _i in range(self.tabs.count() - 1, -1, -1):
                if (self.tabs.tabText(_i) or "").strip().lower() == "models":
                    self.tabs.removeTab(_i)
        except Exception:
            pass

        # Safety: Queue is a core tab and must always be present.
        try:
            if self.tabs.indexOf(self.queue) < 0:
                self.tabs.addTab(self.queue, "Queue")
        except Exception:
            pass

        # Safety: LLM Chat is a core tab once the helper exists. Older saved tab
        # orders should not be able to make it disappear or end up buried after Settings.
        try:
            _llm = getattr(self, "llm_chat", None)
            if _llm is not None:
                try:
                    _llm.setObjectName("tab_llm_chat")
                except Exception:
                    pass
                if self.tabs.indexOf(_llm) < 0:
                    _insert_at = -1
                    try:
                        _insert_at = self.tabs.indexOf(self.queue)
                    except Exception:
                        _insert_at = -1
                    if _insert_at is None or _insert_at < 0:
                        try:
                            _insert_at = self.tabs.indexOf(self.settings)
                        except Exception:
                            _insert_at = -1
                    if _insert_at is not None and _insert_at >= 0:
                        self.tabs.insertTab(_insert_at, _llm, "LLM Chat")
                    else:
                        self.tabs.addTab(_llm, "LLM Chat")
        except Exception as _e:
            print("[framevision] LLM Chat restore check failed:", _e)
  ...
  #              if idx_tools is not None and idx_tools >= 0:
   ...
   #     except Exception as _e:
    #        print('[framevision] ace init failed:', _e)
        ...
        # Allow either side of the main splitter to collapse fully.
        # Keep the normal minimum sizes above so the UI does not jitter while dragging,
        # but let QSplitter ignore those minimums at the far edges.
        try:
        ...
        # LLM Chat pseudo-fullscreen uses the main splitter. Keep the splitter usable:
        # dragging the handle back open counts as leaving LLM fullscreen and updates the button.
        try:
        ...
        try:
            self._ensure_scrollbar_on_tabs({"Tools","Upscaler","Upscale","Queue"})
            try:
        ...
        # Safety: Media Explorer is a core tab. Some older tab-order/cleanup patches
        # could accidentally leave the widget constructed but no longer present in the
        # tab bar. Restore the existing widget instead of touching media_explorer_tab.py.
        try:
            _me = getattr(self, "media_explorer", None)
            if _me is None:
                try:
                    _me = self._init_media_explorer_hardfail()
                    self.media_explorer = _me
                except Exception:
                    _me = None

            if _me is not None:
                try:
                    if not str(_me.objectName() or ""):
                        _me.setObjectName("tab_media_explorer")
                except Exception:
                    pass

                _present = False
                try:
                    _present = (self.tabs.indexOf(_me) >= 0)
                except Exception:
                    _present = False
                if not _present:
                    try:
                        _present = (self._find_tab_index_by_id("tab_media_explorer") is not None)
                    except Exception:
                        _present = False
                ...
                if not _present:
                    _inserted = False
                    try:
                        _idx_tools = self.tabs.indexOf(getattr(self, "tools", None))
                    except Exception:
                        _idx_tools = -1
                    try:
                        if _idx_tools is not None and _idx_tools >= 0:
                            self.tabs.insertTab(_idx_tools, _me, "Media Explorer")
                            _inserted = True
                    except Exception:
                        _inserted = False
                    if not _inserted:
                        try:
                            self.tabs.addTab(_me, "Media Explorer")
                        except Exception:
                            pass
        except Exception as _e:
            try:
                print("[framevision] Media Explorer restore check failed:", _e)
            except Exception:
                pass


        # Final tab-order restore now that startup tab inserts/removals have completed.
        try:
            self._main_tabs_ready_for_order = True
            self._post_init_tab_order_setup()
            QTimer.singleShot(250, self._post_init_tab_order_setup)
        except Exception:
            pass
    ...
    def _main_splitter(self):
        try:
            cw = self.centralWidget()
            if isinstance(cw, QSplitter):
                return cw
            if cw is not None:
                return cw.findChild(QSplitter, 'main_splitter') or cw.findChild(QSplitter)
        except Exception:
            pass
        return None

    def _sync_llm_fullscreen_button(self, active: bool):
        """Tell the LLM tab that the FrameVision splitter fullscreen state changed externally."""
        try:
            llm = getattr(self, 'llm_chat', None)
            if llm is not None and hasattr(llm, 'set_framevision_fullscreen_active'):
                llm.set_framevision_fullscreen_active(bool(active))
        except Exception:
            pass

    def _restore_llm_splitter_constraints(self, splitter=None, left=None):
        restore = getattr(self, '_llm_splitter_restore', {}) if hasattr(self, '_llm_splitter_restore') else {}
        try:
            if splitter is None:
                splitter = self._main_splitter()
            if left is None and splitter is not None and splitter.count() >= 1:
                left = splitter.widget(0)
        except Exception:
            pass
        if left is not None:
            try:
                if 'left_min_size' in restore:
                    left.setMinimumSize(restore.get('left_min_size'))
                elif 'left_min_w' in restore:
                    left.setMinimumWidth(int(restore.get('left_min_w', 320)))
                else:
                    left.setMinimumWidth(320)
            except Exception:
                pass
            try:
                if 'left_max_size' in restore:
                    left.setMaximumSize(restore.get('left_max_size'))
                else:
        ...
        if splitter is not None:
            try:
                if 'children_collapsible' in restore:
                    splitter.setChildrenCollapsible(bool(restore.get('children_collapsible')))
                else:
                    splitter.setChildrenCollapsible(True)
            except Exception:
                pass
            try:
                if 'left_collapsible' in restore:
                    splitter.setCollapsible(0, bool(restore.get('left_collapsible')))
            except Exception:
                pass

    def _on_main_splitter_moved(self, pos=None, index=None):
        """Manual splitter movement cancels LLM pseudo-fullscreen.

        The old behavior forced the left pane to width 0 and left the LLM button checked
        until the button was clicked again. This lets the user simply drag the splitter
        back open; once the left pane has visible width, the LLM tab is no longer in
        fullscreen mode.
        """
        try:
            if getattr(self, '_llm_splitter_programmatic', False):
                return
            restore = getattr(self, '_llm_splitter_restore', {}) if hasattr(self, '_llm_splitter_restore') else {}
            if not (isinstance(restore, dict) and restore.get('active')):
                return
            splitter = self._main_splitter()
            if splitter is None or splitter.count() < 2:
                return
            sizes = list(splitter.sizes())
            # A few pixels can appear from style/layout rounding; treat real manual reopening as visible width.
            if len(sizes) >= 2 and int(sizes[0]) > 24:
                left = splitter.widget(0)
                self._restore_llm_splitter_constraints(splitter, left)
                self._llm_splitter_restore = {'active': False}
                self._sync_llm_fullscreen_button(False)
        except Exception:
            pass

    def set_llm_chat_splitter_fullscreen(self, active: bool):
        """Hide/restore the left media pane while keeping the main splitter manually usable."""
        try:
            splitter = self._main_splitter()
            if splitter is None or splitter.count() < 2:
                return
            ...
            active = bool(active)
            if active:
                if not hasattr(self, '_llm_splitter_restore') or not isinstance(getattr(self, '_llm_splitter_restore', None), dict):
                    self._llm_splitter_restore = {}
                restore = self._llm_splitter_restore
                if not restore.get('active'):
                    try:
                        restore['sizes'] = list(splitter.sizes())
                    except Exception:
                        restore['sizes'] = []
                    try:
                        restore['left_min_w'] = int(left.minimumWidth())
                        restore['left_max_w'] = int(left.maximumWidth())
                        restore['left_min_size'] = left.minimumSize()
                        restore['left_max_size'] = left.maximumSize()
                    except Exception:
                        pass
                    try:
                        restore['children_collapsible'] = bool(splitter.childrenCollapsible())
                    except Exception:
                        pass
                    try:
                        restore['left_collapsible'] = bool(splitter.isCollapsible(0))
                    except Exception:
                        pass
                    restore['active'] = True
                try:
                    left.setMinimumWidth(0)
                    # Do NOT set maximumWidth(0): that locks out manual splitter recovery.
                    left.setMaximumWidth(16777215)
                    splitter.setChildrenCollapsible(True)
                    splitter.setCollapsible(0, True)
                except Exception:
                    pass
                try:
                    total = max(1, sum(splitter.sizes()) or splitter.width() or self.width())
                    self._llm_splitter_programmatic = True
                    splitter.setSizes([0, total])
                finally:
                    self._llm_splitter_programmatic = False
                self._sync_llm_fullscreen_button(True)
            else:
                restore = getattr(self, '_llm_splitter_restore', {}) if hasattr(self, '_llm_splitter_restore') else {}
                self._restore_llm_splitter_constraints(splitter, left)
                try:
                    sizes = restore.get('sizes') if isinstance(restore, dict) else None
                    self._llm_splitter_programmatic = True
                    if sizes and len(sizes) >= 2 and sum(sizes) > 0:
                        splitter.setSizes([int(sizes[0]), int(sizes[1])])
                    else:
                        total = max(1000, splitter.width() or self.width())
                        splitter.setSizes([int(total * 0.50), int(total * 0.50)])
                finally:
                    self._llm_splitter_programmatic = False
                try:
                    self._llm_splitter_restore = {'active': False}
            ...
            try:
                print('[framevision] LLM Chat fullscreen splitter toggle failed:', _e)
            except Exception:
                pass
    ...
    def save_session(self):
        ...
        try:
            ...
            pass
        try:
            _restore = getattr(self, '_llm_splitter_restore', {}) if hasattr(self, '_llm_splitter_restore') else {}
            _saved_normal_sizes = _restore.get('sizes') if isinstance(_restore, dict) and _restore.get('active') else None
            _sp = self.centralWidget().findChild(QSplitter)
            if _sp is not None and _saved_normal_sizes and len(_saved_normal_sizes) >= 2:
                _current_sizes = list(_sp.sizes())
                try:
                    _sp.setSizes([int(_saved_normal_sizes[0]), int(_saved_normal_sizes[1])])
                    ss["splitter_state_b64"] = bytes(_sp.saveState().toBase64()).decode("ascii")
                finally:
                    try:
                        _sp.setSizes(_current_sizes)
                    except Exception:
                        pass
    ...
    def closeEvent(self, ev):
        try:
            ...
            if llm is not None:
                try:
                    if hasattr(llm, "_cleanup_boot_thread"):
                        llm._cleanup_boot_thread()
                    if hasattr(llm, "_stop_process_only"):
                        llm._stop_process_only()
                    if hasattr(llm, "_apply_settings_to_current_session"):
                        llm._apply_settings_to_current_session()
                    if hasattr(llm, "_save_all"):
                        llm._save_all()
                except Exception:
                    pass
            self.save_session()
