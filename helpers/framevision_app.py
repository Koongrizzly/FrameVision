...
# FrameVision V2.1.2 — Full-classic UI + NCNN upscalers + branding
            ...
            if getattr(self, "_compare_active", False):
                ...
                if pm_right is not None and (not pm_right.isNull()):
                    ...
                    if sp_left is not None and (not sp_left.isNull()) and sp_right is not None and (not sp_right.isNull()):
                        ...
                        reveal = int(round(sp_right.width() * (wipe / 1000.0)))

                        p.save()
                        p.setClipRect(QRect(oxR, oyR, reveal, sp_right.height()))
                        ...
                        try:
                            ...
                            self._compare_right_ox = int(oxR)
                            self._compare_right_oy = int(oyR)
                            self._compare_right_w = int(max(1, sp_right.width()))
                            self._compare_right_h = int(max(1, sp_right.height()))
                        except Exception:
                            pass
                        try:
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        super().__init__()
        self.setWindowTitle(APP_NAME + " V2.1.2 " + TAGLINE)
