...
# One-stage uses a lighter default hot window than the two-stage workflows.
# User asked for each one-stage profile default to be 2 GB lower, while still
...
# reductions here. Two-stage uses Stage 1 for the first denoise and Stage 2
# for refinement. One-stage behaves closer to the two-stage Stage-2/refine
# pressure profile, so one-stage Auto uses the Stage-2 anchor value for both
...
def _ltx_auto_survival_floor_for_workflow(workflow_group: str) -> float:
    return LTX_AUTO_HOTSET_SURVIVAL_FLOOR_GB if str(workflow_group).strip().lower() == "one_stage" else LTX_AUTO_STAGE2_SURVIVAL_FLOOR_GB
...
def _ltx_auto_max_frames_for_profile(anchors: List[Tuple[int, float, float, float]], vram_profile_gb: int, hard_max_frames: int, workflow_group: str = "two_stage") -> int:
    ...
    if not ordered:
        return 0
    floor = _ltx_auto_survival_floor_for_workflow(workflow_group)
...
def calculate_ltx_auto_vram_settings(vram_profile_gb: int, resolution_key: str, frame_count: int, workflow_group: str = "two_stage") -> Dict[str, Any]:
    ...
    hard_max = int(table.get("max_frames_24gb", anchors[-1][0]))
    ...
    floor = _ltx_auto_survival_floor_for_workflow(workflow)
...
# Official LTX 2.3 two-stage assets, from the bundled LTX-2 README.
...
class TwoStageAssetDownloadWorker(QThread):
    ...
    def run(self) -> None:
        ...
        try:
            ...
            self.finishedWithResult.emit(True, "Two-stage assets are ready.", resolved)
...
class LTX23RunnerWidget(QWidget):
    ...
    def _build_inputs_tab(self) -> None:
        ...
        hint = QLabel(
            "Normal image-to-video use: add a Start image and run. The UI builds the official LTX "
            "--image PATH FRAME STRENGTH command internally. No command flag selection is needed."
        )
    ...
    def _build_quality_tab(self) -> None:
        ...
        for index in range(4):
            row = PathRow(f"LoRA {index + 1}", mode="file", file_filter="Safetensors (*.safetensors);;All files (*.*)")
            ...
            lora_form.addRow("Quality", strength_host)
        ...
        two_stage_group = QGroupBox("Two-stage / HQ requirements")
        ...
        self._set_tooltip(
            ...
            "Required upsampler for two-stage / HQ output. Auto-download stores it in models/ltx23/spatial_upsampler.",
        )
        self.auto_download_two_stage_check = QCheckBox("Auto-download missing two-stage files on Run")
        ...
        self.two_stage_status_label = QLabel("Two-stage asset status: not checked yet")
        ...
        self.two_stage_download_btn = QPushButton("Find / download missing two-stage files")
        self.two_stage_download_btn.setToolTip(
            "Checks the saved paths, searches the LTX model folders, then downloads only missing two-stage assets. "
            "A progress bar is shown while downloading."
        )
        self.two_stage_status_label.setToolTip("Shows the spatial upsampler status.")
        self.two_stage_progress.setToolTip("Download progress for missing two-stage files.")
        ...
        hint = QLabel(
            "These are normal LTX controls. The UI converts them to official LTX arguments such as "
            "--video-cfg-guidance-scale, --video-stg-guidance-scale and --video-rescale-scale."
        )
        hint.setWordWrap(True)
        layout.addWidget(hint)
        layout.addStretch(1)
    ...
    def _build_advanced_tab(self) -> None:
        ...
        self.vram_profile_combo.setToolTip(
            "Select 12/16/24 GB for manual profile defaults, or auto to calculate hotset / Stage 1 / Stage 2 from VRAM profile, resolution, and frame count. "
            + profile_defaults_tip
        )
    ...
    def _set_auto_vram_spin_state(self, auto_mode: bool) -> None:
        for name in ("stable_hotset_budget_gb_spin", "main_hot_window_spin", "stage2_block_size_limit_spin"):
            ...
            try:
                ...
                pass
            spin.setEnabled(not bool(auto_mode))

    def _apply_auto_vram_settings(self, *, update_hint: bool = True) -> Optional[Dict[str, Any]]:
        if not self._is_auto_vram_profile_mode():
            self._set_auto_vram_spin_state(False)
            return None
        self._set_auto_vram_spin_state(True)
    ...
    def _ensure_auto_vram_settings_supported(self) -> bool:
        if not self._is_auto_vram_profile_mode():
            return True
        result = self._apply_auto_vram_settings(update_hint=True) or {}
        if result.get("supported"):
            return True
        QMessageBox.warning(self, "LTX 2.3", str(result.get("reason") or "Auto VRAM profile is unsupported for the selected resolution/frame count."))
        return False
    ...
    def _repair_cross_workflow_limit_defaults(self) -> None:
        ...
        Earlier test builds only had one saved block-limit map, so a one-stage
        default such as 15.0 GB could accidentally become the saved two-stage
    ...
    def _set_block_limit_spin_for_profile(self, profile: str) -> None:
        ...
        if self._is_auto_vram_profile_mode(profile):
            self._apply_profile_residency_default(profile)
            self._apply_auto_vram_settings(update_hint=True)
            return
        self._set_auto_vram_spin_state(False)
        value = self._current_profile_block_limit(profile, workflow_group)
        stage2_default = self._profile_default_stage2_hot_window(profile)
        self._updating_vram_profile_controls = True
        try:
            self.main_hot_window_spin.blockSignals(True)
            self.main_hot_window_spin.setValue(value)
            self.main_hot_window_spin.blockSignals(False)
            if hasattr(self, "stage2_block_size_limit_spin"):
                # Stage 2 follows the selected VRAM profile. Keep it numeric and
                # update it whenever the profile changes, so 24/16/12 switch to
                # 12.0/4.0/2.0 GB instead of keeping the previous profile's value.
                self.stage2_block_size_limit_spin.blockSignals(True)
                self.stage2_block_size_limit_spin.setValue(stage2_default)
                self.stage2_block_size_limit_spin.blockSignals(False)
            self._apply_profile_residency_default(profile)
        finally:
            self._updating_vram_profile_controls = False
        self._update_vram_profile_hint()
    ...
    def _update_vram_profile_hint(self) -> None:
        ...
        if self._is_auto_vram_profile_mode(raw_profile_key):
            ...
            workflow_label = "one-stage" if workflow_group == "one_stage" else "two-stage"
            if result.get("supported"):
                if workflow_group == "one_stage":
                    self.vram_profile_hint.setText(
                        ...
                        f"one-stage block size {float(result.get('stage1_gb', 0.0)):.1f} GB. "
                        f"One-stage Auto follows the tuned Stage-2/refine budget. "
    ...
    def _safe_resolution_pair(self, width: int, height: int) -> Tuple[int, int]:
        """Return a resolution that is safe for all LTX pipelines, including two-stage."""
    ...
    def apply_settings(self, data: Dict[str, Any]) -> None:
        ...
        # use the legacy single map as the two-stage source and derive one-stage
        # defaults separately.
    ...
    def _effective_distilled_lora_strength(self) -> float:
        """Return the normal approved LoRA strength from the Quality tab."""
        try:
    ...
    def _update_test_lora_strength_ui(self) -> None:
        ...
        if disabled:
            ...
            self.test_lora_strength_status_label.setText(
                f"Normal LoRA mode: active strength {value:g} from the Quality tab. Cache mode: {mode}."
            )
    ...
    def _current_batch_size(self) -> int:
        # One-stage should always run with batch size 1. The visible batch-size
        # control only applies to two-stage workflows.
        try:
            pipeline_name = self._effective_pipeline()
        except Exception:
            pipeline_name = self.pipeline_combo.currentText() if hasattr(self, "pipeline_combo") else "one_stage"
    ...
    def _sanitize_extra_for_pipeline(self, pipeline_name: str, extra: List[str]) -> List[str]:
        """Keep one-stage from accidentally receiving the internal two-stage LoRA.

        Normal two-stage runs still pass the distilled LoRA through the dedicated
        CLI option. For one-stage, stale Custom extra args such as --lora pointing
        to the official distilled LoRA can trigger huge RAM-heavy fusion, so strip
        only those internal two-stage assets.
        """
        ...
        while i < len(extra):
            ...
            i += 1
        if removed:
            self._append_log("One-stage guard removed internal two-stage asset args from Custom extra: " + "; ".join(removed))
        return cleaned

    def _update_pipeline_dependent_controls(self) -> None:
        """Update controls that only apply to two-stage style workflows."""
        ...
        # Max batch size only matters for the two-stage workflows in this UI.
        # Keep one-stage simpler by hiding the setting entirely and forcing the
        # command value to 1 while one-stage is selected.
    ...
    def build_command(self, *, randomize_seed: bool = False, prepare_video_inputs: bool = False) -> Tuple[str, List[str], Path, List[str]]:
        ...
        if pipeline_name in ("two_stages", "two_stages_hq", "a2vid_two_stage"):
            ...
            # two-stage parser optional and runs Stage 2 without the official LoRA.
    ...
    def _connect_auto_save(self) -> None:
        widgets: List[QWidget] = [
            ...
            *getattr(self, "user_lora_strength_spins", []), *getattr(self, "user_lora_strength_sliders", []),
            ...
            self.planned_hotset_check, self.stage1_stable_hotset_fraction_spin, self.stage2_stable_hotset_fraction_spin, self.stable_hotset_budget_gb_spin, self.emergency_free_vram_floor_spin,
    ...
    def _two_stage_asset_is_valid_path(self, key: str, path: Optional[Path]) -> Tuple[bool, str]:
        """Strictly validate a two-stage asset path.
    ...
    def _update_two_stage_asset_status(self, message: str = "") -> None:
        ...
        if message:
            ...
            lines.insert(0, "Two-stage assets ready.")
        else:
            lines.insert(0, "Two-stage assets not ready.")
        if self.pipeline_combo.currentText() not in TWO_STAGE_PIPELINES:
            lines.insert(0, "Two-stage is not selected; these files will only be used for two-stage / HQ / audio-conditioned runs.")
    ...
    def _start_two_stage_download(self, assets: List[Dict[str, Any]], *, start_after_download: bool) -> None:
        ...
        self.status_label.setText("Downloading two-stage files...")
        self.two_stage_status_label.setText(f"Downloading: {names}")
        self._append_log(f"Downloading missing two-stage assets: {names}")
    ...
    def _two_stage_download_finished(self, ok: bool, message: str, resolved: Dict[str, str]) -> None:
        ...
        self._update_two_stage_asset_status(message if not ok else "Two-stage assets ready.")
        self._save_settings(silent=True)
        start_after = self._pending_start_after_download
        self._pending_start_after_download = False
        if not ok:
            QMessageBox.warning(self, "LTX 2.3", f"Could not download two-stage files:\n{message}")
