...
class QueuePane(QWidget):
    ...
    def _populate(self, folder: Path, widget: QListWidget, status: str):
        ...
        def _safe_job_row(_p: Path, _status: str):
            try:
                return JobRowWidget(str(_p), _status)
            except Exception as e:
                # Fallback row to avoid "invisible jobs" when JobRowWidget cannot render a job.
                try:
                    from PySide6.QtWidgets import QWidget, QHBoxLayout, QLabel
                    fb = QWidget()
                    lay = QHBoxLayout(fb)
                    lay.setContentsMargins(8, 6, 8, 6)
                    lay.addWidget(QLabel(_p.name))
                    lay.addStretch(1)
                    lay.addWidget(QLabel(f"[row error] {type(e).__name__}"))
                    return fb
                except Exception:
                    return QWidget()
            ...
            for p in paths:
                ...
                try:
                    ...
                    if not isinstance(d, dict) or (not d.get('type')):
                        continue
                    # Some jobs (e.g., tools jobs, planner lock, lightweight helpers) may not have 'input' or 'frames'.
                    # Only skip "truly empty" jobs that have no actionable args and no output intent.
                    try:
                        _t = str(d.get('type') or '').strip()
                        _args = d.get('args') if isinstance(d.get('args'), dict) else {}
                        _has_payload = bool(d.get('input') or d.get('frames'))
                        _looks_like_tools = _t.startswith('tools_') or _t in ('tools_ffmpeg',)
                        _has_cmd = bool(_args.get('ffmpeg_cmd') or _args.get('cmd') or _args.get('command'))
                        _has_out = bool(_args.get('outfile') or d.get('out_dir') or _args.get('out_dir') or d.get('produced'))
                        if (not _has_payload) and (not _looks_like_tools) and (not _has_cmd) and (not _has_out) and (_t != 'txt2img'):
                            continue
                    except Exception:
                        continue
                except Exception:
