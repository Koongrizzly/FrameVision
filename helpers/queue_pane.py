...
class QueuePane(QWidget):
    ...
    def __init__(self, main, ctx=None, parent=None):
        ...
        self.watch_timer = QTimer(self); self.watch_timer.setInterval(1700); self.watch_timer.timeout.connect(self.request_refresh)
    ...
    def _populate(self, folder: Path, widget: QListWidget, status: str):
        ...
        if status == "pending":
            ...
            max_keep = getattr(self, "MAX_DONE_KEEP", 65)
            max_show = getattr(self, "MAX_DONE_SHOW", 65)
        elif status == "failed":
