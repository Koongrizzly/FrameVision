...
def _locate_settings_container(root: QWidget) -> QWidget | None:
    ...
    This is intentionally defensive. The main app may show emoji-only tab
    labels, restored/reordered tabs, or a legacy placeholder SettingsPane.
    If the Settings page exists but is not in the tab bar, re-add it. If it
    does not exist at all, create a small container so the real Settings UI can
    still be installed instead of silently disappearing.
    """
    if root is None:
        return None
    ...
    # 1) Prefer the real main tabs by visible label when labels are available.
        ...
        try:
            for i in range(tabs.count()):
                if "setting" in (tabs.tabText(i) or "").lower():
                    page = tabs.widget(i)
                    try:
                        if page is not None and not (page.objectName() or ""):
                            page.setObjectName("tab_settings")
                    except Exception:
                        pass
                    try:
                        if page is not None:
                            setattr(root, "settings", page)
                    except Exception:
                        pass
                    return page
        except Exception:
    ...
    # 2) Fallback to the main window attribute. This survives emoji-only labels
    # and older saved tab order states.
    try:
        page = getattr(root, "settings", None)
        if isinstance(page, QWidget):
            try:
                if not (page.objectName() or ""):
                    page.setObjectName("tab_settings")
            except Exception:
                pass
            if isinstance(tabs, QTabWidget):
    ...
    # 3) Fallback to stable object names.
    try:
        for w in root.findChildren(QWidget) or []:
            try:
                ...
                name = ""
            if name in ("settings", "tab_settings", "settings_tab", "framevision_settings"):
                if isinstance(tabs, QTabWidget):
                    try:
                        if tabs.indexOf(w) < 0:
    ...
    # 4) Last-resort repair: create the missing Settings tab container.
    # The caller will wipe/fill it with the real Settings UI immediately.
    if isinstance(tabs, QTabWidget):
        try:
            page = QWidget(root)
            page.setObjectName("tab_settings")
...
def _options_group(page: QWidget) -> QGroupBox:
    ...
    def _ensure_built():
        ...
        cb_clear_pyc.setToolTip(
            "If enabled, FrameVision will delete compiled .pyc cache folders,"
            "on startup to avoid stale bytecode. this will slow down start up of the app"
        )
        clear_default = s.value("clear_pyc_on_start", False, type=bool)
        cb_clear_pyc.setChecked(clear_default)

        # -- Auto update checkbox --------------------------------------------------------
        cb_auto_updates = QCheckBox("Auto check for updates")
        cb_auto_updates.setToolTip(
            "Checks GitHub for new FrameVision files after startup. "
            "Needs an internet connection."
        )
        try:
            from helpers import update_checker as _fv_update_checker
        except Exception:
            _fv_update_checker = None
        try:
            auto_updates_default = bool(_fv_update_checker.get_auto_check_enabled()) if _fv_update_checker else s.value("auto_check_updates", False, type=bool)
        except Exception:
            auto_updates_default = s.value("auto_check_updates", False, type=bool)
        cb_auto_updates.setChecked(bool(auto_updates_default))
        ...
        # -- Startup loading mode checkbox ----------------------------------------------
        cb_fast_startup = QCheckBox("Fast startup / load some tools when opened")
        cb_fast_startup.setToolTip(
            "If enabled, FrameVision can start faster by delaying supported heavy tools "
            "until you open them. First opening of tools like Text-to-Image, Describer, "
            "or HiDream may take a moment longer."
        )
        fast_startup_default = s.value("startup_lazy_load_tools", False, type=bool)
        cb_fast_startup.setChecked(fast_startup_default)

        lab_fast_startup_info = QLabel()
        lab_fast_startup_info.setWordWrap(True)
        try:
            lab_fast_startup_info.setContentsMargins(26, 0, 0, 0)
        except Exception:
            pass

        def _startup_mode_text(enabled: bool) -> str:
            if bool(enabled):
                return (
                    "Fast startup mode: FrameVision opens faster, but supported heavy tabs "
                    "may take a moment the first time you open them."
                )
            return (
                "Full startup mode: common tools load during startup. The full app may take "
                "longer to open, but tabs are ready sooner."
            )

        lab_fast_startup_info.setText(_startup_mode_text(bool(fast_startup_default)))

        # Mirror value into the portable FrameVision config so startup code can read it
        # later without depending on this Settings tab UI being opened.
        try:
            from helpers.framevision_app import config as _cfg, save_config as _save
            _cfg["startup_lazy_load_tools"] = bool(fast_startup_default)
            _save()
        except Exception:
            pass
        ...
        cb_diag.setToolTip("Only for debugging, you can leave this off unless fo bug reports.")
        ...
        # -- Live resource probe --------------------------------------------------------
        cb_resource_probe = QCheckBox("Resource usage report (restart to apply)")
        cb_resource_probe.setToolTip("Debug logging for CPU/RAM/VRAM and app resource suspects.")
        resource_probe_default = s.value("diagnostics_resource_probe", False, type=bool)
        cb_resource_probe.setChecked(bool(resource_probe_default))

        internal_stack_default = True

        # mirror value into config so helper/worker code can see the setting too
        try:
            from helpers.framevision_app import config as _cfg, save_config as _save
            _cfg["diagnostics_resource_probe"] = bool(resource_probe_default)
            _cfg["diagnostics_internal_stack_on_high_cpu"] = bool(internal_stack_default)
            _save()
        except Exception:
            pass
        ...
        cb_banner.setToolTip("Show or hide the fancy banner headers at the top of each tab, may need a restart of the app to activate when off during startup app")
        ...
        _sync_grey_state(cb_auto_updates, bool(auto_updates_default))
        _sync_grey_state(cb_tooltips, bool(tooltips_default))
        _sync_grey_state(cb_keep_settings, bool(keep_default))
        _sync_grey_state(cb_fast_startup, bool(fast_startup_default))
        _sync_grey_state(cb_diag, bool(diag_default))
        _sync_grey_state(cb_resource_probe, bool(resource_probe_default))
        ...
        def _on_auto_updates_toggle(b: bool):
            s.setValue("auto_check_updates", bool(b))
            try:
                if _fv_update_checker:
                    _fv_update_checker.set_auto_check_enabled(bool(b))
            except Exception:
                pass
            _sync_grey_state(cb_auto_updates, bool(b))
        ...
        def _on_fast_startup_toggle(b: bool):
            s.setValue("startup_lazy_load_tools", bool(b))
            try:
                lab_fast_startup_info.setText(_startup_mode_text(bool(b)))
            except Exception:
                pass
            try:
                from helpers.framevision_app import config as _cfg, save_config as _save
                _cfg["startup_lazy_load_tools"] = bool(b)
                _save()
            except Exception:
                pass
            _sync_grey_state(cb_fast_startup, bool(b))

        def _on_diag_toggle(b: bool):
            s.setValue("diag_probe_enabled", bool(b))
            s.setValue("diagnostics_enabled", bool(b))
            try:
                from helpers.framevision_app import config as _cfg, save_config as _save
                _cfg["diag_probe_enabled"] = bool(b)
                _cfg["diagnostics_enabled"] = bool(b)
                _save()
            except Exception:
                pass
            _sync_grey_state(cb_diag, bool(b))

        def _on_resource_probe_toggle(b: bool):
            s.setValue("diagnostics_resource_probe", bool(b))
            s.setValue("diagnostics_internal_stack_on_high_cpu", True)
            # Resource probe writes through diagnostics.log(), so make sure diagnostics are enabled too.
            if bool(b):
                s.setValue("diag_probe_enabled", True)
                s.setValue("diagnostics_enabled", True)
                try:
                    cb_diag.blockSignals(True)
                    cb_diag.setChecked(True)
                    cb_diag.blockSignals(False)
                    _sync_grey_state(cb_diag, True)
                except Exception:
                    pass
            try:
                from helpers.framevision_app import config as _cfg, save_config as _save
                _cfg["diagnostics_resource_probe"] = bool(b)
                _cfg["diagnostics_internal_stack_on_high_cpu"] = True
                if bool(b):
                    _cfg["diag_probe_enabled"] = True
                    _cfg["diagnostics_enabled"] = True
                _save()
            except Exception:
                pass
            _sync_grey_state(cb_resource_probe, bool(b))
        ...
        cb_auto_updates.toggled.connect(lambda b: _on_auto_updates_toggle(b))
        ...
        cb_fast_startup.toggled.connect(lambda b: _on_fast_startup_toggle(b))
        cb_diag.toggled.connect(lambda b: _on_diag_toggle(b))
        cb_resource_probe.toggled.connect(lambda b: _on_resource_probe_toggle(b))
        ...
        v.addWidget(cb_auto_updates)
        v.addWidget(cb_tooltips)
        v.addWidget(cb_keep_settings)
        v.addWidget(cb_fast_startup)
        v.addWidget(lab_fast_startup_info)
        v.addWidget(cb_diag)
        v.addWidget(cb_resource_probe)
        ...
        cb_animbtn.setToolTip("If enabled, some buttons such as 'Generate' and 'View results' will animate on hover.")
...
def _buttons_row(page: QWidget) -> QWidget:
    ...
    btn_cache = QPushButton("Clear Temp files, old logs or cache")
    btn_cache.setToolTip("Clean app-side caches, old logs and temp files. Model folders, environments, and /models/wan22 are skipped.")
    def do_cache():
        ...
        try:
            ...
            cb_pyc = QtWidgets.QCheckBox("Python cache (__pycache__ / .pyc / .pyo)", dlg)
            cb_logs = QtWidgets.QCheckBox("Old logs (older than 72 hours)", dlg)
            cb_thumbs = QtWidgets.QCheckBox("Thumbnail/cache folders (skips models and environments)", dlg)
            cb_qt = QtWidgets.QCheckBox("Qt cache (user profile)", dlg)
            cb_temp = QtWidgets.QCheckBox("Temp folders (output/_temp, work, FrameVision OS temp)", dlg)

            cb_pyc.setToolTip("Deletes app Python bytecode caches. Skips /models, /models/wan22, /environments, .venv/.env, and presets/extra_env.")
            cb_logs.setToolTip("Deletes old .log files from /logs and /helpers when they are older than 72 hours.")
            cb_thumbs.setToolTip("Deletes folders named thumbs, thumbnails, cache, tmp, .tmp, etc. inside the app scan, but prunes model and environment folders before walking them. Also clears old files in /temp after 48 hours.")
            cb_qt.setToolTip("Deletes Qt cache folders from the current Windows user profile, such as AppData/Local/QtProject and AppData/Local/framevision.")
            cb_temp.setToolTip("Clears FrameVision project temp folders: /output/_temp and /work, plus the FrameVision temp folder in the OS temp location.")
    ...
    btn_dump = QPushButton("Create App Report")
    def do_dump():
        try:
            dlg = QtWidgets.QDialog(page)
            dlg.setWindowTitle("Create App Report")
            dlg.setModal(True)
            ...
            tip = QtWidgets.QLabel("Select what to append to logs/framevision.log", dlg)
            tip.setWordWrap(True)
            v.addWidget(tip)

            cb_system = QtWidgets.QCheckBox("System snapshot", dlg)
            cb_process = QtWidgets.QCheckBox("Active/known running process", dlg)
            cb_resources = QtWidgets.QCheckBox("Resource usage report", dlg)
            cb_errors = QtWidgets.QCheckBox("Recent error summary", dlg)
            cb_modules = QtWidgets.QCheckBox("Deep Python module log", dlg)
            cb_tail = QtWidgets.QCheckBox("Full recent log tail", dlg)

            cb_system.setToolTip("Adds Python/Qt/package/ffmpeg info and saved settings snapshot.")
            cb_process.setToolTip("Adds launcher-chain, child processes, and known matching runners.")
            cb_resources.setToolTip("Adds CPU/RAM/VRAM, top offenders, internal thread stacks, and Qt event activity.")
            cb_errors.setToolTip("Scans recent logs for real errors and failed commands.")
            cb_modules.setToolTip("Adds a full sys.modules dump. Useful for import/load debugging, but noisy.")
            cb_tail.setToolTip("Adds the tail of recent .log files from the logs folder into framevision.log.")

            cb_system.setChecked(True)
            cb_process.setChecked(True)
            cb_resources.setChecked(True)
            cb_errors.setChecked(True)
            cb_modules.setChecked(False)
            cb_tail.setChecked(False)

            v.addWidget(cb_system)
            v.addWidget(cb_process)
            v.addWidget(cb_resources)
            v.addWidget(cb_errors)
            v.addWidget(cb_modules)
            v.addWidget(cb_tail)
            ...
            from helpers.diagnostics import append_app_report, LOG_FILE
            path = append_app_report(
                system_snapshot=bool(cb_system.isChecked()),
                running_processes=bool(cb_process.isChecked()),
                resource_snapshot=bool(cb_resources.isChecked()),
                internal_stack_sampler=bool(cb_resources.isChecked()),
                qt_event_sampler=bool(cb_resources.isChecked()),
                recent_error_summary=bool(cb_errors.isChecked()),
                deep_python_modules=bool(cb_modules.isChecked()),
                full_recent_log_tail=bool(cb_tail.isChecked()),
            )

            msg = QtWidgets.QMessageBox(page)
            msg.setWindowTitle("App report created")
            msg.setText("Selected report sections were appended to framevision.log.")
            msg.setInformativeText(str(path or LOG_FILE))
            open_btn = msg.addButton("Open log folder", QtWidgets.QMessageBox.ActionRole)
            msg.addButton("OK", QtWidgets.QMessageBox.AcceptRole)
            msg.exec()
            if msg.clickedButton() == open_btn:
                try:
                    log_dir = os.path.dirname(str(path or LOG_FILE))
                    QtGui.QDesktopServices.openUrl(QtCore.QUrl.fromLocalFile(log_dir))
                except Exception:
                    pass
        except Exception as e:
            try:
                QtWidgets.QMessageBox.warning(page, "App report failed", str(e))
            except Exception:
                pass
...
# Auto-install animated buttons manager (applies saved settings)
        ...
        # Optional startup update check: the helper itself respects the saved toggle.
        try:
            from helpers import update_checker as _uc
            _uc.install_startup_check(main_window, delay_ms=10000)
        except Exception:
            pass
