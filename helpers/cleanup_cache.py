...
import os, shutil, sys, time, tempfile, fnmatch, stat
...
def _safe_abs_path(path: Path) -> str:
    """Return a normalized absolute path string without forcing junction resolution.

    Using Path.resolve() on broken/recursive Windows junctions can raise WinError 1921.
    For cleanup scans we only need stable path comparison, not target resolution.
    """
    try:
...
def _is_under_any(path: Path, roots: list[Path]) -> bool:
    ...
    This intentionally avoids resolving junction targets. Some model folders contain
    recursive Windows reparse points, and resolving those can crash the cleanup scan.
    """
    p = _safe_abs_path(path)
    if not p:
        return False
    for root in roots or []:
        r = _safe_abs_path(root)
        if not r:
            continue
...
def _is_reparse_or_link_dir(path: Path) -> bool:
    """Best-effort detection for symlink/junction/reparse directories."""
    try:
        if path.is_symlink():
            return True
    except Exception:
...
def _iter_matches(base: Path, patterns: list[str], exclude_roots: list[Path] | None = None):
    """Yield paths under base matching patterns, while pruning unsafe folders first.

    This avoids pathlib.rglob() because rglob can walk into Windows junction loops
    before caller-side filters get a chance to reject the path.
    """
    exclude_roots = list(exclude_roots or [])
    try:
        base = Path(base)
    except Exception:
        return
    if _is_under_any(base, exclude_roots):
        return
    ...
    for root, dirs, files in os.walk(base, topdown=True, followlinks=False, onerror=lambda _e: None):
        root_path = Path(root)
        kept_dirs = []
        for d in list(dirs):
            p = root_path / d
        ...
        for f in files:
            if _matches(f):
                p = root_path / f
                if not _is_under_any(p, exclude_roots):
                    yield p
...
def run_cleanup(
    ...
    *,
    ...
    Safe defaults: remove app-side __pycache__/*.pyc/*.pyo and simple log/thumbnail caches under the project. Model folders, environments, presets/extra_env, and /models/wan22 are skipped.
    """
    base = Path(project_root or os.getcwd()).resolve()
    removed = {"pycache": 0, "pyc_pyo": 0, "logs": 0, "thumbs": 0, "qt": 0, "hf": 0, "temp": 0}

    # Global cleanup exclusions. These are pruned before walking so recursive
    # model junctions cannot crash the cleanup helper. /models/wan22 is kept
    # explicit because Wan Turbo installs can contain nested reparse paths.
    exclude_roots = [
        (base / ".venv"),
        (base / ".env"),
        (base / "environments"),
    ...
    if clean_pyc:
        # Do not touch bytecode inside virtual envs, model folders, or extra env presets.
        for p in _iter_matches(base, ["__pycache__"], exclude_roots):
            removed["pycache"] += _rm(p)

        for p in _iter_matches(base, ["*.pyc", "*.pyo"], exclude_roots):
