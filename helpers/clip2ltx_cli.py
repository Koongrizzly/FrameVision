...
def generate_ltx_start_image_for_shot(payload: dict) -> dict:
    ...
    try:
        ...
        hidream_reference_edit_resolution = "1376x768"
...
def _ltx23_build_vramlab_direct_args(
    *,
    ...
    width, height, _ = _parse_resolution_value(resolution, "1280x704")
    # LTX 2.3 two-stage audio/video currently requires 64-divisible dimensions.
    # Music Clip Creator may hand us 720p from the normal video settings; for
    # this own LTX-VRAMLab bridge, always run the proven 704p profile instead.
...
def run_single_ltx_shot_test(payload: dict) -> dict:
    ...
    try:
        ...
        if backend == "vramlab":
            max_vramlab_seconds = 10.0
            max_vramlab_frames = 241
            capped_by_seconds = max(1, int(round(max_vramlab_seconds * max(1, int(fps)))))
            capped_frames = max(1, min(max_vramlab_frames, capped_by_seconds))
            if int(frames) > int(capped_frames):
                _emit(f"LTX-VRAMLab cap active: limiting shot length to {max_vramlab_seconds:.1f}s / {capped_frames} frames for this workflow.")
                frames = int(capped_frames)
