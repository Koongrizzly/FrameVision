...
try:
    ...
    def _fv_init_v8(self, *a, **kw):  # type: ignore[no-redef]
        ...
        # Pin Actions row to the top so it stays visible while scrolling the rest
        try:
            ...
            if not getattr(self, "_fv_actions_pinned", False) and hasattr(self, "btn_describe"):
                ...
                if isinstance(root, QVBoxLayout) and act_box is not None:
                    ...
                    try:
                        if prog is not None:
                            prog.setSizePolicy(QSizePolicy.Preferred, QSizePolicy.Fixed)
                    except Exception:
                        pass
                    ...
                    # Move all existing items except the Actions row (and its progress bar) into the scroll area
                    while root.count():
                        item = root.takeAt(0)
                        w = item.widget()
                        if w is act_box or (prog is not None and w is prog):
                            continue
                        if w is not None:
                    ...
                    # Actions row first, then progress bar, then scrollable content
                    ...
                    if prog is not None:
                        root.setStretch(root.indexOf(prog), 0)
        except Exception:
            pass
