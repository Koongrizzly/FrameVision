...
try:
    from PySide6.QtMultimedia import QAudioProbe, QMediaPlayer, QAudioOutput
...
class PreAnalyzer(QThread):
    ...
    def __init__(self, path: Path, sr: int = 16000, bands: int = 48, hop_ms: int = 70, parent=None):
...
class VisualEngine(QObject):
    ...
    def __init__(self, parent: Optional[QObject] = None, bars: int = 48):
        ...
        # Preload cache for visualization plugins
        self._plugin_cache: dict[str, object] = {}
        # Warm-start gate: hide frames until we have real audio levels
        self._warm_gate_active = True
        self._warm_gate_started = 0.0
        self._got_levels = False
        self._warm_gate_threshold = 0.04  # RMS gate
        self._warm_gate_min_ms = 120      # hold time in ms
    ...
    def set_mode(self, mode: str):
        # Try to use preloaded plugin instance for faster visual switches.
        try:
            if mode in getattr(self, '_plugin_cache', {}):
                self._plugin = self._plugin_cache.pop(mode)
                self.mode = mode
                return
        except Exception:
            pass
        if isinstance(mode, str) and mode.startswith('viz:'):
            disp = mode.split(':', 1)[1]
            self._plugin = None
            for name, cls in _VISUAL_REGISTRY:
                if name == disp:
            ...
            self.mode = mode
            return
        # fallback: built-in spectrum (or anything else)
        self._plugin = None
        self.mode = mode

    def preload_mode(self, mode: str):
        """Instantiate and cache the visualization plugin for later fast switching.
        Does not change the currently displayed visual.
        """
        try:
            if (not mode) or (not isinstance(mode, str)):
                return
            if not mode.startswith('viz:'):
                return
            if mode in getattr(self, '_plugin_cache', {}):
                return
            disp = mode.split(':', 1)[1]
    ...
    def inject_levels(self, levels: List[float]):
        ...
        # Warm-start: record that we have received bands
        try:
            self._got_levels = True
        except Exception:
            pass
        try:
            # update warm gate if RMS already above threshold
            if getattr(self, '_rms', 0.0) >= float(getattr(self, '_warm_gate_threshold', 0.04)):
                if getattr(self, '_warm_gate_started', 0.0) <= 0.0:
                    self._warm_gate_started = _time.time()
                else:
                    if ( _time.time() - float(self._warm_gate_started) ) * 1000.0 >= float(getattr(self, '_warm_gate_min_ms', 120)):
                        self._warm_gate_active = False
        except Exception:
            pass
        ...
        for i, v in enumerate(vals):
            ...
            if i < bass_k:
                ...
                if i < max(16, self._bars // 4):        # low-mids
                    alpha_up, alpha_down = 0.74, 0.58
                elif i < max(24, self._bars // 2):      # mids
                    alpha_up, alpha_down = 0.70, 0.60
                else:                                    # highs
                    alpha_up, alpha_down = 0.76, 0.56
    ...
    def set_enabled(self, enabled: bool, warm_gate: bool | None = None):
        self.enabled = bool(enabled)
        if self.enabled:
            # warm_gate behavior:
            #   warm_gate is False -> NO warm gate (used for outgoing crossfade engine so we don't flash black)
            #   warm_gate is True  -> force warm gate on (hide until audio is alive)
            #   warm_gate is None  -> legacy behavior (same as True)
            if warm_gate is False:
                # explicitly disable warm gate for this engine
                self._warm_gate_active = False
                # mark levels as ready so we don't try to re-arm gate
                self._got_levels = True
                self._warm_gate_started = 0.0
            else:
                # enable warm gate and reset its timers
                self._warm_gate_active = True
                self._got_levels = False
                self._warm_gate_started = 0.0
        # if disabling visuals, leave analyzer/gate state alone
    ...
    def _tick(self):
        ...
        p = QPainter(img)
        try:
            if (not self.enabled) or getattr(self, '_warm_gate_active', False):
...
# Persist a fixed width for the visuals selector across show/hide toggles
        ...
        # audio crossfade controls (step 1)
        self.chk_axfade = QCheckBox('Crossfade audio')
        try:
            self.chk_axfade.setToolTip('Fade out current track while fading in the next track (requires Qt audio backend).')
        except Exception:
            pass
        self.cmb_axfade_sec = QComboBox()
        try:
            self.cmb_axfade_sec.addItems(['1s','2s','3s','4s','5s','6s','7s','8s','9s'])
            self.cmb_axfade_sec.setCurrentIndex(2)  # default 3s
        except Exception:
            pass
        self.chk_axfade_beats = QCheckBox('Beats')
        try:
            self.chk_axfade_beats.setToolTip('Compute fade duration from the current beat estimate (8–64 beats).')
        except Exception:
            pass
        self.cmb_axfade_beats = QComboBox()
        try:
            self.cmb_axfade_beats.addItems(['8','16','32','64'])
            self.cmb_axfade_beats.setCurrentIndex(1)  # default 16 beats
        except Exception:
            pass

        def _axfade_refresh():
            try:
                use_beats = bool(self.chk_axfade_beats.isChecked())
                self.cmb_axfade_sec.setEnabled(not use_beats)
                self.cmb_axfade_beats.setEnabled(use_beats)
            except Exception:
                pass

        try:
            self.chk_axfade_beats.toggled.connect(lambda _c: _axfade_refresh())
            QTimer.singleShot(0, _axfade_refresh)
        except Exception:
            pass

        row_vis4 = QHBoxLayout()
        row_vis4.addWidget(self.chk_axfade)
        row_vis4.addSpacing(8)
        row_vis4.addWidget(QLabel('Fade'))
        row_vis4.addWidget(self.cmb_axfade_sec)
        row_vis4.addSpacing(8)
        row_vis4.addWidget(self.chk_axfade_beats)
        row_vis4.addWidget(self.cmb_axfade_beats)
        row_vis4.addWidget(QLabel('beats'))
        row_vis4.addStretch(1)
        L.addLayout(row_vis4)
...
# Any user activity resets timer and shows UI
    ...
    def peek_next_index(self, delta: int = +1, mode: Optional[str] = None) -> int:
        """Return the index that *would* be played next, without changing state or opening the file."""
        try:
            tracks = getattr(self, '_tracks', []) or []
            n = len(tracks)
            if n <= 0:
                return -1
            m = (mode or self.repeat_mode() or 'all').lower()
            cur = int(getattr(self, '_current_idx', -1))
            if cur < 0:
                cur = 0
            ...
            if m == 'random':
                ...
                return int(random.choice(choices)) if choices else cur
            if m == 'shuffle':
                # Ensure shuffle order exists, but do not advance the internal pointer (peek only)
                try:
                    if not getattr(self, '_shuffle_order', None) or any(i >= n for i in self._shuffle_order):
                        self._build_shuffle_order(cur)
                except Exception:
                    pass
                order = list(getattr(self, '_shuffle_order', []) or [])
                if not order:
                    return (cur + (1 if delta >= 0 else -1)) % n
                try:
                    pos = int(getattr(self, '_shuffle_pos', -1))
                    if pos < 0:
    ...
    def crossfade_ms(self) -> int:
        try:
            ...
            return 2000
    def audio_crossfade_enabled(self) -> bool:
        try:
            return bool(self.chk_axfade.isChecked())
        except Exception:
            return False

    def audio_crossfade_use_beats(self) -> bool:
        try:
            return bool(self.chk_axfade_beats.isChecked())
        except Exception:
            return False

    def audio_crossfade_seconds(self) -> int:
        try:
            txt = str(self.cmb_axfade_sec.currentText()).lower().replace('s', '').strip()
            v = int(float(txt))
            return max(1, min(9, v))
        except Exception:
            return 3

    def audio_crossfade_beats(self) -> int:
        try:
            v = int(float(str(self.cmb_axfade_beats.currentText()).strip()))
            # allow only the offered options
            if v in (8, 16, 32, 64):
                return v
            return 16
        except Exception:
...
class MusicRuntime(QObject):
    def __init__(self, video_pane):
        self._preloaded_idx = None
        self._preload_next_at = 0.0
        ...
        self._xfade_t0 = 0.0
        self._xfade_ms = 2000
        # audio crossfade state (step 1)
        self._axfade_active = False
        self._axfade_t0 = 0.0
        self._axfade_ms = 3000
        self._axfade_next_idx = -1
        self._axfade_next_path = None  # type: Optional[Path]
        self._axfade_base_vol = 1.0
        # two-deck model for future automix: A is the active player, B is the preloaded next player
        self._deck_a = getattr(video_pane, 'player', None)
        self._deck_b = None
        self._deck_a_out = None
        self._deck_b_out = None
    ...
    def _peek_next_visual_index(self) -> int:
        try:
            if not self.overlay:
                return -1
            # Determine mode of progression
            mode = self.overlay.current_viz_mode()
            count = len(getattr(self.overlay, '_visual_modes', [])) if hasattr(self.overlay, '_visual_modes') else 0
            if count <= 1:
                try:
                    return self.overlay.cmb_visual.currentIndex()
                except Exception:
            ...
            if mode == 'all':
                return (cur + 1) % count
            # random: peek next from bag without advancing
            # ensure bag valid
            if not isinstance(getattr(self, '_viz_bag', None), list) or not self._viz_bag or any((i < 0 or i >= count) for i in self._viz_bag):
                self._viz_rebuild_bag()
            pos = int(getattr(self, '_viz_bag_pos', -1)) + 1
            if self._viz_bag and 0 <= pos < len(self._viz_bag):
                return self._viz_bag[pos]
            # fallback
            return cur
        except Exception:
            return -1

    def _preload_next_visual_if_needed(self):
        try:
            if not self.overlay or not self.overlay.visuals_enabled():
                return
            nxt = self._peek_next_visual_index()
            if nxt < 0:
                return
            # Remember what we've already preloaded
            if getattr(self, '_preloaded_idx', None) == nxt:
                return
            modes = getattr(self.overlay, '_visual_modes', [])
            if not modes or nxt >= len(modes):
                return
            next_mode = modes[nxt]
            # Ask the active engine to preload the plugin for this mode
            try:
                self.visual.preload_mode(next_mode)
                self._preloaded_idx = nxt
            except Exception:
                pass
        except Exception:
            pass
    ...
    def _update(self):
        ...
        # Preload next visual plugin in the background (throttled)
        try:
            import time as _time
            now = _time.time()
            if now >= float(getattr(self, '_preload_next_at', 0.0)):
                self._preload_next_visual_if_needed()
                self._preload_next_at = now + 0.5
        except Exception:
            pass

        # audio crossfade tick (step 1)
        try:
            self._tick_audio_crossfade()
        except Exception:
            pass

        # Keep deck A pointing at the pane's current player (unless we're mid-fade)
        try:
            if not bool(getattr(self, '_axfade_active', False)):
                self._deck_a = getattr(self.video, 'player', getattr(self, '_deck_a', None))
        except Exception:
            pass
        ...
        if bpm > 0:
            ...
            ms_per_beat = max(250, min(1200, int(self._ms_per_beat_est or 500)))
        try:
            pos = int(self.video.player.position())
        except Exception:
            pos = 0
        ...
        try:
            ...
            dur = 0
        # audio auto-crossfade into next track (step 1)
        try:
            if (not getattr(self, '_axfade_active', False)) and self.overlay and self.overlay.audio_crossfade_enabled() and dur > 0:
                if self._qt_audio_crossfade_supported():
                    # compute target fade length
                    fade_ms = int(max(1000, min(9000, int(self.overlay.audio_crossfade_seconds()) * 1000)))
                    if bool(self.overlay.audio_crossfade_use_beats()):
                        beats = int(self.overlay.audio_crossfade_beats())
                        fade_ms = int(max(500, min(45000, beats * int(ms_per_beat))))
                    # need at least 2 tracks for a meaningful next-track mix
                    try:
                        n_tracks = len(getattr(self.overlay, '_tracks', []) or [])
                    except Exception:
                        n_tracks = 0
                    if n_tracks >= 2:
                        start_at = max(0, int(dur) - int(fade_ms))
                        if int(pos) >= start_at and not bool(getattr(self, '_end_fired', False)):
                            mode = self.overlay.repeat_mode() if self.overlay else 'all'
                            if mode != 'one':
                                next_idx = int(self.overlay.peek_next_index(+1, mode))
                                if next_idx >= 0:
                                    try:
                                        next_path = self.overlay._tracks[next_idx].path
                                    except Exception:
                                        next_path = None
                                    if next_path:
                                        self._start_audio_crossfade(next_idx, Path(next_path), int(fade_ms))
        except Exception:
            pass
    ...
    # ---------------- Audio crossfade (step 1) ----------------
    def _qt_audio_crossfade_supported(self) -> bool:
        try:
            return bool(QMediaPlayer and QAudioOutput and isinstance(getattr(self, '_deck_a', None), QMediaPlayer))
        except Exception:
            return False

    def _qt_player_output(self, player):
        """Return (and if needed create) a QAudioOutput for a QMediaPlayer."""
        try:
            if not player or not QAudioOutput:
                return None
            out = None
            try:
                ...
                out = None
            if out is None and hasattr(player, 'setAudioOutput'):
                try:
                    out = QAudioOutput(self)  # type: ignore[call-arg]
    ...
    def _qt_load_player(self, player, path: Path) -> bool:
        try:
            if not player or not hasattr(player, 'setSource'):
                return False
            try:
    ...
    def _qt_stop_player(self, player):
        try:
            if not player:
                return
    ...
    def _qt_play_player(self, player):
        try:
            if not player:
                return
    ...
    def _qt_set_out_vol(self, out, v: float):
        try:
            if out and hasattr(out, 'setVolume'):
                out.setVolume(float(max(0.0, min(1.0, v))))
        except Exception:
            pass

    def _qt_get_out_vol(self, out) -> float:
        try:
            if out and hasattr(out, 'volume'):
                return float(out.volume())
        except Exception:
            pass
        return 1.0

    def _apply_meta_from_path(self, p: Path):
        """Update runtime meta/cover without calling video.open()."""
        try:
            tags = _read_all_tags(p)
            title = tags.get('title') or tags.get('TITLE') or p.stem
            artist = tags.get('artist') or tags.get('ARTIST') or ''
            ...
            self.set_cover(cover_pm)
            self.set_meta(meta)
        except Exception:
            pass

    def _overlay_set_current_idx(self, idx: int):
        try:
            if not self.overlay:
                return
            self.overlay._current_idx = int(idx)
            try:
                self.overlay.playlist.setCurrentRow(int(idx))
            except Exception:
                pass
            try:
                self.overlay._persist_playlist_state()
            except Exception:
                pass
        except Exception:
            pass

    def _ensure_deck_b(self):
        """Create the secondary deck if needed."""
        if getattr(self, '_deck_b', None) is not None:
            return
        try:
            if not self._qt_audio_crossfade_supported():
                return
            self._deck_b = QMediaPlayer(self)  # type: ignore[call-arg]
            self._deck_b_out = self._qt_player_output(self._deck_b)
            # start silent
            self._qt_set_out_vol(self._deck_b_out, 0.0)
        except Exception:
            self._deck_b = None
            self._deck_b_out = None

    def _start_audio_crossfade(self, next_idx: int, next_path: Path, fade_ms: int):
        try:
            if not self._qt_audio_crossfade_supported():
                return False
            self._ensure_deck_b()
            if not getattr(self, '_deck_b', None):
                return False
            # Ensure outputs for both decks
            self._deck_a_out = self._qt_player_output(self._deck_a)
            self._deck_b_out = self._qt_player_output(self._deck_b)
            if not self._deck_a_out or not self._deck_b_out:
                return False
            # Base volume follows the user's current volume
            self._axfade_base_vol = float(max(0.0, min(1.0, self._qt_get_out_vol(self._deck_a_out))))
            # Load next track on deck B, start silent
            if not self._qt_load_player(self._deck_b, next_path):
                return False
            self._qt_set_out_vol(self._deck_b_out, 0.0)
            self._qt_play_player(self._deck_b)

            import time as _time
            self._axfade_active = True
            self._axfade_t0 = float(_time.time())
            self._axfade_ms = int(max(300, fade_ms))
            self._axfade_next_idx = int(next_idx)
            self._axfade_next_path = Path(next_path)
            # keep end-advance from firing
            self._end_fired = True
            return True
        except Exception:
            return False

    def _tick_audio_crossfade(self):
        if not getattr(self, '_axfade_active', False):
            return
        try:
            import time as _time
            now = float(_time.time())
            dt_ms = max(0.0, (now - float(self._axfade_t0)) * 1000.0)
            dur_ms = float(max(1, int(getattr(self, '_axfade_ms', 3000))))
            t = min(1.0, dt_ms / dur_ms)
            base = float(max(0.0, min(1.0, float(getattr(self, '_axfade_base_vol', 1.0)))))
            # linear crossfade for step 1 (equal-power can be added later)
            self._qt_set_out_vol(self._deck_a_out, (1.0 - t) * base)
            self._qt_set_out_vol(self._deck_b_out, t * base)
            if t >= 1.0:
                # finalize swap
                self._qt_set_out_vol(self._deck_b_out, base)
                self._qt_set_out_vol(self._deck_a_out, 0.0)
                self._qt_stop_player(self._deck_a)
                # swap decks
                old_a = self._deck_a
                old_a_out = self._deck_a_out
                self._deck_a = self._deck_b
                self._deck_a_out = self._deck_b_out
                self._deck_b = old_a
                self._deck_b_out = old_a_out

                # Make the new deck A the pane's active player
                try:
                    if hasattr(self.video, 'player'):
                        self.video.player = self._deck_a
                except Exception:
                    pass
                # Rebind analyzer to the new player so visuals keep reacting
                try:
                    if getattr(self, 'an', None) and hasattr(self.an, 'rebind_player'):
                        self.an.rebind_player(self._deck_a)
                except Exception:
                    pass

                # Update meta/cover + playlist selection
                try:
                    if getattr(self, '_axfade_next_path', None):
                        self._apply_meta_from_path(Path(self._axfade_next_path))
                except Exception:
                    pass
                try:
                    if getattr(self, '_axfade_next_idx', -1) >= 0:
                        self._overlay_set_current_idx(int(self._axfade_next_idx))
                except Exception:
                    pass
                ...
                self._axfade_active = False
                self._axfade_next_idx = -1
                self._axfade_next_path = None
                self._end_fired = False
        except Exception:
            # fail-safe: stop fading and restore base volume
            try:
                base = float(max(0.0, min(1.0, float(getattr(self, '_axfade_base_vol', 1.0)))))
                self._qt_set_out_vol(self._deck_a_out, base)
                self._qt_set_out_vol(self._deck_b_out, 0.0)
            except Exception:
                pass
            self._axfade_active = False
    ...
    def _start_crossfade(self):
        try:
            ...
            try:
                self.visual_prev.set_enabled(True, warm_gate=False)
    ...
    def set_overlay(self, overlay: MusicOverlay):
        ...
        # load audio crossfade settings (step 1)
        try:
            st = _load_state()
            ax_on = bool(st.get('axfade_on', False))
            ax_mode = str(st.get('axfade_mode', 'seconds'))
            ax_sec = int(st.get('axfade_sec', 3))
            ax_beats = int(st.get('axfade_beats', 16))
            try:
                self.overlay.chk_axfade.setChecked(ax_on)
            except Exception:
                pass
            # seconds combo
            try:
                ax_sec = max(1, min(9, int(ax_sec)))
                sec_items = ['1s','2s','3s','4s','5s','6s','7s','8s','9s']
                self.overlay.cmb_axfade_sec.setCurrentIndex(sec_items.index(f'{ax_sec}s'))
            except Exception:
                pass
            # beats mode + beats combo
            try:
                use_beats = (ax_mode.lower().strip() == 'beats')
                self.overlay.chk_axfade_beats.setChecked(bool(use_beats))
            except Exception:
                pass
            try:
                if ax_beats not in (8, 16, 32, 64):
                    ax_beats = 16
                beat_items = ['8','16','32','64']
                self.overlay.cmb_axfade_beats.setCurrentIndex(beat_items.index(str(ax_beats)))
            except Exception:
                pass
        except Exception:
            pass
        ...
        try:
            ...
            # audio crossfade persistence
            self.overlay.chk_axfade.toggled.connect(lambda _c: self._persist_state())
            self.overlay.cmb_axfade_sec.currentIndexChanged.connect(lambda _i: self._persist_state())
            self.overlay.chk_axfade_beats.toggled.connect(lambda _c: self._persist_state())
            self.overlay.cmb_axfade_beats.currentIndexChanged.connect(lambda _i: self._persist_state())
    ...
    def _persist_state(self):
        ...
        try:
            ...
            pass
        # audio crossfade settings (step 1)
        try:
            st['axfade_on'] = bool(self.overlay.chk_axfade.isChecked())
            st['axfade_sec'] = int(self.overlay.audio_crossfade_seconds())
            st['axfade_beats'] = int(self.overlay.audio_crossfade_beats())
            st['axfade_mode'] = 'beats' if bool(self.overlay.audio_crossfade_use_beats()) else 'seconds'
        except Exception:
            pass
...
def wire_to_videopane(VideoPaneClass):
    ...
    # FrameVision integration note:
    # Some builds renamed VideoPane.open -> open_file/open_media/etc.
    # Don't crash at import time; discover and wrap what exists, and provide a .open alias if missing.
    _fv_open_name = None
    _fv_had_open = False
    for _cand in ("open", "open_media", "open_path", "open_file", "load_file", "load", "set_media", "setSource"):
        try:
            if hasattr(VideoPaneClass, _cand) and callable(getattr(VideoPaneClass, _cand)):
                _fv_open_name = _cand
                break
        except Exception:
            pass
    try:
        _fv_had_open = bool(hasattr(VideoPaneClass, "open") and callable(getattr(VideoPaneClass, "open")))
    except Exception:
        _fv_had_open = False
    if not _fv_open_name:
        # Can't wire; keep app booting.
        return
    orig_open = getattr(VideoPaneClass, _fv_open_name)
    ...
    try:
        setattr(VideoPaneClass, _fv_open_name, open_wrapper)
    except Exception:
        pass
    # If the class didn't have .open originally, provide it so other parts of the app can still call video.open(...)
    try:
        if not _fv_had_open:
