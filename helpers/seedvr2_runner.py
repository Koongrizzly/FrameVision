...
def _legacy_mode(args: argparse.Namespace) -> int:
    ...
    for k in passthru_flags:
        ...
        if v is True:
            ...
            if k == "attention_mode":
                v = _resolve_attention_mode(v)
            cmd += [flag, str(v)]

    try:
        if seed_cfg.get("attention_mode") is not None:
            _write_log(root, f"Attention mode requested={seed_cfg.get('attention_mode')} resolved={_resolve_attention_mode(seed_cfg.get('attention_mode'))}")
    except Exception:
        pass
...
def _resolve_attention_mode(raw: Any) -> str:
    """Normalize FrameVision attention choices to SeedVR2 inference_cli.py choices.

    SeedVR2 CLI expects:
      sdpa | flash_attn_2 | flash_attn_3 | sageattn_2 | sageattn_3

    FrameVision UI / saved settings may pass:
      auto | sage | sageattention | flash | flash_attn | none | sdpa
    """
...
def _normalize_forward_args(forward: List[str]) -> List[str]:
    """Normalize FrameVision shorthand attention values to inference_cli.py choices."""
    if not forward:
        return forward
    out = list(forward)
    i = 0
    while i < len(out):
        ...
        if tok == "--attention_mode" and i + 1 < len(out):
            ...
            norm = _resolve_attention_mode(raw)
            if norm != raw:
                out[i + 1] = norm
...
def _new_mode(args: argparse.Namespace) -> int:
    ...
    _write_log(root, f"Attention availability: sageattention={_module_available('sageattention')} flash_attn={_module_available('flash_attn')}")
