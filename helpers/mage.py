#!/usr/bin/env python3
"""
FrameVision Mage-Flow text-to-image helper UI and persistent backend.

Expected location
-----------------
    <FrameVision root>/helpers/mage.py

Installed runtime
-----------------
    <FrameVision root>/environments/.mage_edit/
    <FrameVision root>/models/mage_edit/Mage-Flow-Turbo/

Persistent settings
-------------------
    <FrameVision root>/presets/setsave/mage.json
...
- Tabs and the bottom footer never scroll.
...
DEFAULT_NAME_TEMPLATE = "mage_{timestamp}_seed{seed}"
...
def settings_path(root: Path) -> Path:
    return root / "presets" / "setsave" / "mage.json"
...
def slug(text: str, fallback: str = "mage", max_length: int = 64) -> str:
...
def prompt_slug(text: str) -> str:
    ...
    return slug("_".join(words[:5]), "mage", 64).lower()
...
class MagePipelineServer:
    """Persistent Mage text-to-image pipeline."""
    ...
    def _render_name(template: str, prompt: str, seed: int, index: int) -> str:
        ...
        return slug(rendered, f"mage_{tokens['timestamp']}_seed{seed}", 180)
    ...
    def generate(self, request: dict[str, Any]) -> None:
        ...
        kwargs: dict[str, Any] = {
            ...
            "cfg": cfg,
            "prompt_template": str(
                request.get("prompt_template", "mage-flow")
            ).strip() or "mage-flow",
        ...
        if size_mode == "explicit":
            ...
            width -= width % 16
            height -= height % 16
        elif size_mode in {"square", "max_side"}:
            # Text-to-image has no source image from which to derive an aspect
            # ratio. Mage's generate_images() accepts only explicit per-sample
            # heights and widths, so both size-only modes produce a square.
            width = max(16, int(request.get("max_size", 1024)))
            width -= width % 16
            height = width
        else:
        ...
        worker_event(
            "status",
            message=(
                f"Generating {count} image{'s' if count != 1 else ''} "
        ...
        results = pipeline.generate(prompts, **kwargs)
        generation_seconds = time.time() - started

        output = dict(request.get("output", {}))
        output_folder = Path(
            str(output.get("folder", self.root / "output" / "images" / "mage"))
        ).expanduser().resolve()
        ...
        if bool(request.get("write_generation_report", True)):
            log_folder = Path(
                str(request.get("log_folder", self.root / "logs" / "mage"))
            ).expanduser().resolve()
            log_folder.mkdir(parents=True, exist_ok=True)
            report_path = unique_path(
                log_folder / f"mage_{datetime.now():%Y%m%d_%H%M%S}.json"
            )
            atomic_write_json(
                report_path,
                {
...
try:
    ...
    raise RuntimeError("helpers/mage.py requires PySide6 in the main FrameVision environment") from exc
...
class MageUI(QWidget):
    ...
    def default_settings(self) -> dict[str, Any]:
        return {
            ...
            "generation": {
                "prompt": "A stylish cinematic portrait of a person in a neon-lit city at night.",
                "negative_prompt": "",
                "steps": 4,
                "cfg": 1.0,
                "seed": -1,
            ...
            "paths": {
                ...
                "output_folder": "output/images/mage",
                "log_folder": "logs/mage",
            },
            "runtime": {
                "residency": "gpu",
    ...
    def build_generation_tab(self) -> None:
        ...
        self.prompt_edit.setPlaceholderText("Describe the image you want Mage to generate.")
        self.prompt_edit.setToolTip(
            "Main text prompt. Mage-Flow-Turbo works best with clear direct instructions "
            "and benefits less from long prose than slower diffusion models."
        )
        prompt_layout.addWidget(self.prompt_edit)

        self.negative_edit = QTextEdit()
        self.negative_edit.setMaximumHeight(72)
        self.negative_edit.setPlaceholderText("Optional negative prompt")
        self.negative_edit.setToolTip(
            "Negative prompt. Most helpful when CFG is above 1.0. For the official "
            "Turbo default CFG 1.0 it usually has only a small effect."
        )
        prompt_layout.addWidget(self.negative_edit)
        self.generation_tab.layout.addWidget(prompt_group)
        ...
        self.steps_spin.setToolTip(
            "Turbo is designed for 4 steps. Use 4 as the main default. "
            "Higher values can change the look but also increase generation time."
        )
        self.cfg_spin = self.dspin(0.0, 20.0, 0.1, 2)
        self.cfg_spin.setToolTip(
            "Classifier-free guidance. Mage-Flow-Turbo is designed for CFG 1.0. "
            "Raise it only if you want the prompt followed more aggressively."
        )
        self.seed_spin = self.spin(-1, 2_147_483_647, 1)
        self.seed_spin.setToolTip(
            "-1 chooses random seeds. When generating multiple outputs, Mage uses "
            "consecutive seeds starting from the chosen base."
        )
        self.count_spin = self.spin(1, 999, 1)
        self.count_spin.setToolTip(
            "Number of images to generate in one packed request. Higher counts use "
            "more VRAM because Mage processes them together."
        )
        ...
        self.width_spin.setToolTip(
            "Explicit output width. Mage accepts arbitrary aspect ratios, but wider "
            "or taller images can be harder to compose cleanly."
        )
        self.height_spin = self.spin(512, 2048, 16)
        self.height_spin.setToolTip(
            "Explicit output height. Keep both dimensions multiples of 16."
        )

        grid.addWidget(QLabel("Output size"), 2, 0)
        grid.addWidget(self.size_mode_combo, 2, 1, 1, 3)
        self.add_grid_pair(grid, 3, "Main size", self.max_size_spin, "", None)
        ...
        self.static_shift_spin.setToolTip(
            ...
            "Leave this at 6.0 unless you deliberately want to experiment."
        )
        advanced.addRow("Static shift", self.static_shift_spin)

        self.prompt_template_edit = QLineEdit("mage-flow")
        self.prompt_template_edit.setToolTip(
            "Official text-to-image prompt template. Normally leave this at mage-flow."
        )
        ...
        self.batch_cfg_check.setToolTip(
            "Official batch_cfg option. It mainly matters when CFG is above 1.0."
        )
        advanced.addRow("", self.batch_cfg_check)

        self.renormalize_check = QCheckBox("CFG velocity renormalization")
        self.renormalize_check.setToolTip(
            "Can help tame over-driven CFG, but is generally unnecessary at the "
            "recommended Turbo default CFG 1.0."
        )
        ...
        self.output_list = ThumbnailList()
    ...
    def build_settings_tab(self) -> None:
        ...
        self.model_combo.setToolTip(
            "Installed non-edit Mage checkpoints found under models/mage_edit. "
            "This helper is intended for Mage-Flow-Turbo."
        )
        ...
        self.environment_edit.setToolTip(
            "Python executable inside environments/.mage_edit. This is the dedicated "
            "Mage environment created by the installer."
        )
        ...
        self.log_folder_edit.setToolTip(
            "Folder for generation reports and optional session logs."
        )
        ...
        self.residency_combo.setToolTip(
            "Real between-job residency choices. Keeping the model on the GPU is "
            "fastest. Moving it to RAM reduces idle VRAM use. Unload is slowest but "
        ...
        self.format_combo.setToolTip(
            "PNG preserves maximum quality. JPEG gives smaller files. WebP is compact "
            "and high quality but some external tools support it less consistently."
        )
        output_form.addRow("Format", self.format_combo)

        self.jpeg_quality_spin = self.spin(1, 100, 1)
        self.jpeg_quality_spin.setToolTip("JPEG quality if the output format is JPEG.")
        output_form.addRow("JPEG quality", self.jpeg_quality_spin)
        self.webp_quality_spin = self.spin(1, 100, 1)
        self.webp_quality_spin.setToolTip("WebP quality if the output format is WebP.")
    ...
    def cancel_generation(self) -> None:
        ...
        self.append_log(
            ...
            "to reload for the next generation.",
            "warning",
        )
        self.pending_request = None
        self.batch_active = False
            ...
            try:
                ...
                if self._session_log_path is None:
                    self._session_log_path = (
                        folder / f"mage_ui_{datetime.now():%Y%m%d_%H%M%S}.log"
                    )
    ...
    def update_size_controls(self, *_args: Any) -> None:
        ...
        self.max_size_spin.setEnabled(mode in {"max_side", "square"})
...
def create_widget(
    ...
    return MageUI(parent=parent, root=root)
