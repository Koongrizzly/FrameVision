            ...
            def step_music_ace15_7a() -> None:
                ...
                if dur_s <= 0.1:
                    try:
                        ...
                        dur_s = 0.0
                # When Videoclip Creator is selected, the user already chose the final duration at job start.
                try:
                    _vp = str((self.job.encoding or {}).get("videoclip_preset") or "Storyline Preset (Hardcuts)")
                    _use_vp = bool(_vp) and (not str(_vp).lower().startswith("storyline preset"))
                except Exception:
                    _use_vp = False
                if _use_vp:
                    try:
...
# Build ffmpeg command
            ...
            # Step 2: Videoclip Creator assembly is enabled when a JSON preset is selected.
            try:
                _vp = str((self.job.encoding or {}).get('videoclip_preset') or 'Storyline Preset (Hardcuts)')
            except Exception:
                _vp = 'Storyline Preset (Hardcuts)'
            _use_videoclip_creator = bool(_vp) and (not str(_vp).lower().startswith('storyline preset'))
            ...
            def step_videoclip_creator_10a() -> None:
                ...
                # Load preset settings from JSON preset manager.
                root = str(_root())
                preset_path = os.path.join(root, 'presets', 'setsave', 'clip_presets.json')
                if not os.path.isfile(preset_path):
                    # Compatibility fallback for older internal builds
                    preset_path = os.path.join(root, 'presets', 'setsave', 'plannerclip.json')
                if not os.path.isfile(preset_path):
                    raise RuntimeError(f"Videoclip preset file not found: {preset_path}")
                ...
                if not isinstance(presets_list, list) or (not presets_list):
                    raise RuntimeError('Videoclip preset file has no presets.')

                # Choose preset by id (preferred) or by name (combo label)
                chosen = None
                try:
                    want_id = str((self.job.encoding or {}).get('videoclip_preset_id') or '').strip()
                except Exception:
                    want_id = ''
                want_name = str(_vp or '').strip()
                if want_id:
                    for p in presets_list:
                        if isinstance(p, dict) and str(p.get('id') or '').strip() == want_id:
                            chosen = p
                            break
                if chosen is None and want_name:
                    for p in presets_list:
                        if isinstance(p, dict) and str(p.get('name') or '').strip() == want_name:
                            chosen = p
                            break
                if chosen is None:
                ...
                settings.setdefault('combo_fit', 0)                # Planner default when using Videoclip Creator: sequential clip order.
                ...
                # Prefer the tool's own discovery helper when available (most robust duration probing).
                if hasattr(_ams, "discover_video_sources"):
                    try:
                        sources = _ams.discover_video_sources(str(clips_dir), ffprobe2)
                    except Exception:
                        sources = []
                elif hasattr(_ams, "scan_sources"):
...
#        fmt.addWidget(self.lbl_bitrate_info, 1, 0, 1, 2)
        ...
        # Step 2: Load presets from clip_presets.json (plus Preset 1 = Hardcuts)
        try:
            self._load_videoclip_preset_combo()
        except Exception:
            try:
...
# -------------------------
    ...
    def _load_videoclip_preset_combo(self) -> None:
        ...
        Expected file: <root>/presets/setsave/clip_presets.json
        Shape: {"version": 1, "presets": [{"id","name","description","settings"}, ...]}
        """
        self._videoclip_preset_items = []  # list of dicts
        try:
            root = str(_root())
            p = os.path.join(root, 'presets', 'setsave', 'clip_presets.json')
            if not os.path.isfile(p):
                # Compatibility fallback (older internal builds)
                p2 = os.path.join(root, 'presets', 'setsave', 'plannerclip.json')
                if os.path.isfile(p2):
                    p = p2
            obj = _safe_read_json(p) if os.path.isfile(p) else {}
            if not isinstance(obj, dict):
                obj = {}
            items = obj.get('presets') or []
            ...
            for it in items:
                ...
                if not name:
                    continue
                cleaned.append({'id': pid, 'name': name, 'description': desc, 'settings': settings})
            self._videoclip_preset_items = cleaned
        except Exception:
            self._videoclip_preset_items = []
        ...
        try:
            self.cmb_videoclip_preset.clear()
            self.cmb_videoclip_preset.addItem("Storyline Preset (Hardcuts)")
            for it in (self._videoclip_preset_items or []):
                self.cmb_videoclip_preset.addItem(str(it.get('name') or ''))

            # Enable only if there are presets to choose from
            self.cmb_videoclip_preset.setEnabled(bool(self._videoclip_preset_items))
            self.cmb_videoclip_preset.setToolTip("Select Videoclip Creator preset (loaded from clip_presets.json)")
        except Exception:
            pass
        ...
        try:
            ...
            if want:
                idx = self.cmb_videoclip_preset.findText(want)
                if idx >= 0:
                    self.cmb_videoclip_preset.setCurrentIndex(idx)
                else:
                    self.cmb_videoclip_preset.setCurrentIndex(0)
            else:
                self.cmb_videoclip_preset.setCurrentIndex(0)
...
# Chunk 6B: narration settings (validated here to block run cleanly)
        ...
        try:
            vp_name = str(enc.get('videoclip_preset') or '').strip()
            vp_id = ''
            for it in (getattr(self, '_videoclip_preset_items', None) or []):
                if str(it.get('name') or '').strip() == vp_name:
                    vp_id = str(it.get('id') or '').strip()
                    break
            if vp_id:
                enc['videoclip_preset_id'] = vp_id
        except Exception:
            pass
