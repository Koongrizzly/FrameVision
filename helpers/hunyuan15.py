...
VRAM_LAB_PRESETS = [
    ("Off", "off"),
    ("On", "safe"),  # Internal safe/default mode while Hunyuan VRAM Lab is being stabilized.
]
...
class Hunyuan15ToolWidget(QWidget):
    ...
    def __init__(self, parent=None, standalone: bool = False):
        ...
        # Settings save lock must exist before any widget setup.
        # Some setup helpers emit signals / call _schedule_persist while the UI is
        # still full of defaults. If this flag is missing, the old fallback allowed
        # those defaults to overwrite hunyuan15_settings.json before _restore() ran.
        ...
        self.btn_queue.setToolTip(
            "Toggle Queue mode.\n"
            "ON: clicking \"Generate Video\" will add the job to the worker queue.\n\n"
        ...
        self.vram_lab = QComboBox()
        try:
            for label, key in VRAM_LAB_PRESETS:
                self.vram_lab.addItem(label, key)
            self.vram_lab.setCurrentIndex(0)
            self.vram_lab.setToolTip(
                "Optional VRAM Lab memory control for Hunyuan startup/generation.\n"
                "Off keeps the current Hunyuan behavior unchanged.\n"
                "On uses the current Hunyuan-local VRAM Lab path."
            )
        except Exception:
            pass
        ...
        try:
            self.vram_profile.addItem("Auto", "auto")
            self.vram_profile.addItem("16 GB VRAM", "16")
            self.vram_profile.addItem("24 GB VRAM", "24")
            self.vram_profile.setCurrentIndex(0)
            self.vram_profile.setToolTip(
                "Lower VRAM cards should use 'Auto' and start testing with low frames at low resolution, for example 41 frames at 304p at 20fps."
            )
            self.vram_profile.currentIndexChanged.connect(self._on_vram_profile_changed)
        except Exception:
            pass

        self.vram_advanced_toggle = QCheckBox("VRAM advanced settings")
        self.vram_advanced_toggle.setChecked(False)
        try:
            self.vram_advanced_toggle.setToolTip("Show manual VRAM Lab tuning controls. Default is OFF; profile defaults are used while this is hidden.")
            self.vram_advanced_toggle.toggled.connect(self._toggle_vram_advanced_settings)
        except Exception:
            pass

        # Hunyuan-local VRAM Lab tuning. These settings are passed only to
        # helpers/hunyuan15_cli.py; shared VRAM Lab files are not changed.
        self.vram_text_hot_gb = QDoubleSpinBox()
        self.vram_text_hot_gb.setDecimals(1)
        self.vram_text_hot_gb.setRange(0.0, 20.0)
        self.vram_text_hot_gb.setSingleStep(0.5)
        self.vram_text_hot_gb.setValue(9.0)
        self.vram_text_hot_gb.setToolTip("Internal/default startup/text guard. Hidden because changing it has no meaningful effect in the current Hunyuan path.")

        self.vram_step_hot_gb = QDoubleSpinBox()
        self.vram_step_hot_gb.setDecimals(1)
        self.vram_step_hot_gb.setRange(0.25, 20.0)
        self.vram_step_hot_gb.setSingleStep(0.25)
        self.vram_step_hot_gb.setValue(1.0)
        self.vram_step_hot_gb.setToolTip("Denoise transformer hot/resident budget in GB. This is the main live VRAM Lab control used by the hooks. Can go very low for edge testing; 0.25 means almost no planned hotset.")

        self.vram_after_hot_gb = QDoubleSpinBox()
        self.vram_after_hot_gb.setDecimals(1)
        self.vram_after_hot_gb.setRange(0.0, 20.0)
        self.vram_after_hot_gb.setSingleStep(0.5)
        self.vram_after_hot_gb.setValue(0.0)
        self.vram_after_hot_gb.setToolTip("After-steps/finalize guard budget in GB. 0 = default cleanup. This is for the later VAE/save stage work.")

        self.vram_decode_min_free_gb = QDoubleSpinBox()
        self.vram_decode_min_free_gb.setDecimals(1)
        self.vram_decode_min_free_gb.setRange(0.0, 20.0)
        self.vram_decode_min_free_gb.setSingleStep(0.5)
        self.vram_decode_min_free_gb.setValue(0.5)
        self.vram_decode_min_free_gb.setToolTip("Target driver-free VRAM to try to preserve before each hidden VAE decode/postprocess call. default 1 or 2.")

        self.vram_decode_cleanup_passes = QSpinBox()
        self.vram_decode_cleanup_passes.setRange(0, 6)
        self.vram_decode_cleanup_passes.setValue(2)
        self.vram_decode_cleanup_passes.setToolTip("Extra cleanup / empty-cache passes around the hidden VAE decode phase. Default 1 or 2.")

        self.vram_decode_chunk_frames = QSpinBox()
        self.vram_decode_chunk_frames.setRange(0, 241)
        self.vram_decode_chunk_frames.setValue(8)
        self.vram_decode_chunk_frames.setToolTip("Experimental: split hidden VAE decode into smaller frame/time chunks. 0 = disabled. default is 8. Try 8, 16, 24, or 32 if the 100% progress-bar phase spills into shared memory.")

        self.vram_decode_chunk_overlap = QSpinBox()
        self.vram_decode_chunk_overlap.setRange(0, 16)
        self.vram_decode_chunk_overlap.setValue(4)
        self.vram_decode_chunk_overlap.setToolTip("Overlap latent time chunks during VAE decode, then crop the extra context. default is 4")

        self.vram_decode_unload_blocks = QCheckBox("Unload hooked transformer blocks before decode")
        self.vram_decode_unload_blocks.setChecked(True)
        self.vram_decode_unload_blocks.setToolTip("Before each hidden VAE decode call, force any transformer blocks still on CUDA back to CPU.")

        self.vram_decode_output_cpu = QCheckBox("Move decode output to CPU immediately")
        self.vram_decode_output_cpu.setChecked(True)
        self.vram_decode_output_cpu.setToolTip("Try to move decoded tensors/frames to CPU immediately after each hidden VAE decode call.")

        self.vram_step_profiler = QCheckBox("Profile one denoise step")
        self.vram_step_profiler.setChecked(False)
        self.vram_step_profiler.setToolTip("Logs per-block time and VRAM deltas for the first denoise step only. Use this to find the repeating sawtooth/drop point.")

        self.vram_step_profiler_steps = QSpinBox()
        self.vram_step_profiler_steps.setRange(1, 3)
        self.vram_step_profiler_steps.setValue(1)
        self.vram_step_profiler_steps.setToolTip("How many early denoise steps to profile. Keep this at 1 unless you really need more detail.")

        self.vram_step_cleanup_every = QSpinBox()
        self.vram_step_cleanup_every.setRange(0, 16)
        self.vram_step_cleanup_every.setValue(4)
        self.vram_step_cleanup_every.setToolTip("Denoise cleanup frequency. 1 = current safe cleanup every block. 2/4/8 = smoother but riskier. 0 = runtime/emergency default only.")

        self.vram_step_empty_cache = QCheckBox("Empty CUDA cache after unload")
        self.vram_step_empty_cache.setChecked(True)
        self.vram_step_empty_cache.setToolTip("Current safe behavior. OFF may reduce sawtooth/thrashing, but can increase shared-memory risk.")

        self.vram_step_release_after_forward = QCheckBox("Release block after forward")
        self.vram_step_release_after_forward.setChecked(True)
        self.vram_step_release_after_forward.setToolTip("Current safe behavior. OFF may keep useful blocks resident and reduce reloads, but uses more VRAM.")

        self.vram_step_unload_before_load = QCheckBox("Unload other blocks before load")
        self.vram_step_unload_before_load.setChecked(True)
        self.vram_step_unload_before_load.setToolTip("Current safe behavior. OFF may reduce unload/reload thrash, but can raise VRAM peaks.")

        # Hidden fixed/internal controls. Keep the behavior ON, but do not expose
        # them as user-facing tuning knobs because testing showed they are not useful
        # day-to-day settings for Hunyuan.
        for _hidden in (
            self.vram_decode_unload_blocks,
            self.vram_decode_output_cpu,
            self.vram_step_profiler,
            self.vram_step_profiler_steps,
            self.vram_step_empty_cache,
            self.vram_step_release_after_forward,
            self.vram_step_unload_before_load,
            self.vram_text_hot_gb,
        ):
            try:
        ...
        self.vram_min_free_gb = QDoubleSpinBox()
        self.vram_min_free_gb.setDecimals(1)
        self.vram_min_free_gb.setRange(0.0, 16.0)
        self.vram_min_free_gb.setSingleStep(0.5)
        self.vram_min_free_gb.setValue(0.5)
        self.vram_min_free_gb.setToolTip("Target free dedicated VRAM reserve in GB. Higher = safer, lower = more risk/speed.")

        self.vram_residency = QComboBox()
        self.vram_residency.addItem("Rolling / one block", "rolling")
        self.vram_residency.addItem("Planned hotset", "planned_hotset")
        try:
            self.vram_residency.setCurrentIndex(self.vram_residency.findData("planned_hotset"))
        except Exception:
            pass
        self.vram_residency.setToolTip("Rolling keeps one block resident. Planned hotset keeps a small planned hotset resident for the Hunyuan VRAM Lab default test profile.")

        self.vram_stable_fraction = QDoubleSpinBox()
        self.vram_stable_fraction.setDecimals(2)
        self.vram_stable_fraction.setRange(0.0, 2.0)
        self.vram_stable_fraction.setSingleStep(0.05)
        self.vram_stable_fraction.setValue(1.15)
        self.vram_stable_fraction.setToolTip("Optional stable hotset fraction for planned_hotset mode. 0 keeps rolling mode clean.")

        self.vram_stable_budget_gb = QDoubleSpinBox()
        self.vram_stable_budget_gb.setDecimals(1)
        self.vram_stable_budget_gb.setRange(0.0, 20.0)
        self.vram_stable_budget_gb.setSingleStep(0.5)
        self.vram_stable_budget_gb.setValue(1.0)
        self.vram_stable_budget_gb.setToolTip("Optional stable hotset GB budget for planned_hotset mode. 0 = disabled.")
        ...
        try:
            self.lbl_vram_control = QLabel("VRAM control:")
            self.lbl_vram_profile = QLabel("VRAM profile:")
            self.lbl_vram_advanced = QLabel("")
            adv_form.addRow(self.lbl_vram_control, self.vram_lab)
            adv_form.addRow(self.lbl_vram_profile, self.vram_profile)
            adv_form.addRow(self.lbl_vram_advanced, self.vram_advanced_toggle)
        except Exception:
            pass
        ...
        general_tab = QWidget()
        general_lay = QVBoxLayout(general_tab)
        general_lay.setContentsMargins(4, 4, 4, 4)
        general_lay.addLayout(adv_form)
        general_lay.addStretch(1)
        self.adv_tabs.addTab(general_tab, "General")

        self.vram_lab_tuning_tab = QWidget()
        vram_form = QFormLayout(self.vram_lab_tuning_tab)
        vram_form.setContentsMargins(8, 8, 8, 8)
        vram_form.setVerticalSpacing(8)
        vram_form.addRow(QLabel("Steps hot GB:"), self.vram_step_hot_gb)
        vram_form.addRow(QLabel("After/finalize guard GB:"), self.vram_after_hot_gb)
        vram_form.addRow(QLabel("Decode min-free GB:"), self.vram_decode_min_free_gb)
        vram_form.addRow(QLabel("Decode cleanup passes:"), self.vram_decode_cleanup_passes)
        vram_form.addRow(QLabel("Decode chunk frames:"), self.vram_decode_chunk_frames)
        vram_form.addRow(QLabel("Decode chunk overlap:"), self.vram_decode_chunk_overlap)
        vram_form.addRow(QLabel("Step cleanup every N blocks:"), self.vram_step_cleanup_every)
        vram_form.addRow(QLabel("Min driver-free GB:"), self.vram_min_free_gb)
        vram_form.addRow(QLabel("Residency:"), self.vram_residency)
        vram_form.addRow(QLabel("Stable hotset fraction:"), self.vram_stable_fraction)
        vram_form.addRow(QLabel("Stable hotset budget GB:"), self.vram_stable_budget_gb)
        # VRAM Lab manual tab is hidden by default and shown by the
        # "VRAM advanced settings" toggle.
        adv_body_lay.addWidget(self.adv_tabs)
        try:
            self._toggle_vram_advanced_settings(False)
        except Exception:
            pass
        ...
        try:
            ...
            pass
        try:
            self.vram_lab.currentIndexChanged.connect(self._on_vram_lab_mode_changed)
        except Exception:
            pass

        # Autosave (debounced) — saves all settings (including Advanced) even if you don't run a job.
        # Save lock was already created at the top of __init__; keep it locked
        # until _restore() has fully finished and the delayed unlock fires.
    ...
    def _restore(self):
        ...
        try:
            mode = str(s.get("vram_lab_mode", "off") or "off").lower().strip()
            if mode in ("on", "safe", "balanced", "aggressive"):
                mode = "safe"
            idx = self.vram_lab.findData(mode)
            self.vram_lab.setCurrentIndex(idx if idx >= 0 else 0)
        except Exception:
            pass
        try:
            prof = str(s.get("vram_lab_profile", "auto") or "auto").lower().strip()
            if prof not in ("auto", "16", "24"):
                prof = "auto"
            idx = self.vram_profile.findData(prof)
            self.vram_profile.setCurrentIndex(idx if idx >= 0 else 0)
            self.vram_advanced_toggle.setChecked(bool(s.get("vram_lab_advanced_settings", False)))
        except Exception:
            pass
        try:
            self.vram_text_hot_gb.setValue(9.0)
            self.vram_step_hot_gb.setValue(float(s.get("vram_lab_step_hot_gb", self.vram_step_hot_gb.value())))
            self.vram_after_hot_gb.setValue(float(s.get("vram_lab_after_hot_gb", self.vram_after_hot_gb.value())))
            self.vram_decode_min_free_gb.setValue(float(s.get("vram_lab_decode_min_free_gb", self.vram_decode_min_free_gb.value())))
            self.vram_decode_cleanup_passes.setValue(int(s.get("vram_lab_decode_cleanup_passes", self.vram_decode_cleanup_passes.value())))
            self.vram_decode_chunk_frames.setValue(int(s.get("vram_lab_decode_chunk_frames", self.vram_decode_chunk_frames.value())))
            self.vram_decode_chunk_overlap.setValue(int(s.get("vram_lab_decode_chunk_overlap", self.vram_decode_chunk_overlap.value())))
            self.vram_decode_unload_blocks.setChecked(True)
            self.vram_decode_output_cpu.setChecked(True)
            self.vram_step_profiler.setChecked(False)
            self.vram_step_profiler_steps.setValue(1)
            self.vram_step_cleanup_every.setValue(int(s.get("vram_lab_step_cleanup_every", self.vram_step_cleanup_every.value())))
            self.vram_step_empty_cache.setChecked(True)
            self.vram_step_release_after_forward.setChecked(True)
            self.vram_step_unload_before_load.setChecked(True)
            self.vram_min_free_gb.setValue(float(s.get("vram_lab_min_free_gb", self.vram_min_free_gb.value())))
            self.vram_stable_fraction.setValue(float(s.get("vram_lab_stable_fraction", self.vram_stable_fraction.value())))
            self.vram_stable_budget_gb.setValue(float(s.get("vram_lab_stable_budget_gb", self.vram_stable_budget_gb.value())))
            want_res = str(s.get("vram_lab_residency", self.vram_residency.currentData()) or "planned_hotset")
            idx_res = self.vram_residency.findData(want_res)
            if idx_res >= 0:
                self.vram_residency.setCurrentIndex(idx_res)
            self._toggle_vram_advanced_settings(bool(self.vram_advanced_toggle.isChecked()))
            self._apply_vram_profile_to_ui(force=not self._vram_advanced_enabled())
        except Exception:
            pass
        try:
            self._on_vram_lab_mode_changed()
        except Exception:
            pass
...
# Restore Queue toggle (Use Queue) — forced OFF when Extend > 0
    ...
    def _schedule_persist(self, *args) -> None:
        ...
        if not getattr(self, "_settings_save_enabled", False):
            return
        try:
            t = getattr(self, "_autosave_timer", None)
            if t is not None:
    ...
    def _detect_vram_total_gb(self) -> float:
        """Best-effort local GPU VRAM detection for the Hunyuan VRAM profile selector."""
        try:
    ...
    def _selected_vram_profile_key(self) -> str:
        try:
            key = str(self.vram_profile.currentData() or "auto").lower().strip()
        except Exception:
            key = "auto"
        if key not in ("auto", "16", "24"):
            key = "auto"
        if key == "auto":
            total = self._detect_vram_total_gb()
            # Auto split requested for Hunyuan VRAM Lab:
            # below 23.01 GB -> 16 GB profile, above 23 GB -> 24 GB profile.
            # In practice this keeps 24 GB cards on the 24 profile while 16/20/22 GB
            # cards stay on the safer 16 profile.
            return "24" if total > 23.0 else "16"
        return key

    def _selected_vram_profile_hotset_budget_gb(self) -> float:
        # Hunyuan VRAM Lab profile defaults.
        # 16 GB profile: keep the live denoise/hotset budgets very conservative.
        # 24 GB profile: allow a larger hotset on cards with enough real VRAM.
        # Users can still tune these manually in the VRAM tab; selecting the profile
        # again applies these helper defaults again.
        return 9.0 if self._selected_vram_profile_key() == "24" else 1.0

    def _selected_vram_profile_step_hot_gb(self) -> float:
        return 9.0 if self._selected_vram_profile_key() == "24" else 1.0

    def _vram_advanced_enabled(self) -> bool:
        try:
            return bool(self.vram_advanced_toggle.isChecked())
        except Exception:
            return False

    def _apply_vram_profile_to_ui(self, *, force: bool = False) -> None:
        try:
            if force or not self._vram_advanced_enabled():
                self.vram_step_hot_gb.setValue(float(self._selected_vram_profile_step_hot_gb()))
                self.vram_stable_budget_gb.setValue(float(self._selected_vram_profile_hotset_budget_gb()))
        except Exception:
            pass

    def _on_vram_profile_changed(self, *_args) -> None:
        try:
            # Re-selecting/changing a profile should always restore only the
            # planned-hotset budget helper default for that profile.
            self._apply_vram_profile_to_ui(force=True)
        except Exception:
            pass
        try:
    ...
    def _vram_lab_is_on(self) -> bool:
        try:
            mode = str(self.vram_lab.currentData() if getattr(self, "vram_lab", None) is not None else "off").lower().strip()
        except Exception:
            mode = "off"
        return mode not in ("", "off", "none")

    def _toggle_vram_advanced_settings(self, checked: bool) -> None:
        try:
            checked = bool(checked) and self._vram_lab_is_on()
            idx = self.adv_tabs.indexOf(self.vram_lab_tuning_tab)
            if checked:
                if idx < 0:
                    self.adv_tabs.addTab(self.vram_lab_tuning_tab, "VRAM Lab")
                self.vram_lab_tuning_tab.setVisible(True)
            else:
                if idx >= 0:
                    self.adv_tabs.removeTab(idx)
                self._apply_vram_profile_to_ui(force=True)
        except Exception:
            pass
        try:
    ...
    def _wire_autosave(self) -> None:
        ...
        # combos
        for cb_name in ("model", "attn", "resolution", "aspect", "vram_lab", "vram_profile", "vram_residency"):
            try:
        ...
        # spinboxes
        for sb_name in ("frames", "steps", "fps", "bitrate_kbps", "seed", "extend", "extend_join_drop_spin", "extend_join_blend_spin", "vram_step_hot_gb", "vram_after_hot_gb", "vram_decode_min_free_gb", "vram_decode_cleanup_passes", "vram_decode_chunk_frames", "vram_decode_chunk_overlap", "vram_step_cleanup_every", "vram_min_free_gb", "vram_stable_fraction", "vram_stable_budget_gb"):
            try:
        ...
        # checkboxes
        for chk_name in ("extend_merge", "offload", "group_offload", "first_block_cache", "pab", "attn_slicing", "vae_slicing", "tiling", "chk_video2video", "vram_advanced_toggle"):
            try:
    ...
    def _persist(self):
        ...
        s = dict(
            ...
            vram_lab_mode=str(self.vram_lab.currentData() if getattr(self, 'vram_lab', None) is not None else "off"),
            vram_lab_profile=str(self.vram_profile.currentData() if getattr(self, 'vram_profile', None) is not None else "auto"),
            vram_lab_advanced_settings=bool(getattr(self, 'vram_advanced_toggle', None).isChecked() if getattr(self, 'vram_advanced_toggle', None) is not None else False),
            vram_lab_step_hot_gb=float(getattr(self, 'vram_step_hot_gb', None).value() if getattr(self, 'vram_step_hot_gb', None) is not None else 9.0),
            vram_lab_after_hot_gb=float(getattr(self, 'vram_after_hot_gb', None).value() if getattr(self, 'vram_after_hot_gb', None) is not None else 6.0),
            vram_lab_decode_min_free_gb=float(getattr(self, 'vram_decode_min_free_gb', None).value() if getattr(self, 'vram_decode_min_free_gb', None) is not None else 6.0),
            vram_lab_decode_cleanup_passes=int(getattr(self, 'vram_decode_cleanup_passes', None).value() if getattr(self, 'vram_decode_cleanup_passes', None) is not None else 1),
            vram_lab_decode_chunk_frames=int(getattr(self, 'vram_decode_chunk_frames', None).value() if getattr(self, 'vram_decode_chunk_frames', None) is not None else 0),
            vram_lab_decode_chunk_overlap=int(getattr(self, 'vram_decode_chunk_overlap', None).value() if getattr(self, 'vram_decode_chunk_overlap', None) is not None else 2),
            vram_lab_decode_unload_blocks=True,
            vram_lab_decode_output_cpu=True,
            vram_lab_step_cleanup_every=int(getattr(self, 'vram_step_cleanup_every', None).value() if getattr(self, 'vram_step_cleanup_every', None) is not None else 1),
            vram_lab_step_empty_cache=True,
            vram_lab_step_release_after_forward=True,
            vram_lab_step_unload_before_load=True,
            vram_lab_min_free_gb=float(getattr(self, 'vram_min_free_gb', None).value() if getattr(self, 'vram_min_free_gb', None) is not None else 6.5),
            vram_lab_residency=str(getattr(self, 'vram_residency', None).currentData() if getattr(self, 'vram_residency', None) is not None else 'rolling'),
            vram_lab_stable_fraction=float(getattr(self, 'vram_stable_fraction', None).value() if getattr(self, 'vram_stable_fraction', None) is not None else 0.0),
            vram_lab_stable_budget_gb=float(getattr(self, 'vram_stable_budget_gb', None).value() if getattr(self, 'vram_stable_budget_gb', None) is not None else 0.0),
    ...
    def _on_vram_lab_mode_changed(self, *args) -> None:
        """Show Hunyuan VRAM Lab profile/advanced controls only when VRAM Lab is on."""
        on = self._vram_lab_is_on()

        # Keep the simple on/off selector visible, but hide the profile selector
        # and the advanced-tab toggle while VRAM Lab itself is off.
        for name in ("lbl_vram_profile", "lbl_vram_advanced", "vram_profile", "vram_advanced_toggle"):
            try:
        ...
        try:
            self.vram_lab_tuning_tab.setEnabled(bool(on))
        except Exception:
            pass

        # If VRAM Lab is off, make sure the manual VRAM tab is not visible,
        # even when the saved advanced toggle is ON. When turned back on, the
        # saved toggle decides whether the tab appears again.
        try:
            if not on:
                idx = self.adv_tabs.indexOf(self.vram_lab_tuning_tab)
                if idx >= 0:
    ...
    def _set_queue_enabled(self, enabled: bool, persist: bool = True, quiet: bool = False) -> None:
        ...
        try:
            q_tip = (
                ...
                "Extend == 0  → Queue ON (jobs are added to the worker queue).\n"
        ...
        try:
            run_tip = (
                "Generate Video will add the job to the worker queue.\n"
...
# Stop the direct-run log timer and reset UI state.
    ...
    # ---------- env paths ----------
    def _env_python_candidates(self) -> list[Path]:
        """Return all plausible Hunyuan 1.5 Python interpreters, best-first.

        The official Hunyuan installer now uses a local conda-style env at:
            environments/.hunyuan15_official/python.exe

        Older/venv layouts used:
            environments/.hunyuan15_official/Scripts/python.exe

        The important part: Scripts/python.exe may exist but be the wrong
        interpreter for the conda env. So _env_python() below does not just pick
        the first existing file; it verifies that the interpreter can import the
        Hunyuan runtime dependency (diffusers) and caches the result.
        """
        root = root_dir()
        envs = [
        ...
        # Manual emergency override, useful if a user has a custom conda env path.
        try:
            override = os.environ.get("FV_HUNYUAN15_PYTHON", "").strip()
            if override:
        ...
        for env in envs:
            candidates.extend([
                env / "python.exe",              # conda -p layout on Windows
                env / "Scripts" / "python.exe",  # venv-style fallback
                env / "bin" / "python",          # non-Windows fallback
            ])
        return candidates

    def _python_can_import_diffusers(self, py: Path) -> bool:
        """Small cached probe so we don't launch a Python without Hunyuan deps."""
        try:
            key = str(Path(py).resolve()).lower()
        except Exception:
            key = str(py).lower()
        cache = getattr(self, "_hunyuan_python_probe_cache", None)
        if not isinstance(cache, dict):
            cache = {}
            try:
                self._hunyuan_python_probe_cache = cache
            except Exception:
                pass
        if key in cache:
        ...
        try:
            if py.exists() and py.is_file():
                # Keep this quiet and bounded. A good env returns quickly; a bad
                # env returns with ModuleNotFoundError, which is exactly what we
                # are trying to avoid in queued jobs.
                proc = subprocess.run(
                    [str(py), "-c", "import diffusers"],
                    cwd=str(root_dir()),
    ...
    def _env_python(self) -> Path:
        """Return the Hunyuan 1.5 env Python without blocking the UI.

        Older builds verified the interpreter by launching:
            python -c "import diffusers"

        That probe is safe in a worker/installer, but it is too expensive to do
        on the Qt UI thread when the user clicks Generate/Queue. With the conda
        installer now fixed, the correct runtime location is deterministic:
            <root>/environments/.hunyuan15_official/python.exe

        So this resolver only checks file existence and records what it chose.
        The worker/CLI will still show the real import error if the env is broken.
        """
        candidates = self._env_python_candidates()
        checked: list[str] = []

        # 1) Strongly prefer the official conda-prefix Python. This is the
        # non-blocking fast path and avoids freezing the app while queueing.
        for cand in candidates:
            try:
                norm = str(cand).replace("\\", "/").lower()
                if (
                    "/environments/.hunyuan15_official/python.exe" in norm
                    and "/scripts/" not in norm
                    and cand.exists()
                    and cand.is_file()
                ):
                    checked.append(f"{cand} => selected official conda python (import probe skipped)")
                    try:
                        self._hunyuan_python_last_checked = checked[:]
                    except Exception:
                        pass
                    return cand
                if not (cand.exists() and cand.is_file()):
                    checked.append(f"{cand} => missing")
            except Exception as ex:
                checked.append(f"{cand} => check error: {ex}")

        # 2) Manual override or any other conda/root python, still without an
        # import probe. This keeps the UI responsive.
        for cand in candidates:
            try:
                norm = str(cand).replace("\\", "/").lower()
                if cand.exists() and cand.is_file() and norm.endswith("/python.exe") and "/scripts/" not in norm:
                    checked.append(f"{cand} => selected root python fallback (import probe skipped)")
                    try:
                        self._hunyuan_python_last_checked = checked[:]
                    except Exception:
                        pass
                ...
                checked.append(f"{cand} => fallback check error: {ex}")

        # 3) Last resort: existing venv-style interpreter. This is only for old
        # portable installs and should not be used by the current conda installer.
        for cand in candidates:
            try:
                if cand.exists() and cand.is_file():
                    checked.append(f"{cand} => selected last-resort existing python (import probe skipped)")
                    try:
                        self._hunyuan_python_last_checked = checked[:]
                    except Exception:
                        pass
    ...
    def on_install(self):
        ...
        if script.exists():
            ...
            self._run(str(py), ["-u", str(script), "--root", str(root)], root)
            return

        # Legacy fallback only, for older copies that still ship the BAT installer.
        bat = root / "presets" / "extra_env" / "hunyuan15_install.bat"
        if not bat.exists():
            QMessageBox.critical(self, "Missing file", f"Not found: {script}")
            return
        self._run("cmd.exe", ["/c", str(bat)], root)
...
Pick a Start image, or enable Video→Video and pick a source video.""",
    ...
    def on_generate(self):
        ...
        # If Queue toggle is ON, do not run the direct-run preflight first.
        # Older code validated/env-checked here and then called on_queue_generate(),
        # which repeated the same work and could make the app look frozen.
        try:
            ext_now = int(self.extend.value())
        except Exception:
            ext_now = 0
        if self._queue_enabled() and ext_now <= 0:
            try:
                self._append("[ui timing] Generate button routed directly to Queue preflight (no duplicate Direct preflight).")
            except Exception:
                pass
            try:
        ...
        try:
            _prev = _t0
            parts = []
            for _name, _tm in _marks:
                parts.append(f"{_name}={(_tm - _prev):.3f}s")
                _prev = _tm
            self._append("[ui timing] direct generate preflight: " + ", ".join(parts) + f", total={(time.perf_counter() - _t0):.3f}s")
        except Exception:
            pass
        self._run(str(venv_py), args, root_dir())

    def on_queue_generate(self):
        """Queue a single generation job without switching away from the current tab."""
        _t0 = time.perf_counter()
        _marks = []
        def _mark(name: str):
            try:
        ...
        try:
            ...
            for _name, _tm in _marks:
                parts.append(f"{_name}={(_tm - _prev):.3f}s")
                _prev = _tm
            self._append("[ui timing] queue generate preflight: " + ", ".join(parts) + f", total={(time.perf_counter() - _t0):.3f}s")
        except Exception:
            pass
    ...
    def _build_generate_args(
        ...
        if self.tiling.isChecked():
            args += ["--tiling"]
        try:
            vram_mode = str(self.vram_lab.currentData() if getattr(self, "vram_lab", None) is not None else "off").lower().strip()
        except Exception:
            vram_mode = "off"
        if vram_mode in ("on", "safe", "balanced", "aggressive"):
            vram_mode = "safe"
        elif vram_mode != "off":
            vram_mode = "off"
        args += ["--vram-lab", vram_mode]
        if vram_mode != "off":
            try:
                step_hot = float(self.vram_step_hot_gb.value())
                stable_budget = float(self.vram_stable_budget_gb.value())
                if not self._vram_advanced_enabled():
                    step_hot = float(self._selected_vram_profile_step_hot_gb())
                    stable_budget = float(self._selected_vram_profile_hotset_budget_gb())
                    try:
                        self.vram_step_hot_gb.setValue(step_hot)
                        self.vram_stable_budget_gb.setValue(stable_budget)
                    except Exception:
                        pass
                args += ["--vram-lab-step-hot-gb", str(step_hot)]
                args += ["--vram-lab-after-hot-gb", str(float(self.vram_after_hot_gb.value()))]
                args += ["--vram-lab-decode-min-free-gb", str(float(self.vram_decode_min_free_gb.value()))]
                args += ["--vram-lab-decode-cleanup-passes", str(int(self.vram_decode_cleanup_passes.value()))]
                args += ["--vram-lab-decode-chunk-frames", str(int(self.vram_decode_chunk_frames.value()))]
                args += ["--vram-lab-decode-chunk-overlap", str(int(self.vram_decode_chunk_overlap.value()))]
                args += ["--vram-lab-decode-unload-blocks"]
                args += ["--vram-lab-decode-output-cpu"]
                args += ["--vram-lab-step-cleanup-every", str(int(self.vram_step_cleanup_every.value()))]
                args += ["--vram-lab-step-empty-cache"]
                args += ["--vram-lab-step-release-after-forward"]
                args += ["--vram-lab-step-unload-before-load"]
                args += ["--vram-lab-min-free-gb", str(float(self.vram_min_free_gb.value()))]
                args += ["--vram-lab-residency", str(self.vram_residency.currentData() or "rolling")]
                args += ["--vram-lab-stable-fraction", str(float(self.vram_stable_fraction.value()))]
                args += ["--vram-lab-profile", str(self._selected_vram_profile_key())]
                args += ["--vram-lab-stable-budget-gb", str(stable_budget)]
            except Exception:
                pass
        return args
    ...
    def _queue_job(self, args: list[str], outfile: Path, label: str = "hunyuan15", quiet: bool = False, sidecar_meta: dict | None = None) -> str | None:
        ...
        try:
            ...
            venv_py = self._env_python()
            # Final guard: prefer conda-root python.exe over Scripts/python.exe for the official Hunyuan env.
            # This prevents creating jobs that later fail with missing packages such as diffusers.
            try:
                venv_txt = str(venv_py).replace('\\', '/').lower()
                if '.hunyuan15_official/scripts/python.exe' in venv_txt:
                    root_py = Path(str(venv_py)).parent.parent / 'python.exe'
                    if root_py.exists() and root_py.is_file():
                        venv_py = root_py
            except Exception:
                pass
            ...
            jid = enq(
                ...
                str(Path(str(outfile)).parent),
                {
                    ...
                    "model": model_key,
                    "hunyuan15_python_selected": str(venv_py),
                    "hunyuan15_python_checked": list(getattr(self, "_hunyuan_python_last_checked", []) or [])[:20],
                },
                priority=650,
            )
            if not quiet:
