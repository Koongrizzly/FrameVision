...
def build_structured_caption(prompt: str, width: int, height: int) -> str:
...
if _FVQtWidgets is not None:
    class Ideogram4Widget(_FVQtWidgets.QWidget):
        ...
        def _build_generate_tab(self) -> None:
            ...
            self.guidance = _FVQtWidgets.QDoubleSpinBox(); self.guidance.setRange(0.0, 20.0); self.guidance.setDecimals(2); self.guidance.setSingleStep(0.1); self.guidance.setValue(3.5)
            self.seed = _FVQtWidgets.QSpinBox(); self.seed.setRange(-1, 2147483647); self.seed.setValue(-1)
            for label, widget in (("W", self.width), ("H", self.height), ("Steps", self.steps), ("CFG", self.guidance), ("Seed", self.seed)):
        ...
        def _connect_signals(self) -> None:
            ...
            for w in (self.gguf_dir, self.sd_cli_path, self.output_dir):
            ...
            for w in (self.width, self.height, self.steps, self.guidance, self.seed):
        ...
        def refresh_files(self) -> None:
            ...
            saved = load_saved_settings()
            pairs = [
                (self.diffusion_file, "diffusion", saved.get("gguf_diffusion_file") or default_gguf_file_name("diffusion")),
                (self.uncond_file, "unconditional", saved.get("gguf_unconditional_file") or default_gguf_file_name("unconditional")),
                (self.llm_file, "llm", saved.get("gguf_llm_file") or default_gguf_file_name("llm")),
                (self.vae_file, "vae", saved.get("gguf_vae_file") or default_gguf_file_name("vae")),
            ]
            for combo, role, preferred in pairs:
                ...
                if preferred and preferred not in names:
                    names.insert(0, str(preferred))
                if not names:
                    names = [str(preferred or "")]
                for name in names:
                    if name:
                        combo.addItem(name)
                combo.blockSignals(False)
                self._choose_combo_text(combo, str(preferred or ""))
        ...
        def _load_settings_into_ui(self) -> None:
            ...
            try: self.guidance.setValue(float(s.get("guidance", 3.5)))
            except Exception: pass
        ...
        def add_current_to_framevision_queue(self) -> None:
            ...
            try:
                ...
                try:
                    _FVQtWidgets.QMessageBox.information(self, "Queued", f"Job queued to jobs/pending.\n\n{jid}")
                except Exception:
                    pass
        ...
        def _job_to_config(self, job: dict[str, Any]) -> GenerateConfig:
            ...
            return GenerateConfig(
                ...
                guidance=float(job.get("guidance") or 3.5),
                preset="Custom",
...
def launch_gui() -> int:
    ...
    class App:
        def __init__(self, root: tk.Tk):
            ...
            self.guidance_var = tk.StringVar(value="3.5")
            ...
            self._refresh_gguf_file_dropdowns()
        ...
        def _build_ui(self):
            ...
            ToolTip(self.guidance_label, "Guidance / CFG-like strength. Default: 3.5.")
            ToolTip(self.guidance_entry, "Guidance / CFG-like strength. Default: 3.5.")
        ...
        def _build_settings_tab(self):
            ...
            self.gguf_dir_entry.bind("<Return>", lambda event: (self._refresh_gguf_file_dropdowns(), self._save_settings()))
            self.gguf_dir_entry.bind("<FocusOut>", lambda event: (self._refresh_gguf_file_dropdowns(), self._save_settings()))
        ...
        def _load_settings(self):
            ...
            self.gguf_diffusion_var.set(str(data.get("gguf_diffusion_file", GUI_DEFAULTS["gguf_diffusion_file"])))
            self.gguf_unconditional_var.set(str(data.get("gguf_unconditional_file", GUI_DEFAULTS["gguf_unconditional_file"])))
            self.gguf_llm_var.set(str(data.get("gguf_llm_file", GUI_DEFAULTS["gguf_llm_file"])))
            self.gguf_vae_var.set(str(data.get("gguf_vae_file", GUI_DEFAULTS["gguf_vae_file"])))
        ...
        def _gather_settings(self) -> dict[str, Any]:
            return {
                ...
                "gguf_diffusion_file": self.gguf_diffusion_var.get().strip() or GUI_DEFAULTS["gguf_diffusion_file"],
                "gguf_unconditional_file": self.gguf_unconditional_var.get().strip() or GUI_DEFAULTS["gguf_unconditional_file"],
                "gguf_llm_file": self.gguf_llm_var.get().strip() or GUI_DEFAULTS["gguf_llm_file"],
                "gguf_vae_file": self.gguf_vae_var.get().strip() or GUI_DEFAULTS["gguf_vae_file"],
        ...
        def _pick_default_file(self, names: list[str], current: str, key: str) -> str:
            ...
            if key == "diffusion":
                for name, low in lowered:
                    if "ideogram4" in low and "uncond" not in low:
                        return name
            elif key == "unconditional":
                for name, low in lowered:
                    if "uncond" in low:
                        return name
            elif key == "llm":
                ...
                for name, low in lowered:
                    if "flux2" in low or "vae" in low or "ae" in low:
                        return name
            fallback = DEFAULT_GGUF_FILES.get(key, "")
            if fallback in names:
                return fallback
            return names[0] if names else fallback

        def _refresh_gguf_file_dropdowns(self):
            ...
            self.gguf_diffusion_var.set(self._pick_default_file(diffusion_names, self.gguf_diffusion_var.get().strip(), "diffusion"))
            self.gguf_unconditional_var.set(self._pick_default_file(unconditional_names, self.gguf_unconditional_var.get().strip(), "unconditional"))
            self.gguf_llm_var.set(self._pick_default_file(llm_names, self.gguf_llm_var.get().strip(), "llm"))
            self.gguf_vae_var.set(self._pick_default_file(vae_names, self.gguf_vae_var.get().strip(), "vae"))
        ...
        def _collect_config(self) -> GenerateConfig:
            ...
            return GenerateConfig(
                ...
                raw_prompt=self.raw_prompt_var.get(),
