...
- Uses ONNX models stored under: root/models/bg/
    - BiRefNet-COD-epoch_125.onnx (or another BiRefNet*.onnx alias)
    - modnet.onnx (or another modnet*.onnx alias)
...
def _project_root() -> Path:
    # helpers/background.py -> root. If this file is ever run/copied from the
    # project root directly, use that folder instead of stepping one level too far.
...
def _onnx_file_ok(path: Path, min_bytes: int = 1024) -> bool:
    """Return True for a real, complete-looking ONNX file. Never accepts temp files."""
    try:
        if not path.exists() or not path.is_file():
            return False
        name = path.name.lower()
        if name.endswith((".part", ".tmp", ".download", ".crdownload")):
            return False
...
def _iter_onnx_files(models_dir: Path):
    try:
        for p in sorted(Path(models_dir).iterdir(), key=lambda x: x.name.lower()):
...
def _find_onnx_model(models_dir: Path, preferred_names, name_markers, min_bytes: int) -> Path:
    """Find a model by exact filename first, then by safe filename markers.

    This avoids the old problem where a folder being non-empty could be treated as
    success, while still accepting downloaded aliases like BiRefNet*.onnx.
    """
    models_dir = Path(models_dir)

    for name in preferred_names:
        p = models_dir / str(name)
        if _onnx_file_ok(p, min_bytes):
            return p

    markers = tuple(str(m).lower() for m in name_markers if str(m).strip())
    for p in _iter_onnx_files(models_dir):
        n = p.name.lower()
        if any(m in n for m in markers) and _onnx_file_ok(p, min_bytes):
            return p

    # Missing fallback keeps the UI error stable and tells the user the canonical name.
    return models_dir / str(preferred_names[0])


def _birefnet_model_path(models_dir: Path) -> Path:
    return _find_onnx_model(
        models_dir,
        preferred_names=(
        ...
        name_markers=("birefnet", "biref"),
        # The COD model is roughly 900 MB. A low floor avoids accepting broken stubs.
        min_bytes=100 * 1024 * 1024,
    )


def _modnet_model_path(models_dir: Path) -> Path:
    return _find_onnx_model(
        models_dir,
        preferred_names=(
            "modnet.onnx",
        ...
        name_markers=("modnet", "portrait_matting"),
        min_bytes=1 * 1024 * 1024,
    )


def _rmbg14_model_path(models_dir: Path, *, fp16: bool = False) -> Path:
    if fp16:
        return _find_onnx_model(
            models_dir,
            preferred_names=(
            ...
            name_markers=("rmbg-14_fp16", "rmbg_14_fp16", "rmbg-1.4_fp16", "model_fp16"),
    ...
    return _find_onnx_model(
        ...
        name_markers=("rmbg-14", "rmbg_14", "rmbg-1.4"),
        min_bytes=80 * 1024 * 1024,
    )


def _friendly_onnx_label(path: Path) -> str:
    name = path.name
    low = name.lower()
    if "biref" in low:
        return "BiRefNet (general objects, slow)"
    if "modnet" in low or "portrait" in low:
        return "MODNet (portraits, fast)"
    if "rmbg" in low:
        if "fp16" in low:
            return "RMBG-1.4 FP16 (test)"
        return "RMBG-1.4 (test)"
    return f"ONNX: {path.stem}"


def _unique_model_label(base: str, existing: Dict[str, OnnxModel] | set[str]) -> str:
    used = set(existing.keys()) if isinstance(existing, dict) else set(existing)
    if base not in used:
        return base
    i = 2
...
class OnnxModel:
    ...
    def ensure_loaded(self) -> None:
        ...
        self._input_type = getattr(inputs[0], "type", None)
    ...
    def run(self, input_tensor: np.ndarray) -> np.ndarray:
        ...
        try:
            if self._input_type and "float16" in str(self._input_type).lower():
...
class BgRemovePane(QtWidgets.QWidget):
    ...
    def __init__(self, parent: Optional[QtWidgets.QWidget] = None):
        ...
        self.models: Dict[str, OnnxModel] = self._discover_onnx_models()

        defaults = {
            "model_key": "BiRefNet (general objects, slow)",
            "ref_long_side": 1024,
            "normalize": "-1..1",
            "cutoff": 0.15,
            "gamma": 1.0,
    ...
    def _discover_onnx_models(self) -> Dict[str, OnnxModel]:
        """Build the model dropdown from installed ONNX files.

        Known models get friendly names. Any other .onnx file in models/bg is also
        shown as a test entry so new one-file ONNX background removers can be tried
        without patching the dropdown again.
        """
        models: Dict[str, OnnxModel] = {}
        used_paths: set[str] = set()

        def add(label: str, path: Path, model_label: str) -> None:
            try:
            ...
            if rp in used_paths:
                return
            used_paths.add(rp)
            models[_unique_model_label(label, models)] = OnnxModel(Path(path), model_label)

        known = (
            ("BiRefNet (general objects, slow)", _birefnet_model_path(self._models_dir), "BiRefNet"),
            ("MODNet (portraits, fast)", _modnet_model_path(self._models_dir), "MODNet"),
            ("RMBG-1.4 (test)", _rmbg14_model_path(self._models_dir, fp16=False), "RMBG-1.4"),
            ("RMBG-1.4 FP16 (test)", _rmbg14_model_path(self._models_dir, fp16=True), "RMBG-1.4 FP16"),
        )
        for label, path, model_label in known:
            if _onnx_file_ok(Path(path), min_bytes=1 * 1024 * 1024):
                add(label, Path(path), model_label)

        for p in _iter_onnx_files(self._models_dir):
            if not _onnx_file_ok(p, min_bytes=1 * 1024 * 1024):
                continue
            label = _friendly_onnx_label(p)
            model_label = label.replace(" (test)", "").replace("ONNX: ", "")
            add(label, p, model_label)

        if not models:
            # Stable fallback when no ONNX files are installed yet.
            add("BiRefNet (general objects, slow)", _birefnet_model_path(self._models_dir), "BiRefNet")
            add("MODNet (portraits, fast)", _modnet_model_path(self._models_dir), "MODNet")
        return models

    def _sync_model_paths(self) -> None:
        """Refresh model list in case ONNX files were added while the pane was open."""
        old_key = str(self.settings.data.get("model_key", ""))
        current = self.cmb_model.currentText() if hasattr(self, "cmb_model") else old_key
        old_available = {k: str(m.path) for k, m in self.models.items()}
        new_models = self._discover_onnx_models()
        new_available = {k: str(m.path) for k, m in new_models.items()}
        if old_available == new_available:
            return
        self.models = new_models
        if hasattr(self, "cmb_model"):
            blocker = QtCore.QSignalBlocker(self.cmb_model)
            _ = blocker
            self.cmb_model.clear()
            self.cmb_model.addItems(list(self.models.keys()))
            target = current if current in self.models else old_key
            idx = self.cmb_model.findText(target)
            if idx < 0 and self.cmb_model.count() > 0:
                idx = 0
            if idx >= 0:
                self.cmb_model.setCurrentIndex(idx)
                try:
                    self.settings.set("model_key", self.cmb_model.currentText())
                except Exception:
                    pass
  ...
  #      titles.addStretch(1)
   #     titles.addWidget(self.lbl_right)
    ...
    def _apply_settings_to_ui(self, s: Dict[str, Any]) -> None:
        ...
        if idx < 0 and model_key == "BiRefNet (general objects)":
            idx = self.cmb_model.findText("BiRefNet (general objects, slow)")
        if idx >= 0:
            self.cmb_model.setCurrentIndex(idx)
    ...
    def _refresh_model_status(self) -> None:
        self._sync_model_paths()
        model_key = str(self.settings.data.get("model_key") or self.cmb_model.currentText())
        model = self.models.get(model_key)
        if model is None and self.cmb_model.count() > 0:
            model_key = self.cmb_model.currentText()
            model = self.models.get(model_key)
        if model is None or not model.is_available():
            missing_name = model.path.name if model is not None else "no ONNX model selected"
        ...
        count = len([m for m in self.models.values() if m.is_available()])
        self.lbl_info.setText(
            f"Detected {count} ONNX background model(s). "
            "Known files get friendly names; unknown .onnx files are shown as test entries."
        )
        self.lbl_info.setStyleSheet("color: #888;")
    ...
    def run_remove(self) -> None:
        if self._pil_original is None:
            ...
            return
        self._sync_model_paths()
        model_key = str(self.settings.data.get("model_key"))
        model = self.models.get(model_key)
        if model is None and model_key == "BiRefNet (general objects)":
            model_key = "BiRefNet (general objects, slow)"
            model = self.models.get(model_key)
        if model is None:
            model_key = self.cmb_model.currentText()
        ...
        if not model.is_available():
            ...
            return
        try:
            self.settings.set("model_key", str(model_key))
        except Exception:
            pass
...
def remove_background_file(
    ...
    mod_modnet = OnnxModel(_modnet_model_path(models_dir), "MODNet")
    mod_biref = OnnxModel(_birefnet_model_path(models_dir), "BiRefNet")
