...
def _install_ltx_gemma_partial_streaming_gate(ctx: Dict[str, Any], allow_main_transformer_streaming: bool = False) -> None:
    ...
    This gate keeps the useful early owner point but changes the shape:
      ...
      * Gemma's streaming builder gets a small hybrid source: budgeted pinned
        hot blocks plus disk-backed cold blocks. That prevents full Gemma RAM
        pinning while avoiding pure disk mode.
    ...
    # Tuned after the 19GB compromise feedback:
    # Do NOT tie Gemma's VRAM target to the CPU hot-pin budget. The 22GB tune
    # raised both, which can fill 64GB DDR even when Task Manager does not show
    # Gemma visibly reaching the target VRAM. Keep the requested aggressive Gemma VRAM
    # target, but keep CPU pinned hot blocks small. Cold blocks use the small
    # disk/LRU source instead of full DDR residency.
    try:
        hot_budget_gb = float(os.environ.get("FRAMEVISION_LTX_GEMMA_HOT_PIN_GB", "6.0") or "6.0")
    except Exception:
        hot_budget_gb = 6.0
    try:
        gemma_target_vram_gb = float(os.environ.get("FRAMEVISION_LTX_GEMMA_TARGET_VRAM_GB", "20.5") or "20.5")
    except Exception:
        gemma_target_vram_gb = 20.5
    try:
    ...
    if getattr(original_build, "_framevision_ltx_gemma_partial_stream_gate", False):
        ...
        def guarded_build(self: Any, target_device: Any, dtype: Any, cpu_slots_count: int | None = None, gpu_slots_count: int | None = None, **kwargs: Any) -> Any:
            ...
            max_hot_by_budget = max(1, int(hot_budget_bytes // per_block_bytes)) if hot_budget_bytes > 0 else 1
            hot_count = min(len(available_indices), max_hot_by_budget)
            ...
            if hot_indices:
                ...
                hot_views = allocate_layout_views(hot_memory_layout, device=torch.device("cpu"), pin_memory=True)
            ...
            ctx["ltx_gemma_partial_stream_gate_status"] = (
                f"Gemma streaming targets about {gemma_target_vram_gb:.1f}GB VRAM with {resolved_gpu_slots} GPU slots, "
                f"but CPU hot pinning is capped at {hot_budget_gb:.1f}GB ({len(pinned)} blocks) to avoid DDR fill; "
                f"{disk_slots_count} disk-backed cold slots; main transformer streaming probe={'ON' if allow_main_transformer_streaming else 'OFF'}."
            )
...
def _ltx_find_component_file_override(ctx: Dict[str, Any], component: str) -> Path | None:
    """Find FrameVision's optional small file for LTX Vocoder only.

    Important: this resolver is intentionally vocoder-only for now. A previous
    broad component redirect proved unsafe: the split VAE file matched 0 tensors
    in the native VideoEncoder path and left the module on meta tensors. User
    LoRA, text encoder, video VAE, and audio VAE are not redirected here.
    """
...
def _ltx_component_name_from_model_instance(model: Any) -> str | None:
    ...
    This is intentionally stricter than the configurator classifier. Only
    VocoderWithBWE is currently redirected.
    """
    try:
    ...
    try:
        ...
        if not getattr(original_load_state_dict, "_framevision_ltx_main_direct_load_gate", False):
            def guarded_load_state_dict(model_path: Any, loader: Any, registry: Any, device: Any, model_sd_ops: Any = None) -> Any:
                ...
                if is_main_path and is_cuda and _active_weight_model_is_main_ltx():
                    ...
                    ctx["ltx_main_direct_load_gate_status"] = f"forcing main LTXModel load_state_dict device from CUDA to CPU; active_model={active_model}"
                    ...
                    if echo:
                        try:
                            print(f"[vram-lab-ltx-main-direct] load_state_dict main LTXModel: {device} -> cpu | active_model={active_model} | {_summarise_model_paths(model_path)} | {_cuda_snapshot(torch)}", flush=True)
                        except Exception:
                            pass
                    _record_ltx_main_load_trace(ctx, "FrameVisionMainDirectLoadGate:load_state_dict", f"forced device {device} -> cpu; active_model={active_model}; {_summarise_model_paths(model_path)}", torch, echo)
    ...
    try:
        ...
        if not getattr(original_lmw, "_framevision_ltx_main_load_trace", False):
            def traced_load_model_weights(*args: Any, **kwargs: Any) -> Any:
                ...
                # Final and safe vocoder-only redirect. The builder object is frozen,
                # but this call boundary receives the actual model_path argument
                # that the safetensors loader will open. Do not redirect VAE/audio
                # files here until their key layouts have been proven to match.
                component = _ltx_component_name_from_model_instance(meta_model)
                if component == "Vocoder":
                    override = _ltx_find_component_file_override(ctx, "Vocoder")
                    if override is not None:
                        ...
                        status = f"Vocoder: final load redirected to small component file {override} ({_file_size_summary([replacement])}); original was {original_summary}"
                        ctx["ltx_component_file_vocoder_status"] = "redirected_final_load"
                        ctx["ltx_component_file_vocoder_path"] = replacement
                        ctx["ltx_component_file_vocoder_original"] = original_summary
                        try:
                            events = ctx.setdefault("ltx_component_file_events", [])
                            if isinstance(events, list) and len(events) < 100:
                        ...
                        status = f"Vocoder: no small override file found at final load; using original path(s): {_file_size_summary(_normalise_model_paths(model_path))}"
                        ctx["ltx_component_file_vocoder_status"] = "fallback_original_final_load"
                        try:
                            events = ctx.setdefault("ltx_component_file_events", [])
                            if isinstance(events, list) and len(events) < 100:
                                events.append(status)
...
def _ltx_argv(args: argparse.Namespace) -> List[str]:
    ...
    if str(args.pipeline) in ("two_stages", "two_stages_hq", "a2vid_two_stage"):
        if not bool(getattr(args, "disable_distilled_lora", False)):
...
def main() -> int:
    ...
    ctx: Dict[str, Any] = {
        ...
        "main_transformer_streaming_probe": "ON" if bool(getattr(args, "main_transformer_stream_probe", False)) else "OFF",
        "main_transformer_partial_streaming_installed": "NO",
        "main_transformer_partial_streaming_reason": "disabled" if not bool(getattr(args, "main_transformer_stream_probe", False)) else "not attempted yet",
    ...
    try:
        ...
        if args.vram_lab != "off":
            ...
            main_stream_probe_enabled = bool(getattr(args, "main_transformer_stream_probe", False))
