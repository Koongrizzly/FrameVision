...
def _fv_startup_lazy_load_tools_enabled() -> bool:
    ...
    False = full startup / eager-load supported heavy tools.
    True  = fast startup / lazy-load supported heavy tools.
    """
    try:
...
# Sections
        ...
        _descl.setContentsMargins(0, 0, 0, 0)
        _descl.setSpacing(6)

        if not _fv_startup_lazy_load_tools_enabled():
            # Full startup mode: restore the old eager behavior so Describer is
            # ready immediately after FrameVision opens.
            _desc = None
            try:
            ...
            # Fast startup mode: keep Describer lazy so transformers/torch are
            # not imported during startup unless the user opens this section.
            _desc_state = {"loaded": False, "loading": False}
            _desc_label = QLabel(
                "Describer is not loaded yet.\n"
                "Open this section or click Load Describer to import it."
            )
            try:
                _desc_label.setWordWrap(True)
            except Exception:
                pass
            _desc_btn = QPushButton("Load Describer")
            try:
                _desc_btn.setToolTip("Load the Qwen3 VL Describer tool only when needed.")
            except Exception:
                pass
            _descl.addWidget(_desc_label)
            _descl.addWidget(_desc_btn)

            def _clear_describer_layout():
                try:
                    while _descl.count():
                        item = _descl.takeAt(0)
            ...
            def _load_describer_lazy():
                try:
                    if _desc_state.get("loaded") or _desc_state.get("loading"):
                        return
                    _desc_state["loading"] = True
                    try:
                        _desc_btn.setEnabled(False)
                        _desc_btn.setText("Loading Describer...")
                        _desc_label.setText("Loading Describer...")
                    except Exception:
                        pass

                    def _do_load_describer():
                        try:
                            from helpers.describer import DescriberWidget
                            _desc = DescriberWidget()
                            try:
                            ...
                            try:
                                ...
                                pass
                            _clear_describer_layout()
                            _descl.addWidget(_desc, 1)
                            _desc_state["loaded"] = True
                            try:
                            ...
                            try:
                                _desc_label.setText(
                                    "Describer failed to load.\n\n"
                                    f"{e}"
                                )
                                _desc_btn.setText("Retry Load Describer")
                                _desc_btn.setEnabled(True)
                            except Exception:
                                pass
                        finally:
                            _desc_state["loading"] = False

                    try:
                        QTimer.singleShot(0, _do_load_describer)
                    except Exception:
                        _do_load_describer()
                except Exception:
                    try:
            ...
            try:
                _desc_btn.clicked.connect(_load_describer_lazy)
            except Exception:
                pass
            try:
                sec_describe.toggle.toggled.connect(
                    lambda on: _load_describer_lazy()
                    if bool(on) and bool(getattr(self, "_describer_lazy_ready", False))
                    else None
                )
            except Exception:
                pass
        ...
        # ---- dots.tts Text to Speech ----
        sec_dotstts = None
        if not _optional_hide_is_hidden("dotstts"):
            sec_dotstts = CollapsibleSection("dots.tts Text to Speech", expanded=False)
            _dots_wrap = QWidget(); _dots_l = QVBoxLayout(_dots_wrap); _dots_l.setContentsMargins(0,0,0,0)
            _dots_l.setSpacing(6)
            _dots_widget = None
            try:
                from helpers.dotstts import DotsTtsTab as _DotsTtsTab
                _dots_widget = _DotsTtsTab(parent=self)
            except Exception:
                _dots_widget = None
            if _dots_widget is not None:
                try:
                    _dots_widget.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Expanding)
                except Exception:
                    pass
                try:
                    _dots_widget.setMinimumHeight(720)
                except Exception:
                    pass
                _dots_l.addWidget(_dots_widget, 1)
            else:
                _fallback = QLabel("dots.tts tool is unavailable (missing helpers/dotstts.py or its UI dependencies).")
                _fallback.setWordWrap(True)
                _dots_l.addWidget(_fallback)
            sec_dotstts.setContentLayout(_dots_l)
            try:
                sec_dotstts.content.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Expanding)
            except Exception:
                pass
...
# ---- Meme / Caption ----
        ...
        # ---- Image FX Lab ----
        sec_image_fx_lab = CollapsibleSection("Image FX Lab", expanded=False)
        _ifx_wrap = QWidget()
        _ifx_layout = QVBoxLayout(_ifx_wrap)
        _ifx_layout.setContentsMargins(0, 0, 0, 0)
        _ifx_layout.setSpacing(6)
        try:
            try:
                from helpers.image_fx_lab import create_image_fx_lab_widget as _create_image_fx_lab_widget
                _ifx_widget = _create_image_fx_lab_widget(parent=self)
            except Exception:
                from helpers.image_fx_lab import ImageFxLab as _ImageFxLab
                _ifx_widget = _ImageFxLab(parent=self, embedded=True)
            try:
                _ifx_widget.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Expanding)
            except Exception:
                pass
            try:
                _ifx_widget.setMinimumHeight(760)
                _ifx_widget.setProperty("expand_min_h", 760)
            except Exception:
                pass
        except Exception:
            _ifx_widget = QLabel("Image FX Lab failed to load.")
            try:
                _ifx_widget.setWordWrap(True)
            except Exception:
                pass
        _ifx_layout.addWidget(_ifx_widget, 1)
        sec_image_fx_lab.setContentLayout(_ifx_layout)
        try:
            sec_image_fx_lab.content.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Expanding)
            sec_image_fx_lab.content.setProperty("expand_min_h", 760)
        except Exception:
            pass
...
# populate menu
        ...
        try:
            self._describer_lazy_ready = True
        except Exception:
            pass
    ...
    def _run(self, cmd_list, outfile):
        ...
        try:
            ...
            try:
                ...
                pass
            # Do NOT switch to the Queue tab after enqueue.
            # Queueing should be silent/non-stealing: keep the user on the current tool/model tab.
            # The queue pane will refresh from its normal timer when opened.
        except Exception as e:
            try:
