...
def sdxl_inpaint_python_exe() -> Optional[Path]:
    """Return the python executable for the dedicated SDXL-inpaint environment.

    New layout: <project>/environments/.sdxl_inpaint/
    Old root-level .sdxl_inpaint is kept only as a migration fallback.
    """
    root = project_root()
    candidates = [
        # Conda prefix env (Windows)
        root / "environments" / ".sdxl_inpaint" / "python.exe",
        # venv-style fallback if a user-created env exists there
        root / "environments" / ".sdxl_inpaint" / "Scripts" / "python.exe",
        # POSIX fallback
        root / "environments" / ".sdxl_inpaint" / "bin" / "python",
        # Legacy root-level env fallbacks
        root / ".sdxl_inpaint" / "Scripts" / "python.exe",
        root / ".sdxl_inpaint" / "bin" / "python",
    ]
...
def _ensure_sdxl_runner(tmp_dir: Path) -> Path:
    ...
    This script is executed under the dedicated environments/.sdxl_inpaint environment.
    """
...
class InpaintWorker(QObject):
    ...
    def run(self):
        try:
            ...
            if py is None:
                raise RuntimeError(
                    "SDXL-Inpaint environment not found. Expected: <project>/environments/.sdxl_inpaint/.\n"
