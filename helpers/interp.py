...
class InterpPane(QWidget):
    ...
    def _build_ui(self):
        ...
        topbar = QWidget()
        topbar_l = QHBoxLayout(topbar); topbar_l.setContentsMargins(0,0,0,0); topbar_l.setSpacing(8)

        self.btn_start = QPushButton("Add to Queue")
        self.btn_batch = QPushButton("Add Batch")
        self.btn_open_folder = QPushButton("View results")

        # Larger font on action buttons (+3px) and green hover on "Add to Queue"
        ...
        topbar_l.addWidget(self.btn_start); topbar_l.addWidget(self.btn_batch); topbar_l.addWidget(self.btn_open_folder); topbar_l.addStretch(1)
 ...
 #      )
  #     root.addWidget(self.banner)
   #    root.addSpacing(4)
        ...
        self.btn_open_folder.setToolTip("View results in Media Explorer.")
        ...
        self.pb_cancel.clicked.connect(self._on_cancel_clicked)
    ...
    def _on_cancel_clicked(self):
        # Connected once during UI build; avoids repeated connect/disconnect warnings.
...
# ---- model handling ----
    ...
    def _on_open_folder(self):
        folder = str(_outputs_dir(self.ROOT))
        # Prefer Media Explorer tab if available
        try:
            w = self.window()
            if w is not None and hasattr(w, 'open_media_explorer_folder'):
                try:
                    w.open_media_explorer_folder(folder)
                    return
                except Exception:
                    pass
        except Exception:
