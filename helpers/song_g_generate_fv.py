...
def _ensure_ffmpeg_in_path():
    """Ensure ffmpeg/ffprobe are discoverable for Demucs (used for prompt audio separation).

    FrameVision ships ffmpeg in <root>/presets/bin on Windows. Demucs calls ffprobe via subprocess,
    which will fail with WinError 2 if that folder isn't on PATH.
    """
    try:
        ...
        candidates = [
            ...
            os.path.join(root, "Preset", "bin"),
            os.path.join(root, "bin"),
        ]

        path_env = os.environ.get("PATH", "")
        path_parts = [p.strip() for p in path_env.split(os.pathsep) if p.strip()]
        ...
        for cand in candidates:
            ...
            if os.path.exists(ffmpeg_exe) or os.path.exists(ffprobe_exe):
                if cand.lower() not in lower_parts:
                    os.environ["PATH"] = cand + os.pathsep + path_env
                    print(f"[SongG] Added ffmpeg folder to PATH for Demucs: {cand}")
                return
    except Exception as e:
        # Never crash on PATH setup; just continue and let downstream error report.
        print(f"[SongG] ffmpeg PATH setup skipped: {e}")
...
def parse_args():
    ...
    parser.add_argument('--cfg_coef', type=float, default=1.5, help='CFG coefficient (default: 1.5)')
    ...
    parser.add_argument('--top_p', type=float, default=0.0, help='Top-p nucleus sampling (default: 0.0)')
...
def generate(args):
    ...
    cfg_coef = float(getattr(args, 'cfg_coef', 1.5))
    temp = float(getattr(args, 'temperature', 0.9))
...
if __name__ == "__main__":
    ...
    seed = args.seed if getattr(args, 'seed', None) is not None else int(time.time())
    ...
    # --- FlashAttention guard (Windows / missing dependency) ---
    if getattr(args, "use_flash_attn", False):
        try:
            # FlashAttention is typically not supported on Windows wheels; avoid hard-crashing.
            if sys.platform.startswith("win"):
                print("FlashAttention requested but disabled on Windows; falling back to standard attention.")
                args.use_flash_attn = False
            else:
                from flash_attn.flash_attn_interface import flash_attn_func as _flash_attn_func  # type: ignore
                # Some local LLaMA forks reference a global `flash_attn_func` without defining it.
                try:
                    import importlib
                    _mdl = importlib.import_module("models.song_generation.codeclm.models.llama.modeling_llama")
                    setattr(_mdl, "flash_attn_func", _flash_attn_func)
                except Exception:
                    pass
        except Exception as _e:
            print(f"FlashAttention requested but not available ({_e}); disabling.")
            args.use_flash_attn = False
