...
class HiDreamUI(QMainWindow):
    ...
    def _make_settings_tab(self) -> QWidget:
        ...
        self.environment_info_label = QLabel(self.environment_info_text())
        ...
        behavior_form.addRow("Environment", self.environment_info_label)
    ...
    def apply_model_settings_to_ui(self, key: str) -> None:
        ...
        if "edit" in self._gen_widget_sets:
            self.apply_generation_settings_to_widgets("edit", settings)
        if "multi" in self._gen_widget_sets:
            self.apply_generation_settings_to_widgets("multi", settings)
    ...
    def on_generation_controls_value_changed(self, source: str) -> None:
        ...
        try:
            ...
            for other in self._gen_widget_sets:
                if other != source:
                    self.apply_generation_settings_to_widgets(other, merged)
        finally:
            self._syncing_generation_controls = False
    ...
    def update_model_status(self) -> None:
        ...
        if installed:
            ...
            self.model_status.setText("Missing — run install.bat again and select this model")
    ...
    def start_process(self, args: list[str], expected_output: Path, job: dict | None = None) -> None:
        if self.process and self.process.state() != QProcess.NotRunning:
            ...
            return
        if not ENV_PY or not ENV_PY.exists() or not RUNNER or not RUNNER.exists():
            QMessageBox.critical(self, "Missing install", "The environment or shared runner was not found. Run install.bat first.")
            return
        self.save_settings()
