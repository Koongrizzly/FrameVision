...
Requirements (handled by presets/extra_env/zimage_install.py):
  * Python runtime is the shared image-model env: <root>/environments/.images_models/
...
def _run_gguf(args) -> dict:
    ...
    candidates = []
    try:
        env_cli = os.environ.get("SD_CLI_PATH") or os.environ.get("SD_CLI") or ""
        if env_cli:
    ...
    def _resolve_gguf_path(value: str, fallback: Path | None = None) -> Path | None:
        try:
            ...
            if not p.is_absolute():
                p = (gguf_dir / p).resolve()
            return p
        except Exception:
            return fallback

    # Preferred explicit GGUF selections. The txt2img GGUF section passes these
    # through for selective edit so it uses the exact model/LLM/VAE already selected there.
    try:
        diffusion = _resolve_gguf_path(getattr(args, "gguf_model", ""), diffusion) or diffusion
    except Exception:
        pass
    try:
        llm = _resolve_gguf_path(getattr(args, "gguf_instruct", ""), llm) or llm
    except Exception:
        pass
    try:
        vae = _resolve_gguf_path(getattr(args, "gguf_vae", ""), vae) or vae
    except Exception:
        pass

    # Backwards compatible transport for older txt2img jobs: lora = diffusion, lora2 = VAE.
    try:
        if getattr(args, "lora", "") and str(getattr(args, "lora")).lower().endswith(".gguf"):
            p = _resolve_gguf_path(str(getattr(args, "lora")), diffusion)
            if p is not None:
                diffusion = p
    except Exception:
        pass
    try:
        if getattr(args, "lora2", "") and str(getattr(args, "lora2")).lower().endswith((".safetensors", ".pt", ".sft")):
            p = _resolve_gguf_path(str(getattr(args, "lora2")), vae)
            if p is not None:
                vae = p
    except Exception:
        pass

    # Backwards compatible env transport for instruct/LLM path.
    try:
        env_llm = os.environ.get("ZIMAGE_GGUF_INSTRUCT_PATH") or os.environ.get("ZIMAGE_LLM_PATH") or os.environ.get("LLM_PATH") or ""
        if env_llm and not str(getattr(args, "gguf_instruct", "") or "").strip():
            p = _resolve_gguf_path(env_llm, llm)
            if p is not None:
                llm = p
    except Exception:
        pass
    ...
    for idx in range(batch):
        ...
        # Optional img2img / selective edit. Current sd-cli keeps init-image/mask under
        # the normal img_gen mode, so do NOT pass a fake --mode img2img value.
        try:
            init_img = str(getattr(args, "init_image", "") or "").strip()
        except Exception:
            init_img = ""
        try:
            mask_img = str(getattr(args, "mask_image", "") or "").strip()
        except Exception:
            mask_img = ""
        if init_img:
            try:
            ...
            try:
                ...
                strength = 0.35
            cmd += ["--init-img", init_img, "--strength", str(strength)]
            if mask_img:
                cmd += ["--mask", mask_img]
...
def main(argv=None) -> int:
    ...
    # Optional img2img / selective edit: pass an init image, optional mask, and strength
    parser.add_argument("--init-image", dest="init_image", default="")
    parser.add_argument("--mask-image", dest="mask_image", default="")
    ...
    # Explicit GGUF selections used by the Z-Image selective-edit tab.
    parser.add_argument("--gguf-model", dest="gguf_model", default="")
    parser.add_argument("--gguf-instruct", dest="gguf_instruct", default="")
    parser.add_argument("--gguf-vae", dest="gguf_vae", default="")
