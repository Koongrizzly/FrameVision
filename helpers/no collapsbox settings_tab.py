...
def _options_group(page: QWidget) -> QGroupBox:
    ...
    # -- Change tab location (reorder tabs) -------------------------------------
    ...
    # Place the reset button on the same row as the toggle (no full-width stretching)
    btn_reset_tabs.setSizePolicy(QSizePolicy(QSizePolicy.Fixed, QSizePolicy.Fixed))

    row_tabloc = QWidget(g)
    h_tabloc = QHBoxLayout(row_tabloc); h_tabloc.setContentsMargins(0,0,0,0); h_tabloc.setSpacing(12)
    h_tabloc.addWidget(cb_tabloc)
    h_tabloc.addStretch(1)
    h_tabloc.addWidget(btn_reset_tabs)
    v.addWidget(row_tabloc)

    # Apply persisted tab reorder immediately
    try:
