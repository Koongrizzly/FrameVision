...
# WanGP model key for the current LTX 2.3 22B distilled 1.1 default.
# The older ltx2_22B_distilled key points at the previous distilled model and
# can trigger a fresh download if that older model has been removed.
DEFAULT_LTX_MODEL_TYPE = "ltx2_22B_distilled_1_1"
...
def _wangp_ltx_lora_dir(args: argparse.Namespace) -> str:
    """WanGP resolves activated_loras by basename inside the model LoRA folder.
    For LTX2 this is normally <WanGP>/loras/ltx2.
    """
    root = _norm(getattr(args, "wangp_root", ""))
    lora_dir = os.path.join(root, "loras", "ltx2") if root else ""
    if lora_dir:
        Path(lora_dir).mkdir(parents=True, exist_ok=True)
    return lora_dir


def _prepare_lora_for_wangp(args: argparse.Namespace, lora_path: str) -> Tuple[str, str]:
    """Return (basename_for_activated_loras, full_path_used).

    WanGP's queue settings expect activated_loras to contain filenames that exist in the
    model LoRA folder. A Planner browse path can be anywhere, so copy it into WanGP's LTX
    LoRA folder when needed.
    """
    src = _norm(lora_path)
    if not src or not os.path.isfile(src):
        raise FileNotFoundError(f"LoRA file not found: {lora_path}")
    lora_dir = _wangp_ltx_lora_dir(args)
    if not lora_dir:
        return os.path.basename(src), src
    dst = os.path.join(lora_dir, os.path.basename(src))
    try:
        same = os.path.abspath(src).lower() == os.path.abspath(dst).lower()
    except Exception:
        same = False
    if not same:
        try:
            ...
            raise RuntimeError(f"Could not copy LoRA into WanGP LoRA folder: {src} -> {dst}: {e}") from e
    return os.path.basename(dst), dst


def _append_lora_activation(args: argparse.Namespace,
                            activated: List[str],
                            multipliers: List[float],
                            full_paths: List[str],
                            lora_path: str,
                            multiplier: float) -> None:
    basename, used_path = _prepare_lora_for_wangp(args, lora_path)
    activated.append(basename)
    try:
        mult = float(multiplier)
    except Exception:
        mult = 1.0
    multipliers.append(max(0.5, min(1.5, mult)))
    full_paths.append(used_path)
...
def _build_settings_payload(args: argparse.Namespace) -> Dict[str, Any]:
    ...
    settings["model_type"] = str(args.model_type or DEFAULT_LTX_MODEL_TYPE).strip()
    ...
    activated_loras: List[str] = []
    lora_multipliers: List[float] = []
    lora_full_paths: List[str] = []

    lora_file_path = _norm(args.lora_file)
    if lora_file_path:
        _append_lora_activation(args, activated_loras, lora_multipliers, lora_full_paths, lora_file_path, float(args.lora_multiplier))
        settings["ltx_transition_lora_file"] = lora_full_paths[-1]

    extra_lora_files = list(getattr(args, "extra_lora_file", None) or [])
    extra_lora_mults = list(getattr(args, "extra_lora_multiplier", None) or [])
    for idx, extra_path in enumerate(extra_lora_files):
        try:
            extra_mult = float(extra_lora_mults[idx]) if idx < len(extra_lora_mults) else 1.0
        except Exception:
            extra_mult = 1.0
        _append_lora_activation(args, activated_loras, lora_multipliers, lora_full_paths, extra_path, extra_mult)

    if activated_loras:
        settings["activated_loras"] = activated_loras
        settings["loras_multipliers"] = " ".join([f"{float(x):.4g}" for x in lora_multipliers])
        settings["ltx_lora_files"] = lora_full_paths
    ...
    if audio_path:
        ...
        # LTX lipsync/audio-conditioning path: pass the split WAV as audio_guide only.
        # Do not use audio_source or MMAudio here; final master audio is handled later.
...
def _make_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        prog="ltx23_cli.py",
        description="Private bridge CLI: run WanGP LTX 2.3 22B distilled 1.1 from FrameVision/planner.",
    )
    ...
    g.add_argument("--lora-file", default="", help="Optional transition LoRA safetensors to activate for this run")
    ...
    g.add_argument("--lora-multiplier", type=float, default=1.0, help="Multiplier for --lora-file")
    g.add_argument("--extra-lora-file", action="append", default=[], help="Optional extra LoRA safetensors. May be passed multiple times.")
    g.add_argument("--extra-lora-multiplier", action="append", type=float, default=[], help="Multiplier for the matching --extra-lora-file. May be passed multiple times.")
    g.add_argument("--model-type", default=DEFAULT_LTX_MODEL_TYPE, help=f"WanGP model type key (default: {DEFAULT_LTX_MODEL_TYPE})")
