...
# Flux Klein and FireRed can stay lazy for faster FrameVision startup.
# HiDream is intentionally eager-loaded with Qwen Edit because its embedded
# lazy placeholder path was unreliable inside FrameVision.



def _remove_hide_entry_hidden(*entry_ids: str) -> bool:
    """Return True when a remove/hide optional entry is hidden.

    Kept local and defensive so qwen2511.py can still run standalone when
    helpers/remove_hide.py is missing or broken.
    """
    try:
        try:
        ...
        state = _rh.load_state(_rh.get_state_path())
        hidden = set(str(x) for x in (state.get("hidden_ids") or []))
        return any(str(entry_id) in hidden for entry_id in entry_ids)
    except Exception:
        return False


def _firered_hidden_by_remove_hide() -> bool:
    # Main current id + a tiny legacy/fallback alias for older state files.
    return _remove_hide_entry_hidden("firered_11_gguf", "firered")


def _flux_klein_hidden_by_remove_hide() -> bool:
    """Return True when the shared Flux Klein tab should be hidden.

    Remove/Hide tracks 4B and 9B as separate entries, but qwen2511.py embeds
    one shared Flux Klein UI tab.  Therefore the outer tab is only removed when
    both model entries are hidden, or when a future all-in-one Flux alias exists.
    """
    if _remove_hide_entry_hidden("flux_klein", "flux_klein_all"):
        return True
    return (
        _remove_hide_entry_hidden("flux_klein_4b", "klein4b", "flux4b")
        and _remove_hide_entry_hidden("flux_klein_9b", "klein9b", "flux9b")
    )


def _hidream_hidden_by_remove_hide() -> bool:
    """Return True when HiDream should not be added to the embedded tabs.

    The remove/hide manager may use one all-in-one HiDream id or model-specific
    ids, depending on the FrameVision build.  Support both so older state files
    and future split entries do not leave an error placeholder behind.
    """
    return _remove_hide_entry_hidden(
        ...
        "hidream_edit_base",
        "hidream_edit_dev",
        "hidream_edit_dev_2604_bf16",
        "hidream_edit_both",
        "hidream_edit_base_fp8",
        "hidream_edit_dev_fp8",
        "hidream_edit_both_fp8",
        "hidream_edit_all",
    )
...
def _load_firered_pane_class():
    try:
        ...
        try:
            from firered import FireRedPane  # type: ignore
            return FireRedPane
        except Exception:
            return None


_LAST_HIDREAM_IMPORT_ERROR = ""

def _load_hidream_ui_class():
    global _LAST_HIDREAM_IMPORT_ERROR
    _LAST_HIDREAM_IMPORT_ERROR = ""
    errors = []
    try:
        from helpers.hidream_ui import HiDreamUI  # type: ignore
        return HiDreamUI
    except Exception as exc:
        errors.append("helpers.hidream_ui: " + "".join(traceback.format_exception_only(type(exc), exc)).strip())
    try:
        from hidream_ui import HiDreamUI  # type: ignore
        return HiDreamUI
    except Exception as exc:
        errors.append("hidream_ui: " + "".join(traceback.format_exception_only(type(exc), exc)).strip())
    _LAST_HIDREAM_IMPORT_ERROR = "\n".join(errors)
    return None


def _fv_startup_lazy_load_tools_enabled() -> bool:
    """Return Settings > Fast startup / load some tools when opened.

    False = full startup / eager-load supported optional pages.
    True  = fast startup / keep supported optional pages lazy.
    """
    try:
        return bool(QtCore.QSettings("FrameVision", "FrameVision").value("startup_lazy_load_tools", False, type=bool))
    except Exception:
        pass
    try:
...
# -----------------------------
# Headless entrypoint for Planner automation
# -----------------------------
def generate_one_from_job(job: Dict[str, Any], out_dir: str) -> Dict[str, Any]:
    """
    Generate a single image using sd-cli with Qwen Edit 2511 settings.
    Designed for the Planner automation path (Chunk 4 Part 1).

    Expected job keys (best-effort):
      - prompt (str)
      - negative_prompt (str)
      - seed (int)
      - width (int)
      - height (int)
      - ref_images (List[str])  # up to 2
      - out_file (str)          # optional explicit output file
      - lora_model_dir (str)    # optional
      - lora_name (str)         # optional filename
      - lora_strength (float)   # optional
    """
    _ensure_dir(out_dir)

    settings = _read_jsonish(SETSAVE_PATH)
    if not isinstance(settings, dict):
        settings = {}

    # Apply defaults consistent with the pane
    d = default_model_paths()
    settings.setdefault("sdcli_path", DEFAULT_SDCLI)
    settings.setdefault("unet_path", d.get("unet") or "")
    settings.setdefault("llm_path", d.get("llm") or "")
    settings.setdefault("mmproj_path", d.get("mmproj") or "")
    settings.setdefault("vae_path", d.get("vae") or "")
    settings.setdefault("steps", 25)
    settings.setdefault("cfg", 2.35)
    settings.setdefault("strength", 0.9)
    settings.setdefault("seed", -1)
    settings.setdefault("width", 1024)
    settings.setdefault("height", 576)
    settings.setdefault("sampling_method", "Euler")
    settings.setdefault("shift", 1.0)
    settings.setdefault("ref_increase_index", True)
    settings.setdefault("ref_disable_auto_resize", False)

    # Low-vram defaults
    settings.setdefault("use_vae_tiling", False)
    settings.setdefault("vae_tile_size", "")
    settings.setdefault("vae_tile_overlap", 0.0)
    settings.setdefault("use_offload", False)
    settings.setdefault("use_mmap", False)
    settings.setdefault("use_vae_on_cpu", False)
    settings.setdefault("use_clip_on_cpu", False)
    settings.setdefault("use_diffusion_fa", False)

    # LoRA defaults
    settings.setdefault("lora_model_dir", "")
    settings.setdefault("lora_name", "")
    settings.setdefault("lora_strength", 1.0)

    # Override with job values
    prompt = str(job.get("prompt") or "")
    negative = str(job.get("negative_prompt") or job.get("negative") or "")
    try:
        seed = int(job.get("seed")) if job.get("seed") is not None else int(settings.get("seed") or -1)
    except Exception:
        seed = int(settings.get("seed") or -1)

    try:
        width = int(job.get("width") or settings.get("width") or 1024)
        height = int(job.get("height") or settings.get("height") or 576)
    except Exception:
        width, height = 1024, 576

    # Output file
    out_file = str(job.get("out_file") or "").strip()
    if not out_file:
        out_file = os.path.join(out_dir, f"qwen2511_{int(time.time())}.png")

    # Init image: for pure ref-driven generations we use a blank canvas.
    tmp_blank = os.path.join(out_dir, f"_blank_{int(time.time()*1000)}.png")
    try:
        _write_blank_png(tmp_blank, width, height)
    except Exception:
        tmp_blank = ""
    ...
    try:
        ...
        refs = []
    refs = [p for p in refs if p][:2]

    # LoRA override
    lora_model_dir = str(job.get("lora_model_dir") or settings.get("lora_model_dir") or "")
    lora_name = str(job.get("lora_name") or settings.get("lora_name") or "")
    try:
        lora_strength = float(job.get("lora_strength") if job.get("lora_strength") is not None else settings.get("lora_strength") or 1.0)
    except Exception:
        lora_strength = 1.0

    sdcli_path = str(settings.get("sdcli_path") or DEFAULT_SDCLI)
    ...
    cmd = build_sdcli_cmd(
        ...
        caps=caps,
        init_img=tmp_blank,
        mask_path=str(job.get("mask_path") or ""),
        ref_images=refs,
        use_increase_ref_index=bool(settings.get("ref_increase_index")),
        disable_auto_resize_ref_images=bool(settings.get("ref_disable_auto_resize")),
        prompt=prompt,
        negative=negative,
        ...
        width=width,
        height=height,
        strength=float(settings.get("strength") or 0.9),
        sampling_method=str(settings.get("sampling_method") or "Euler"),
        shift=float(settings.get("shift") or 1.0),
        out_file=out_file,
        use_vae_tiling=bool(settings.get("use_vae_tiling")),
        vae_tile_size=str(settings.get("vae_tile_size") or ""),
        vae_tile_overlap=float(settings.get("vae_tile_overlap") or 0.0),
        use_offload=bool(settings.get("use_offload")),
        use_mmap=bool(settings.get("use_mmap")),
        use_vae_on_cpu=bool(settings.get("use_vae_on_cpu")),
        use_clip_on_cpu=bool(settings.get("use_clip_on_cpu")),
        use_diffusion_fa=bool(settings.get("use_diffusion_fa")),
    ...
    ok = False
    try:
        rc, _out_text = _run_capture(cmd)
        ok = (rc == 0 and os.path.exists(out_file))
    except Exception:
        ok = False
    ...
    if not ok:
        return {"ok": False, "files": [], "out_file": out_file}

    return {"ok": True, "files": [out_file], "out_file": out_file}
...
class Qwen2511Pane(QtWidgets.QWidget):
    def __init__(self, parent=None):
        ...
        try:
            ...
            self.main = None
        # Do not run sd-cli --help during FrameVision startup.  Capability
        # detection is refreshed when the user picks sd-cli or starts a run.
        self.caps: SdCliCaps = SdCliCaps()
        self._caps_detection_deferred = True
        ...
        # becomes selected it must reliably build itself again.  Earlier restore
        # suppression could leave Flux/Firered stuck forever on a
        # placeholder, so selection now counts as the load trigger.
        self._lazy_startup_suppressed = False
        self._lazy_user_requested_index = None
        self._lazy_user_request_armed = False
        self._lazy_deferred_auto_load_scheduled = False
        self._lazy_deferred_auto_load_done = False
        ...
        try:
            QtCore.QTimer.singleShot(0, self._arm_lazy_tab_loading)
            QtCore.QTimer.singleShot(250, self._lazy_current_tab_watchdog)
            QtCore.QTimer.singleShot(1000, self._lazy_current_tab_watchdog)
        except Exception:
            self._lazy_startup_suppressed = False
        ...
        self._append_log("sd-cli capability detection deferred until Run or sd-cli path selection.\n")
        self._append_log(
...
#Y        self.tabs.setToolTip("Switch between Qwen Edit 2511, Flux Klein, HiDream Image Edit and FireRed 1.1.")
        ...
        # delayed placeholder path can leave it stuck on "not loaded yet".
        if not _flux_klein_hidden_by_remove_hide():
            self.tabs.addTab(
                self._make_lazy_placeholder(
                    "Flux Klein",
                    "Flux Klein is not loaded yet. It will import when this tab is opened.",
                ),
                "Flux Klein",
            )
        if not _hidream_hidden_by_remove_hide():
            try:
                _hidream_page = self._build_hidream_tab()
                self.tabs.addTab(_hidream_page, "HiDream Image Edit")
                self._lazy_tab_loaded.add("HiDream Image Edit")
            except Exception as exc:
                detail = "".join(traceback.format_exception_only(type(exc), exc)).strip()
                self.tabs.addTab(
                    self._make_tool_error_page(
                        "HiDream Image Edit",
                        "HiDream Image Edit failed to load during startup.\n\nReason:\n" + detail,
                    ),
                    "HiDream Image Edit",
                )
        if not _firered_hidden_by_remove_hide():
            self.tabs.addTab(
                self._make_lazy_placeholder(
                    "Firered 1.1",
                    "Firered 1.1 is not loaded yet. It will import when this tab is opened.",
                ),
                "Firered 1.1",
            )
    ...
    def _find_tab_index_by_title(self, title: str) -> int:
        try:
            ...
            for i in range(self.tabs.count()):
                if str(self.tabs.tabText(i)).strip() == str(title).strip():
                    return i
        except Exception:
            pass
        return -1

    def _remove_optional_tab(self, title: str) -> None:
        try:
            ...
            try:
                ...
                pass
            try:
                self._lazy_tab_loaded.discard(title)
            except Exception:
                pass
            if title == "HiDream Image Edit":
                self.hidream_pane = None
            elif title == "Firered 1.1":
                self.fire_red_pane = None
        except Exception:
            pass

    def _insert_optional_tab(self, title: str, widget: QtWidgets.QWidget) -> None:
        try:
            ...
            for i in range(self.tabs.count()):
                existing = str(self.tabs.tabText(i)).strip()
                if existing in order and order.index(existing) > wanted_pos:
                    insert_at = i
                    break
    ...
    def refresh_hidden_tools(self) -> None:
        ...
        try:
            ...
            if flux_hidden:
                self._remove_optional_tab("Flux Klein")
            if hidream_hidden:
                self._remove_optional_tab("HiDream Image Edit")
            if firered_hidden:
                self._remove_optional_tab("Firered 1.1")

            # Add newly unhidden tabs in the original order.
            if (not flux_hidden) and self._find_tab_index_by_title("Flux Klein") < 0:
                self._insert_optional_tab(
                    "Flux Klein",
                    self._make_lazy_placeholder(
                        "Flux Klein",
                        "Flux Klein is not loaded yet. It will import when this tab is opened.",
            ...
            if (not hidream_hidden) and self._find_tab_index_by_title("HiDream Image Edit") < 0:
                try:
                    page = self._build_hidream_tab()
                    self._insert_optional_tab("HiDream Image Edit", page)
                    self._lazy_tab_loaded.add("HiDream Image Edit")
                except Exception as exc:
                    detail = "".join(traceback.format_exception_only(type(exc), exc)).strip()
                    self._insert_optional_tab(
                        ...
                        self._make_tool_error_page(
                            "HiDream Image Edit",
                            "HiDream Image Edit failed to load.\n\nReason:\n" + detail,
                        ),
                    )
            if (not firered_hidden) and self._find_tab_index_by_title("Firered 1.1") < 0:
                self._insert_optional_tab(
                    "Firered 1.1",
                    self._make_lazy_placeholder(
                        "Firered 1.1",
            ...
            try:
                self._last_hidden_optional_tabs_state = self._hidden_optional_tabs_state()
            except Exception:
                pass
            try:
                self._on_tab_changed(self.tabs.currentIndex())
    ...
    def _install_hidden_tools_refresh_timer(self) -> None:
        """Poll remove/hide JSON so tab hide/unhide updates live."""
        try:
            self._last_hidden_optional_tabs_state = self._hidden_optional_tabs_state()
        except Exception:
            self._last_hidden_optional_tabs_state = None
        try:
            self._hidden_tools_timer = QtCore.QTimer(self)
    ...
    def _check_hidden_tools_refresh(self) -> None:
        try:
            state = self._hidden_optional_tabs_state()
            if state != getattr(self, "_last_hidden_optional_tabs_state", None):
                self.refresh_hidden_tools()
        except Exception:
            pass

    def _arm_lazy_tab_loading(self) -> None:
        """Compatibility hook for older startup lazy-tab scheduling."""
        try:
            self._lazy_startup_suppressed = False
        except Exception:
            pass
    ...
    def _schedule_deferred_lazy_tab_autoload(self) -> None:
        """After the pane becomes visible, load a restored lazy tab once.

        State restore may select Flux/Firered programmatically during
        startup.  That must not build heavy tools inside restore_all(), but a
        visible restored placeholder should not look broken either.
        """
        try:
            if bool(getattr(self, "_lazy_deferred_auto_load_done", False)):
                return
            if bool(getattr(self, "_lazy_deferred_auto_load_scheduled", False)):
                return
            # If startup restored a lazy tab as the current page, make the
            # placeholder honest immediately instead of showing the generic
            # "open this tab" text while the delayed import waits.
            try:
                if hasattr(self, "tabs") and self.tabs is not None:
                    ...
                    if title in {"Flux Klein", "Firered 1.1"} and title not in getattr(self, "_lazy_tab_loaded", set()):
                        self._set_lazy_placeholder_message(
                            idx,
                            title,
                            f"{title} was selected last time.\nFrameVision will import it shortly...",
                        )
            except Exception:
                pass
            self._lazy_deferred_auto_load_scheduled = True
            QtCore.QTimer.singleShot(750, self._deferred_auto_load_current_lazy_tab)
        except Exception:
            try:
                self._lazy_deferred_auto_load_scheduled = False
            except Exception:
    ...
    def _deferred_auto_load_current_lazy_tab(self) -> None:
        """Load the currently visible restored lazy tab once, after startup."""
        try:
            self._lazy_deferred_auto_load_scheduled = False
            if bool(getattr(self, "_lazy_deferred_auto_load_done", False)):
                return
            if getattr(self, "_lazy_startup_suppressed", True):
                # Startup guard has not armed yet; try once more shortly.
                self._lazy_deferred_auto_load_scheduled = True
                QtCore.QTimer.singleShot(750, self._deferred_auto_load_current_lazy_tab)
                return
            if not hasattr(self, "tabs") or self.tabs is None:
            ...
            if title in getattr(self, "_lazy_tab_loaded", set()):
                ...
                return
            self._lazy_deferred_auto_load_done = True
            self._set_lazy_placeholder_message(idx, title, f"Loading {title}...")
            self._ensure_lazy_tab_loaded(idx, user_requested=True)
        except Exception:
            try:
                self._lazy_deferred_auto_load_done = True
            except Exception:
    ...
    def _mark_lazy_user_tab_request(self, idx: int) -> None:
        """Remember that a tab change came from a user action, not state restore."""
        try:
            self._lazy_user_requested_index = int(idx)
            self._lazy_user_request_armed = True
        except Exception:
            self._lazy_user_requested_index = None
            self._lazy_user_request_armed = False

    def _clear_lazy_user_tab_request(self) -> None:
        try:
            self._lazy_user_requested_index = None
            self._lazy_user_request_armed = False
        except Exception:
    ...
    def _lazy_tab_change_is_user_requested(self, idx: int) -> bool:
        # Kept for compatibility with older code paths.  Selecting a lazy tab is
        # enough to load it; do not require a special mouse/user marker.
        try:
            return True
        except Exception:
            return True

    def _load_lazy_tab_from_user_request(self, idx: int) -> None:
        """Load a lazy tab after an explicit user click, including already-selected tabs."""
        try:
            self._mark_lazy_user_tab_request(int(idx))
            self._ensure_lazy_tab_loaded(int(idx), user_requested=True)
        except Exception:
            pass
        finally:
            self._clear_lazy_user_tab_request()

    def eventFilter(self, obj, event):
        try:
            if bool(getattr(obj, "property", lambda *_: False)("_fv_lazy_placeholder")):
                try:
                    if event.type() in (QtCore.QEvent.Show, QtCore.QEvent.MouseButtonPress, QtCore.QEvent.MouseButtonRelease):
                        QtCore.QTimer.singleShot(0, self._lazy_current_tab_watchdog)
                except Exception:
                    pass
            if hasattr(self, "tabs") and self.tabs is not None and obj is self.tabs.tabBar():
                et = event.type()
                if et == QtCore.QEvent.MouseButtonPress:
                    ...
                    try:
                        if event.button() == QtCore.Qt.LeftButton:
                            idx = self.tabs.tabBar().tabAt(event.pos())
                            if idx >= 0:
                                QtCore.QTimer.singleShot(0, lambda idx=int(idx): self._load_lazy_tab_from_user_request(idx))
                    except Exception:
                        pass
        except Exception:
            pass
    ...
    def _make_lazy_placeholder(self, title: str, message: str) -> QtWidgets.QWidget:
        page = QtWidgets.QWidget(self)
        try:
            page.setProperty("_fv_lazy_placeholder", True)
            page.setProperty("_fv_lazy_title", str(title))
        ...
        try:
            ...
            pass
        msg = QtWidgets.QLabel(str(message))
        msg.setWordWrap(True)
        msg.setTextInteractionFlags(QtCore.Qt.TextSelectableByMouse)
        try:
            page._fv_lazy_message_label = msg
        except Exception:
            pass
    ...
    def _set_lazy_placeholder_message(self, idx: int, title: str, message: str) -> None:
        try:
            if not hasattr(self, "tabs") or self.tabs is None:
                return
            ...
            if page is None or not bool(page.property("_fv_lazy_placeholder")):
                return
            lbl = getattr(page, "_fv_lazy_message_label", None)
            if lbl is not None:
    ...
    def _make_tool_error_page(self, title: str, message: str) -> QtWidgets.QWidget:
        page = QtWidgets.QWidget(self)
        lay = QtWidgets.QVBoxLayout(page)
        lay.setContentsMargins(14, 14, 14, 14)
        msg = QtWidgets.QLabel(f"{title} could not be loaded.\n\n{message}")
        msg.setWordWrap(True)
        msg.setTextInteractionFlags(QtCore.Qt.TextSelectableByMouse)
        lay.addWidget(msg)
        lay.addStretch(1)
    ...
    def _replace_tab_widget(self, idx: int, title: str, widget: QtWidgets.QWidget) -> None:
        if not hasattr(self, "tabs") or self.tabs is None:
            return
        if idx < 0 or idx >= self.tabs.count():
            return
    ...
    def _build_flux_tab(self) -> QtWidgets.QWidget:
        if _flux_klein_hidden_by_remove_hide():
            ...
            # entries are hidden. Return an empty page if a stale lazy-load
            # signal fires during startup/restore.
            return QtWidgets.QWidget(self)
        try:
            import flux_klein_editor_ui as fluxui  # type: ignore
            page = fluxui.MainWindow()
            try:
                page.setWindowTitle("")
            except Exception:
                pass
    ...
    def _build_hidream_tab(self) -> QtWidgets.QWidget:
        if _hidream_hidden_by_remove_hide():
            # Safety guard: this should normally never be reached because the
            # tab is not added when hidden. Avoid showing an import-error
            # placeholder if stale restore/lazy-load code calls this anyway.
            self.hidream_pane = None
            return QtWidgets.QWidget(self)
        cls = _load_hidream_ui_class()
        if cls is None:
            self.hidream_pane = None
            detail = globals().get("_LAST_HIDREAM_IMPORT_ERROR", "") or "No Python exception was reported."
            return self._make_tool_error_page(
                "HiDream Image Edit",
                "hidream_ui.py could not be imported.\n\nReason:\n" + str(detail),
            )
        try:
            self.hidream_pane = cls(self)
            try:
                self.hidream_pane.setWindowTitle("")
            except Exception:
                pass
            return self.hidream_pane
        except Exception as exc:
            self.hidream_pane = None
    ...
    def _build_firered_tab(self) -> QtWidgets.QWidget:
        if _firered_hidden_by_remove_hide():
            # Safety guard: this should normally never be reached because the
            # tab is not added when hidden. Return an empty page instead of an
            # import-error placeholder if a stale signal tries to load it.
            self.fire_red_pane = None
            return QtWidgets.QWidget(self)
        cls = _load_firered_pane_class()
        if cls is None:
            self.fire_red_pane = None
            return self._make_tool_error_page(
                "Firered 1.1",
                "firered.py could not be imported.",
            )
        try:
            self.fire_red_pane = cls(self)
            return self.fire_red_pane
        except Exception as exc:
            self.fire_red_pane = None
            return self._make_tool_error_page("Firered 1.1", str(exc))

    def _ensure_lazy_tab_loaded(self, idx: int, user_requested: bool = False) -> None:
        if getattr(self, "_lazy_tab_loading", False):
            return
        try:
            if not hasattr(self, "tabs") or self.tabs is None:
                return
            ...
            if title not in builders:
                return
            if title in getattr(self, "_lazy_tab_loaded", set()):
                return
            # Reliability first: whenever a lazy optional tab becomes selected,
            # load it.  This preserves fast startup when Qwen Edit is selected,
            # but avoids restored Flux/Firered tabs getting stuck on the
            # placeholder forever.
            self._set_lazy_placeholder_message(idx, title, f"Loading {title}...")
            self._lazy_tab_loading = True
            try:
                page = builders[title]()
                self._replace_tab_widget(idx, title, page)
                self._lazy_tab_loaded.add(title)
            finally:
                self._lazy_tab_loading = False
        except Exception as exc:
            try:
                ...
                pass
            try:
                title = str(self.tabs.tabText(idx)).strip() if hasattr(self, "tabs") and self.tabs is not None and 0 <= int(idx) < self.tabs.count() else "Lazy tool"
                detail = "".join(traceback.format_exception_only(type(exc), exc)).strip()
                self._replace_tab_widget(int(idx), title, self._make_tool_error_page(title, "Lazy tab loader failed.\n\nReason:\n" + detail))
            except Exception:
                pass


    def _lazy_current_tab_watchdog(self) -> None:
        """Load a visible lazy placeholder even when tab restore/click signals were missed."""
        try:
            if not hasattr(self, "tabs") or self.tabs is None:
                return
            idx = int(self.tabs.currentIndex())
            ...
            if title in getattr(self, "_lazy_tab_loaded", set()):
                return
            page = self.tabs.widget(idx)
            if page is not None and bool(page.property("_fv_lazy_placeholder")):
                self._set_lazy_placeholder_message(idx, title, f"Loading {title}...")
                self._ensure_lazy_tab_loaded(idx, user_requested=True)
        except Exception:
            pass
    ...
    def _on_tab_changed(self, idx: int):
        """Lazy-load optional tabs and update the pinned banner."""
        try:
            self._ensure_lazy_tab_loaded(int(idx))
            QtCore.QTimer.singleShot(0, self._lazy_current_tab_watchdog)
        except Exception:
            pass
        try:
    ...
    def _pick_sdcli(self):
        ...
        if fn:
            ...
            self._caps_detection_deferred = False
            self._append_log("\n" + self._caps_summary())
