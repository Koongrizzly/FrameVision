...
def enqueue_wan22_from_widget(inner) -> bool:
    ...
    try:
        ...
        try:
            ...
            mode = "text2video"
        # Turbo is now a real engine selection, not a checkbox-only mode.
        try:
            engine_key = str(inner._wan_engine_key()).strip().lower()
        except Exception:
            try:
                engine_key = str(inner.cmb_engine.currentData() or inner.cmb_engine.currentText()).strip().lower()
            except Exception:
                engine_key = "wan22"
        try:
            turbo_enabled = bool(inner._wan_turbo_enabled())
        except Exception:
            turbo_enabled = engine_key in ("wan22_turbo", "wan 2.2 turbo")
        if turbo_enabled:
            job_type = "wan22_turbo_image2video" if "image" in mode else "wan22_turbo_text2video"
        else:
        ...
        try:
            ...
            image = ""
        if job_type in ("wan22_image2video", "wan22_turbo_image2video") and not image:
            raise RuntimeError("Start image is required for image2video / Turbo TI2V mode.")
        ...
        try:
            ...
            frames = 121
        if turbo_enabled:
            # Turbo/few-step path is fixed at 4 steps and does not use normal CFG.
            steps = 4
            guidance = 0.0
        # FPS (best-effort)
        try:
        ...
        label = (prompt[:80] or (("Wan2.2 Turbo " if turbo_enabled else "Wan2.2 ") + ("image2video" if job_type in ("wan22_image2video", "wan22_turbo_image2video") else "text2video")))
        # Wan VRAM Lab / worker options. These must be stored in the queue job
        # because the worker runs in a separate process and cannot read the UI.
        try:
            vram_lab = str(inner._vram_lab_mode()).strip().lower()
        except Exception:
            vram_lab = "off"
        try:
            vram_profile = str(inner._vram_lab_profile()).strip()
        except Exception:
            vram_profile = "24"
        try:
            offload_model = bool(getattr(inner, "chk_offload_model", None) and inner.chk_offload_model.isChecked())
        except Exception:
            offload_model = False
        try:
            t5_cpu = bool(getattr(inner, "chk_t5_cpu", None) and inner.chk_t5_cpu.isChecked())
        except Exception:
            t5_cpu = False
        try:
            if hasattr(inner, "_wan_flash_attention_enabled"):
                flash_attention = bool(inner._wan_flash_attention_enabled())
            else:
                flash_attention = bool(getattr(inner, "chk_flash_attention", None) and inner.chk_flash_attention.isChecked())
        except Exception:
            flash_attention = True
        try:
            if hasattr(inner, "_wan_crawl_guard_enabled"):
                crawl_guard = bool(inner._wan_crawl_guard_enabled())
            else:
                crawl_guard = bool(getattr(inner, "chk_crawl_guard", None) and inner.chk_crawl_guard.isChecked())
        except Exception:
            crawl_guard = False
        args = {
            "prompt": prompt,
            "engine": "wan22_turbo" if turbo_enabled else "wan22",
            "turbo_model": bool(turbo_enabled),
            "mode": "image2video" if job_type in ("wan22_image2video", "wan22_turbo_image2video") else "text2video",
            "image": image if job_type in ("wan22_image2video", "wan22_turbo_image2video") else "",
            ...
            "save_file": save_file,
            "label": label,
            "vram_lab": ("safe" if str(vram_lab or "off").lower() != "off" else "off"),
            "vram_profile": (str(vram_profile) if str(vram_profile) in ("12", "16", "24") else "24"),
            "offload_model": bool(offload_model),
            "t5_cpu": bool(t5_cpu),
            "flash_attention": bool(flash_attention),
            "crawl_guard": bool(crawl_guard),
        }
        ...
        if job_type in ("wan22_image2video", "wan22_turbo_image2video"):
            input_path = image
        else:
