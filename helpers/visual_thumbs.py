...
class VisualThumbManager(QObject):
    ...
    Behaviour:
    - Normal startup does not pre-build thumbnails. This keeps FrameVision
      startup fast even when many music-player visual modes exist.
    - When the UI asks for an icon for a visual mode, we lazily ensure a
      thumbnail exists and return a QIcon built from it.
    - A full pre-build scan can still be requested for maintenance/testing by
      setting FRAMEVISION_PREBUILD_VISUAL_THUMBS=1 before launching the app.
    ...
    _thumb_path_cache: dict[str, Optional[str]] = {}
    ...
    def __init__(self, parent=None, ffmpeg: Optional[str] = None) -> None:
        ...
        # Do not pre-scan visual thumbnails during normal app startup.
        # Startup profiling showed that repeated automatic scans can become a
        # major launch-time cost when several VisualThumbManager instances are
        # created. Lazy generation below still keeps the UI working normally.
        try:
            prebuild = os.environ.get("FRAMEVISION_PREBUILD_VISUAL_THUMBS", "")
            if str(prebuild).strip().lower() in {"1", "true", "yes", "on"}:
    ...
    def _initial_scan(self) -> None:
        ...
        The scan is guarded at class/session level because multiple
        VisualThumbManager instances can exist. Only one scan may run, and a
        completed scan is not repeated in the same app session.
        ...
        """
        cls = type(self)
        if cls._scan_running or cls._scan_done:
            return

        cls._scan_running = True
        try:
            try:
    ...
    def _ensure_thumb_dir(self, path: str) -> bool:
        """Create the thumbnail folder once per app session."""
        folder = os.path.dirname(path)
        cls = type(self)
        if folder in cls._ensured_dirs:
            return True
        ...
        cls._ensured_dirs.add(folder)
        return True

    def _ensure_thumbnail(self, mode_id: str) -> Optional[str]:
        key = str(mode_id)
        cls = type(self)
        if key in cls._thumb_path_cache:
            return cls._thumb_path_cache.get(key)

        path = self._thumb_path_for_mode(key)
        if not self._ensure_thumb_dir(path):
            cls._thumb_path_cache[key] = None
            return None

        if os.path.exists(path):
            cls._thumb_path_cache[key] = path
            return path

        ok = self._create_real_thumbnail(key, path)
        if not ok:
            ok = self._create_text_placeholder_thumbnail(key, path)
        if not ok:
            cls._thumb_path_cache[key] = None
            return None
