...
# --- Lightweight ETA timer (time-based, no per-step) ---
    ...
    def _build_ui(self):
        ...
        # VRAM/workflow controls live in Settings / Advanced, keeping Generation clean.
        try:
            self.vram_label.show()
            self.vram_profile.show()
            self.restore_auto.show()
        except Exception:
            pass
        try:
     ...
     #   adv_form.addRow(self.hires_helper)
        ...
        self._txt2img_footer_widgets = [
            self.add_to_queue, self.generate_now, self.batch_label_bottom,
            self.batch, self.open_folder_btn, self.chroma_offload_cpu, self.add_and_run,
        ]
    ...
    def _on_engine_changed(self, *_):
        ...
        try:
            ...
            if bool(key in ("zimage_gguf", "qwen2512", "lens_turbo_u4", "chroma", "ideogram4_gguf")):
                try:
...
def _t2i_apply_from_dict(self, s: dict):
    ...
    widgets = [
        ...
        'chroma_offload_cpu','use_queue','show_in_player','vram_profile','output_path'
    ]
    blocked = []
    for name in widgets:
