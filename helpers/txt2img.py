...
class Txt2ImgPane(QWidget):
    ...
    def _hidden_engine_keys_from_remove_hide(self) -> set[str]:
        """Read remove/hide state and return txt2img engine keys to hide."""
        try:
            try:
    ...
    def _txt2img_engine_defs(self) -> list[tuple[str, str]]:
        return [
    ...
    def refresh_hidden_tools(self) -> None:
        """Refresh txt2img engine visibility without restarting FrameVision."""
        try:
            cb = getattr(self, "engine_combo", None)
            if cb is None:
                return
            hidden_keys = self._hidden_engine_keys_from_remove_hide()
            state = tuple(sorted(hidden_keys))
            current_key = None
            try:
                current_key = cb.currentData()
            except Exception:
                current_key = None
            if current_key is None:
                try:
                    current_key = self._engine_key_selected()
                except Exception:
                    current_key = None
            ...
            try:
                cb.clear()
                for label, key in self._txt2img_engine_defs():
                ...
                if cb.count() <= 0:
                    try:
                        cb.addItem("All engines hidden by user", "none")
                    except Exception:
                        cb.addItem("All engines hidden by user")
                    try:
                        cb.setEnabled(False)
                    except Exception:
                        pass
                    ...
                    try:
                        ...
                        pass
                    restore_idx = -1
                    if current_key and current_key not in hidden_keys:
                        try:
                            restore_idx = cb.findData(current_key)
                        except Exception:
                            restore_idx = -1
                    if restore_idx < 0:
                        restore_idx = 0
                    try:
                        cb.setCurrentIndex(restore_idx)
                    except Exception:
                        pass
            finally:
                try:
                    ...
                    pass
            try:
                self._last_hidden_engine_state = state
            except Exception:
                pass
            try:
    ...
    def _install_hidden_tools_refresh_timer(self) -> None:
        """Poll the hide-state JSON so hide/unhide takes effect immediately."""
        try:
            self._last_hidden_engine_state = tuple(sorted(self._hidden_engine_keys_from_remove_hide()))
        except Exception:
    ...
    def _check_hidden_tools_refresh(self) -> None:
        try:
            state = tuple(sorted(self._hidden_engine_keys_from_remove_hide()))
            if state != getattr(self, "_last_hidden_engine_state", None):
                self.refresh_hidden_tools()
        except Exception:
            pass

    def _force_hide_img2img_section(self) -> None:
        """Hide the generic img2img/init-image row and force it off.

        The controls are kept in the file for compatibility with old saved JSON and
        older job readers, but the visible txt2img UI no longer exposes this row.
        """
        try:
            if getattr(self, "zimg_init_enable", None) is not None:
                try:
                    self.zimg_init_enable.blockSignals(True)
                except Exception:
                    pass
                try:
                    ...
                    pass
                try:
                    self.zimg_init_enable.blockSignals(False)
                except Exception:
                    pass
        except Exception:
            pass
            ...
            try:
                if _w is not None:
                    ...
                    try:
                        ...
                        pass
                    try:
                        _w.setMinimumHeight(0)
                    except Exception:
                        pass
                
            except Exception:
                pass
        for _w in (getattr(self, "zimg_init_path", None), getattr(self, "zimg_init_browse", None),
                   getattr(self, "zimg_init_clear", None), getattr(self, "zimg_strength", None)):
            try:
                if _w is not None:
                    _w.setEnabled(False)
            except Exception:
    ...
    def _apply_settings_from_dict(self, s: dict):
            ...
            try:
                ...
                pass
            try:
                # Generic img2img/init-image row is intentionally hidden/disabled.
                # Keep any saved old value from turning it back on at startup.
                self._force_hide_img2img_section()
            except Exception:
                pass
            try:
    ...
    def __init__(self, app_window):
        ...
        try:
            self.add_to_queue.show()
            self.add_and_run.hide()
            self.generate_now.hide()
        except Exception:
            pass
...
# --- Lightweight ETA timer (time-based, no per-step) ---
    ...
    def _engine_key_selected(self) -> str:
        ...
        if ("lens" in key) or ("turbo_u4" in key) or ("turbo u4" in key):
            return "lens_turbo_u4"
    ...
    def _build_ui(self):
        ...
        # - Generation keeps only the day-to-day controls users need to create an image.
        # - Settings holds output/workflow controls and advanced generation options.
        self.txt2img_tabs = QTabWidget(self)
        try:
            self.txt2img_tabs.setDocumentMode(True)
        except Exception:
            pass
        outer.addWidget(self.txt2img_tabs, 1)
        ...
        try:
            ...
            pass
        self.txt2img_tabs.addTab(scroll, "Generation")
        ...
        settings_container = QWidget(); settings_scroll.setWidget(settings_container)
        settings_root = QVBoxLayout(settings_container)
        try:
            settings_root.setContentsMargins(10, 10, 10, 10)
            settings_root.setSpacing(10)
        except Exception:
            pass
        self.txt2img_tabs.addTab(settings_scroll, "Settings / Advanced")

        # Embedded selective-edit tab for Z-Image GGUF only.
        self._zimg_sel_edit_tab = QWidget(self)
        self._zimg_sel_edit_tab_layout = QVBoxLayout(self._zimg_sel_edit_tab)
        try:
            self._zimg_sel_edit_tab_layout.setContentsMargins(0, 0, 0, 0)
            self._zimg_sel_edit_tab_layout.setSpacing(0)
        except Exception:
            pass
        self._zimg_sel_edit_loading_label = QLabel("Loading Z-Image selective edit UI...")
        try:
            self._zimg_sel_edit_loading_label.setAlignment(Qt.AlignCenter)
            self._zimg_sel_edit_loading_label.setStyleSheet("padding: 24px; font-size: 16px; font-weight: 600;")
            self._zimg_sel_edit_tab_layout.addWidget(self._zimg_sel_edit_loading_label, 1)
        except Exception:
            pass
        self._zimg_sel_edit_open_pending = False
        ...
        try:
            ...
            self.engine_combo.addItem("Z-Image Turbo (FP16)", "zimage")
            ...
            self.engine_combo.addItem("Ideogram 4 GGUF", "ideogram4_gguf")
            self.engine_combo.addItem("Krea 2 GGUF", "krea2")
            ...
            self.engine_combo.addItem("Z-Image Turbo (FP16)")
            self.engine_combo.addItem("Z-Image Turbo (GGUF Low VRAM)")
            self.engine_combo.addItem("qwen 2.5 12B GGUF (Low VRAM)")
            self.engine_combo.addItem("Ideogram 4 GGUF")
            self.engine_combo.addItem("Krea 2 GGUF")
        ...
        # Keep the engine selector above the Generation / Settings tabs.
        # Some engines, such as Lens, bring their own embedded UI and do not use these tabs.
        try:
            outer.insertLayout(2, engine_row)
        except Exception:
            try:
        ...
        # Embedded Lens UI host. Lens stays in its own helper/UI code, but is shown inside this tab.
        self._txt2img_root_layout = root
        self._lens_embed_host = QWidget(self)
        self._lens_embed_host.setVisible(False)
        self._lens_embed_layout = QVBoxLayout(self._lens_embed_host)
        try:
            self._lens_embed_layout.setContentsMargins(0, 0, 0, 0)
        except Exception:
            pass
        self._lens_loading_label = QLabel("Loading Lens Turbo U4 UI...")
        try:
            self._lens_loading_label.setAlignment(Qt.AlignCenter)
            self._lens_loading_label.setStyleSheet("padding: 24px; font-size: 16px; font-weight: 600;")
            self._lens_embed_layout.addWidget(self._lens_loading_label, 1)
        except Exception:
            pass
        self._lens_open_pending = False
        # Important: Lens is a complete embedded helper UI with its own tabs/footer.
        # Keep it OUTSIDE the normal Generation QScrollArea. This matches the Wan22
        # engine-page pattern: imported helper UIs get their own full page area, while
        # txt2img's normal scrolled controls are hidden when that helper engine is active.
        try:
            self._lens_embed_host.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Expanding)
        except Exception:
            pass
        outer.addWidget(self._lens_embed_host, 1)

        # Embedded Chroma UI host. Chroma keeps its own helper UI/settings/logs and
        # txt2img only embeds it, like Lens, instead of running it through the main UI.
        self._chroma_embed_host = QWidget(self)
        self._chroma_embed_host.setVisible(False)
        self._chroma_embed_layout = QVBoxLayout(self._chroma_embed_host)
        try:
            self._chroma_embed_layout.setContentsMargins(0, 0, 0, 0)
        except Exception:
            pass
        self._chroma_loading_label = QLabel("Loading Chroma UI...")
        try:
            self._chroma_loading_label.setAlignment(Qt.AlignCenter)
            self._chroma_loading_label.setStyleSheet("padding: 24px; font-size: 16px; font-weight: 600;")
            self._chroma_embed_layout.addWidget(self._chroma_loading_label, 1)
        except Exception:
            pass
        self._chroma_open_pending = False
        # Same rule as Lens: Chroma is a full helper page, not a child of the
        # txt2img Generation scroll area.
        try:
            self._chroma_embed_host.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Expanding)
        except Exception:
            pass
        outer.addWidget(self._chroma_embed_host, 1)

        # Embedded Ideogram 4 GGUF UI host. This is a real PySide embedded helper page,
        # not a Tkinter subprocess placeholder. It stays isolated from txt2img controls.
        self._ideogram4_embed_host = QWidget(self)
        self._ideogram4_embed_host.setVisible(False)
        self._ideogram4_embed_layout = QVBoxLayout(self._ideogram4_embed_host)
        try:
            self._ideogram4_embed_layout.setContentsMargins(0, 0, 0, 0)
        except Exception:
            pass
        self._ideogram4_loading_label = QLabel("Loading Ideogram 4 GGUF UI...")
        try:
            self._ideogram4_loading_label.setAlignment(Qt.AlignCenter)
            self._ideogram4_loading_label.setStyleSheet("padding: 24px; font-size: 16px; font-weight: 600;")
            self._ideogram4_embed_layout.addWidget(self._ideogram4_loading_label, 1)
        except Exception:
            pass
        self._ideogram4_open_pending = False
        try:
            self._ideogram4_embed_host.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Expanding)
        except Exception:
            pass
        outer.addWidget(self._ideogram4_embed_host, 1)

        # Embedded Krea 2 GGUF UI host. Krea 2 is a full helper page that runs
        # through sd-cli, so txt2img embeds its own UI instead of mixing controls.
        self._krea2_embed_host = QWidget(self)
        self._krea2_embed_host.setVisible(False)
        self._krea2_embed_layout = QVBoxLayout(self._krea2_embed_host)
        try:
            self._krea2_embed_layout.setContentsMargins(0, 0, 0, 0)
        except Exception:
            pass
        self._krea2_loading_label = QLabel("Loading Krea 2 GGUF UI...")
        try:
            self._krea2_loading_label.setAlignment(Qt.AlignCenter)
            self._krea2_loading_label.setStyleSheet("padding: 24px; font-size: 16px; font-weight: 600;")
            self._krea2_embed_layout.addWidget(self._krea2_loading_label, 1)
        except Exception:
            pass
        self._krea2_open_pending = False
        try:
            self._krea2_embed_host.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Expanding)
        except Exception:
            pass
        outer.addWidget(self._krea2_embed_host, 1)

        # Embedded Boogu Image UI host. Boogu is a full sd-cli helper page with
        # its own Create/Edit/Settings tabs and queue toggle.
        self._boogu_embed_host = QWidget(self)
        self._boogu_embed_host.setVisible(False)
        self._boogu_embed_layout = QVBoxLayout(self._boogu_embed_host)
        try:
            self._boogu_embed_layout.setContentsMargins(0, 0, 0, 0)
        except Exception:
            pass
        self._boogu_loading_label = QLabel("Loading Boogu Image UI...")
        try:
            self._boogu_loading_label.setAlignment(Qt.AlignCenter)
            self._boogu_loading_label.setStyleSheet("padding: 24px; font-size: 16px; font-weight: 600;")
            self._boogu_embed_layout.addWidget(self._boogu_loading_label, 1)
        except Exception:
            pass
        self._boogu_open_pending = False
        try:
            self._boogu_embed_host.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Expanding)
        except Exception:
            pass
        ...
        preset_row.addWidget(self.style_builder_btn, 0)
        self.preset_row_wrap = QWidget(self)
        self.preset_row_wrap.setLayout(preset_row)
        form.addRow(self.preset_label, self.preset_row_wrap)
        ...
        try:
            ...
            # Hidden by request: keep img2img/init-image off while hidden.
            # Do not add this row to the visible form layout.
            try:
                self.zimg_i2i_label.setVisible(False)
            except Exception:
                pass
            try:
                self.zimg_i2i_wrap.setVisible(False)
            except Exception:
                pass
            self._force_hide_img2img_section()
        except Exception:
        ...
        row.addWidget(self.seed_policy, 1)
        ...
        form.addRow("Output size", size_row)
        ...
        self.seed_used_label = QLabel("—"); self.seed_used_label.setVisible(False)
        form.addRow("Seed used", self.seed_used_label)
        ...
        # Output path belongs in Settings / Advanced, not the main Generation tab.

        # Workflow controls. The old VRAM profile dropdown was removed because it was
        # cosmetic in this tab. Keep a constant compatibility value for older queue/result
        # code that still reads job["vram_profile"].
        self._vram_profile_compat = "Auto"
        workflow_row = QHBoxLayout()
        workflow_row.addWidget(self.show_in_player)
        workflow_row.addWidget(self.use_queue)
        workflow_row.addStretch(1)
        try:
            _settings_note = QLabel("Less-used settings are here so the Generation tab stays simple for new users.")
            _settings_note.setWordWrap(True)
            _settings_note.setStyleSheet("font-size: 11px; color: palette(mid);")
            settings_root.addWidget(_settings_note)
        except Exception:
            pass
        try:
            _output_box = QGroupBox("Output & Workflow")
            _output_lay = QVBoxLayout(_output_box)
            _output_lay.setContentsMargins(10, 10, 10, 10)
            _output_lay.setSpacing(8)
            _output_lay.addLayout(out_row)
            _output_lay.addLayout(workflow_row)
            settings_root.addWidget(_output_box)
        except Exception:
            try:
                settings_root.addLayout(out_row)
                settings_root.addLayout(workflow_row)
            except Exception:
                pass
        ...
        self._model_picker = _Disclosure("Model / LoRA", mdl_body, start_open=False, parent=self)
        # Added after the basic generation form so LoRA/model controls sit below prompt/seed/steps.
        ...
        try:
            ...
            pass
        try:
            if hasattr(self, "txt2img_tabs") and self.txt2img_tabs is not None:
                self.txt2img_tabs.currentChanged.connect(lambda *_: self._sync_zimg_sel_edit_footer_visibility())
        except Exception:
            pass
        try:
     ...
     #   adv_form.addRow(self.hires_helper)
        ...
        try:
            ...
            pass
        self._advanced = _Disclosure("Advanced Generation Settings", adv_body, start_open=False, parent=self)
        try:
            settings_root.addWidget(self._advanced)
        ...
        self._txt2img_progress_widgets = [self.progress, self.status]
        ...
        # Legacy action buttons are kept for signal compatibility but are not shown in the footer.
        self.add_and_run = QPushButton("Add & Run")
        self.add_and_run.hide()
        self.generate_now = QPushButton("Generate image(s)")
        self.generate_now.hide()
        # Batch controls stay next to the visible queue action.
        ...
        self.chroma_offload_cpu = QCheckBox("Chroma CPU offload")
        try:
            self.chroma_offload_cpu.setChecked(True)
            self.chroma_offload_cpu.setVisible(False)
            self.chroma_offload_cpu.setToolTip("Chroma only: offload parts of the model to CPU to save VRAM, but generation becomes slower.")
        except Exception:
            pass
        btns.addWidget(self.chroma_offload_cpu)
        btns.addStretch(1)
        self._txt2img_footer_widgets = [
            self.add_to_queue, self.batch_label_bottom,
            self.batch, self.open_folder_btn, self.chroma_offload_cpu,
        ]
    ...
    def _restore_saved_lora_selection(self, saved: dict | None = None):
        ...
        try:
            ...
            pass
        try:
            if hasattr(self, "chroma_offload_cpu") and "chroma_offload_cpu" in (s or {}):
                self.chroma_offload_cpu.setChecked(bool((s or {}).get("chroma_offload_cpu")))
        except Exception:
            pass
        try:
    ...
    def _open_lens_turbo_ui(self):
        """Show the existing Lens Turbo U4 UI embedded inside the txt2img tab."""
        try:
            host = getattr(self, "_lens_embed_host", None)
            lay = getattr(self, "_lens_embed_layout", None)
            if host is None or lay is None:
                return None

            self._lens_open_pending = False
            win = getattr(self, "_lens_turbo_window", None)
            if win is None:
                try:
                    loading = getattr(self, "_lens_loading_label", None)
                    if loading is not None:
                        loading.setVisible(True)
                        loading.setText("Loading Lens Turbo U4 UI...")
                    host.setVisible(True)
                    QApplication.processEvents()
                except Exception:
                    pass
                try:
                    from helpers.lens_turbo_u4_ui import LensTurboWindow  # type: ignore
                except Exception:
                    from lens_turbo_u4_ui import LensTurboWindow  # type: ignore

                try:
                    win = LensTurboWindow(embedded=True)
                except TypeError:
                    win = LensTurboWindow()
                self._lens_turbo_window = win
                try:
                    win.setParent(host)
                except Exception:
                    pass
                ...
                try:
                    ...
                    pass
                try:
                    win.destroyed.connect(lambda *_: setattr(self, "_lens_turbo_window", None))
                except Exception:
                    pass
            ...
            try:
                if hasattr(self, "status"):
                    self.status.setText("Lens Turbo U4 UI shown inside TXT to IMG")
            except Exception:
                pass
            return win
            ...
            try:
                ...
                if loading is not None:
                    loading.setVisible(True)
                    loading.setText(f"Could not load Lens UI:\n{exc}")
            except Exception:
                pass
            try:
                QMessageBox.warning(self, "Lens Turbo U4", f"Could not show Lens UI:\n{exc}")
            except Exception:
                pass
            try:
                if hasattr(self, "status"):
                    self.status.setText(f"Could not show Lens UI: {exc}")
            except Exception:
                pass
            return None

    def _set_lens_embedded_visible(self, visible: bool):
        """Toggle Lens embedded view and hide txt2img controls/footer/tabs while Lens owns the page."""
        try:
            # update_ui() calls Lens and Chroma visibility one after another.
            # If Chroma is active, this Lens false-call must not restore the normal
            ...
            # Lens owns a complete helper page. Do not put or keep it inside the
            # normal txt2img tab/scroll stack; hide the entire txt2img tabs widget
            # while Lens is active so the parent scroll cannot move Lens tabs/footer.
            try:
            ...
            # These live outside the scrolled root, so hide them separately.
            ...
            if visible:
                try:
                    loading = getattr(self, "_lens_loading_label", None)
                    if loading is not None and getattr(self, "_lens_turbo_window", None) is None:
                        loading.setVisible(True)
                        loading.setText("Loading Lens Turbo U4 UI...")
                except Exception:
                    pass
                if getattr(self, "_lens_turbo_window", None) is None and not bool(getattr(self, "_lens_open_pending", False)):
                    self._lens_open_pending = True
                    try:
                        QTimer.singleShot(40, self._open_lens_turbo_ui)
                    except Exception:
                        self._open_lens_turbo_ui()
                else:
                    try:
                        self._open_lens_turbo_ui()
                    except Exception:
    ...
    def _open_chroma_ui(self):
        """Show the existing Chroma helper UI embedded inside the txt2img tab."""
        try:
            host = getattr(self, "_chroma_embed_host", None)
            lay = getattr(self, "_chroma_embed_layout", None)
            if host is None or lay is None:
                return None

            self._chroma_open_pending = False
            win = getattr(self, "_chroma_window", None)
            if win is None:
                try:
                    loading = getattr(self, "_chroma_loading_label", None)
                    if loading is not None:
                        loading.setVisible(True)
                        loading.setText("Loading Chroma UI...")
                    host.setVisible(True)
                    QApplication.processEvents()
                except Exception:
                    pass
                try:
                    from helpers.chroma import ChromaWindow  # type: ignore
                except Exception:
                    from chroma import ChromaWindow  # type: ignore

                win = ChromaWindow()
                self._chroma_window = win
                try:
                    win.setParent(host)
                except Exception:
                    pass
                ...
                try:
                    ...
                    pass
                try:
                    win.destroyed.connect(lambda *_: setattr(self, "_chroma_window", None))
                except Exception:
                    pass
            ...
            try:
                if hasattr(self, "status"):
                    self.status.setText("Chroma UI shown inside TXT to IMG")
            except Exception:
                pass
            return win
        except Exception as exc:
            ...
            try:
                ...
                if loading is not None:
                    loading.setVisible(True)
                    loading.setText(f"Could not load Chroma UI:\n{exc}")
            except Exception:
                pass
            try:
                QMessageBox.warning(self, "Chroma", f"Could not show Chroma UI:\n{exc}")
            except Exception:
                pass
            try:
                if hasattr(self, "status"):
                    self.status.setText(f"Could not show Chroma UI: {exc}")
            except Exception:
                pass
            return None

    def _set_chroma_embedded_visible(self, visible: bool):
        """Toggle Chroma embedded view and hide txt2img controls/footer/tabs while Chroma owns the page."""
        try:
            # update_ui() calls Lens and Chroma visibility one after another.
            # If Lens is active, this Chroma false-call must not restore the normal
            ...
            try:
                if chroma_host is not None:
                    chroma_host.setVisible(bool(visible))
            except Exception:
                pass
            for _host in (lens_host, krea2_host, boogu_host):
                try:
            ...
            for i in range(root.count()):
                ...
                if w is chroma_host:
                    try:
                        ...
                        pass
                elif w is lens_host or w is krea2_host or w is boogu_host:
                    try:
                        if w is not None:
                            w.setVisible(False)
                    except Exception:
            ...
            if visible:
                try:
                    loading = getattr(self, "_chroma_loading_label", None)
                    if loading is not None and getattr(self, "_chroma_window", None) is None:
                        loading.setVisible(True)
                        loading.setText("Loading Chroma UI...")
                except Exception:
                    pass
                if getattr(self, "_chroma_window", None) is None and not bool(getattr(self, "_chroma_open_pending", False)):
                    self._chroma_open_pending = True
                    try:
                        QTimer.singleShot(40, self._open_chroma_ui)
                    except Exception:
                        self._open_chroma_ui()
                else:
                    try:
                        self._open_chroma_ui()
                    except Exception:
                ...
                try:
                    if chroma_host is not None:
                        chroma_host.setVisible(False)
                except Exception:
                    pass
        except Exception:
            pass

    def _open_ideogram4_ui(self):
        """Show the Ideogram 4 GGUF helper UI embedded inside the txt2img tab."""
        try:
            host = getattr(self, "_ideogram4_embed_host", None)
            lay = getattr(self, "_ideogram4_embed_layout", None)
            if host is None or lay is None:
                return None

            self._ideogram4_open_pending = False
            win = getattr(self, "_ideogram4_window", None)
            if win is None:
                try:
                    loading = getattr(self, "_ideogram4_loading_label", None)
                    if loading is not None:
                        loading.setVisible(True)
                        loading.setText("Loading Ideogram 4 GGUF UI...")
                    host.setVisible(True)
                    QApplication.processEvents()
                except Exception:
                    pass
                try:
                    from helpers.ideogram4 import Ideogram4Widget  # type: ignore
                except Exception:
                    from ideogram4 import Ideogram4Widget  # type: ignore

                win = Ideogram4Widget(embedded=True)
                self._ideogram4_window = win
                try:
                    win.setParent(host)
                except Exception:
                    pass
                ...
                try:
                    ...
                    pass
                try:
                    win.destroyed.connect(lambda *_: setattr(self, "_ideogram4_window", None))
                except Exception:
                    pass
            ...
            try:
                if hasattr(self, "status"):
                    self.status.setText("Ideogram 4 GGUF UI shown inside TXT to IMG")
            except Exception:
                pass
            return win
        except Exception as exc:
            ...
            try:
                ...
                if loading is not None:
                    loading.setVisible(True)
                    loading.setText(f"Could not load Ideogram 4 GGUF UI:\n{exc}")
            except Exception:
                pass
            try:
                QMessageBox.warning(self, "Ideogram 4 GGUF", f"Could not show Ideogram 4 GGUF UI:\n{exc}")
            except Exception:
                pass
            try:
                if hasattr(self, "status"):
                    self.status.setText(f"Could not show Ideogram 4 GGUF UI: {exc}")
            except Exception:
                pass
            return None

    def _set_ideogram4_embedded_visible(self, visible: bool):
        """Toggle Ideogram embedded view and hide txt2img controls/footer/tabs while Ideogram owns the page."""
        try:
            if not bool(visible):
                try:
            ...
            for _host in (lens_host, chroma_host, krea2_host, boogu_host):
                try:
                    if _host is not None:
                        _host.setVisible(False)
                except Exception:
                    pass
            try:
                if ideogram_host is not None:
                    ideogram_host.setVisible(bool(visible))
            except Exception:
                pass
            ...
            for i in range(root.count()):
                ...
                try:
                    ...
                    w = None
                if w is ideogram_host:
                    try:
                        ideogram_host.setVisible(bool(visible))
                    except Exception:
                        pass
                elif w is lens_host or w is chroma_host or w is krea2_host or w is boogu_host:
                    try:
                        if w is not None:
                            w.setVisible(False)
                    except Exception:
            ...
            if visible:
                try:
                    loading = getattr(self, "_ideogram4_loading_label", None)
                    if loading is not None and getattr(self, "_ideogram4_window", None) is None:
                        loading.setVisible(True)
                        loading.setText("Loading Ideogram 4 GGUF UI...")
                except Exception:
                    pass
                if getattr(self, "_ideogram4_window", None) is None and not bool(getattr(self, "_ideogram4_open_pending", False)):
                    self._ideogram4_open_pending = True
                    try:
                        QTimer.singleShot(40, self._open_ideogram4_ui)
                    except Exception:
                        self._open_ideogram4_ui()
                else:
                    try:
                        self._open_ideogram4_ui()
                    except Exception:
                ...
                try:
                    if ideogram_host is not None:
                        ideogram_host.setVisible(False)
                except Exception:
                    pass
        except Exception:
            pass

    def _open_krea2_ui(self):
        """Show the Krea 2 GGUF helper UI embedded inside the txt2img tab."""
        try:
            host = getattr(self, "_krea2_embed_host", None)
            lay = getattr(self, "_krea2_embed_layout", None)
            if host is None or lay is None:
                return None

            self._krea2_open_pending = False
            win = getattr(self, "_krea2_window", None)
            if win is None:
                try:
                    loading = getattr(self, "_krea2_loading_label", None)
                    if loading is not None:
                        loading.setVisible(True)
                        loading.setText("Loading Krea 2 GGUF UI...")
                    host.setVisible(True)
                    QApplication.processEvents()
                except Exception:
                    pass
                try:
                    from helpers.krea2 import Krea2Widget  # type: ignore
                except Exception:
                    from krea2 import Krea2Widget  # type: ignore

                win = Krea2Widget(parent=host)
                self._krea2_window = win
                try:
                    win.setParent(host)
                except Exception:
                    pass
                ...
                try:
                    ...
                    pass
                try:
                    win.destroyed.connect(lambda *_: setattr(self, "_krea2_window", None))
                except Exception:
                    pass
            ...
            try:
                if hasattr(self, "status"):
                    self.status.setText("Krea 2 GGUF UI shown inside TXT to IMG")
            except Exception:
                pass
            return win
        except Exception as exc:
            ...
            try:
                ...
                if loading is not None:
                    loading.setVisible(True)
                    loading.setText(f"Could not load Krea 2 GGUF UI:\n{exc}")
            except Exception:
                pass
            try:
                QMessageBox.warning(self, "Krea 2 GGUF", f"Could not show Krea 2 GGUF UI:\n{exc}")
            except Exception:
                pass
            try:
                if hasattr(self, "status"):
                    self.status.setText(f"Could not show Krea 2 GGUF UI: {exc}")
            except Exception:
                pass
            return None

    def _set_krea2_embedded_visible(self, visible: bool):
        """Toggle Krea 2 embedded view and hide txt2img controls/footer/tabs while Krea owns the page."""
        try:
            if not bool(visible):
                try:
            ...
            for _host in (lens_host, chroma_host, ideogram_host, boogu_host):
                try:
                    if _host is not None:
                        _host.setVisible(False)
                except Exception:
                    pass
            try:
                if krea2_host is not None:
                    krea2_host.setVisible(bool(visible))
            except Exception:
                pass
            ...
            for i in range(root.count()):
                ...
                if w is krea2_host:
                    try:
                        ...
                        pass
                elif w is lens_host or w is chroma_host or w is ideogram_host or w is boogu_host:
                    try:
                        if w is not None:
                            w.setVisible(False)
                    except Exception:
            ...
            if visible:
                try:
                    loading = getattr(self, "_krea2_loading_label", None)
                    if loading is not None and getattr(self, "_krea2_window", None) is None:
                        loading.setVisible(True)
                        loading.setText("Loading Krea 2 GGUF UI...")
                except Exception:
                    pass
                if getattr(self, "_krea2_window", None) is None and not bool(getattr(self, "_krea2_open_pending", False)):
                    self._krea2_open_pending = True
                    try:
                        QTimer.singleShot(40, self._open_krea2_ui)
                    except Exception:
                        self._open_krea2_ui()
                else:
                    try:
                ...
                try:
                    if krea2_host is not None:
                        krea2_host.setVisible(False)
                except Exception:
                    pass
        except Exception:
            pass

    def _open_boogu_ui(self):
        """Show the Boogu Image helper UI embedded inside the txt2img tab."""
        try:
            host = getattr(self, "_boogu_embed_host", None)
            lay = getattr(self, "_boogu_embed_layout", None)
            if host is None or lay is None:
                return None

            self._boogu_open_pending = False
            win = getattr(self, "_boogu_window", None)
            if win is None:
                try:
                    loading = getattr(self, "_boogu_loading_label", None)
                    if loading is not None:
                        loading.setVisible(True)
                        loading.setText("Loading Boogu Image UI...")
                    host.setVisible(True)
                    QApplication.processEvents()
                except Exception:
                    pass
                try:
                    from helpers.boogu_ui import BooguUI  # type: ignore
                except Exception:
                    from boogu_ui import BooguUI  # type: ignore

                win = BooguUI(parent=host)
                self._boogu_window = win
                try:
                    win.setParent(host)
                except Exception:
                    pass
                ...
                try:
                    ...
                    pass
                try:
                    win.destroyed.connect(lambda *_: setattr(self, "_boogu_window", None))
                except Exception:
                    pass
            ...
            try:
                if hasattr(self, "status"):
                    self.status.setText("Boogu Image UI shown inside TXT to IMG")
            except Exception:
                pass
            return win
        except Exception as exc:
            ...
            try:
                ...
                if loading is not None:
                    loading.setVisible(True)
                    loading.setText(f"Could not load Boogu Image UI:\n{exc}")
            except Exception:
                pass
            try:
                QMessageBox.warning(self, "Boogu Image", f"Could not show Boogu Image UI:\n{exc}")
            except Exception:
                pass
            try:
                if hasattr(self, "status"):
                    self.status.setText(f"Could not show Boogu Image UI: {exc}")
            except Exception:
                pass
            return None

    def _set_boogu_embedded_visible(self, visible: bool):
        """Toggle Boogu embedded view and hide txt2img controls/footer/tabs while Boogu owns the page."""
        try:
            if not bool(visible):
                try:
            ...
            for _host in (lens_host, chroma_host, ideogram_host, krea2_host):
                try:
                    if _host is not None:
                        _host.setVisible(False)
                except Exception:
                    pass
            try:
                if boogu_host is not None:
                    boogu_host.setVisible(bool(visible))
            except Exception:
                pass
            ...
            for i in range(root.count()):
                ...
                if w is boogu_host:
                    try:
                        ...
                        pass
                elif w is lens_host or w is chroma_host or w is ideogram_host or w is krea2_host:
                    try:
                        if w is not None:
                            w.setVisible(False)
                    except Exception:
            ...
            if visible:
                try:
                    loading = getattr(self, "_boogu_loading_label", None)
                    if loading is not None and getattr(self, "_boogu_window", None) is None:
                        loading.setVisible(True)
                        loading.setText("Loading Boogu Image UI...")
                except Exception:
                    pass
                if getattr(self, "_boogu_window", None) is None and not bool(getattr(self, "_boogu_open_pending", False)):
                    self._boogu_open_pending = True
                    try:
                        QTimer.singleShot(40, self._open_boogu_ui)
                    except Exception:
                        self._open_boogu_ui()
                else:
                    try:
    ...
    def _sync_zimg_sel_edit_footer_visibility(self):
        ...
        try:
            tabs = getattr(self, "txt2img_tabs", None)
            sel_tab = getattr(self, "_zimg_sel_edit_tab", None)
            active = bool(tabs is not None and sel_tab is not None and tabs.currentWidget() is sel_tab)
            for _w in list(getattr(self, "_txt2img_progress_widgets", []) or []) + list(getattr(self, "_txt2img_footer_widgets", []) or []):
                try:
                    if _w is not None:
                        _w.setVisible(not active)
                except Exception:
                    pass
        except Exception:
            pass

    def _zimg_sel_edit_current_gguf_settings(self) -> dict:
        """Return the currently selected GGUF settings from txt2img's existing GGUF section."""
        data = {"gguf_model_path": "", "gguf_instruct_path": "", "gguf_vae_path": "", "sd_cli_path": ""}
        try:
            if hasattr(self, "gguf_model_combo") and self.gguf_model_combo is not None:
                data["gguf_model_path"] = str(self.gguf_model_combo.currentData() or "")
        except Exception:
            pass
        try:
            if hasattr(self, "gguf_instruct_combo") and self.gguf_instruct_combo is not None:
                data["gguf_instruct_path"] = str(self.gguf_instruct_combo.currentData() or "")
        except Exception:
            pass
        try:
            if hasattr(self, "gguf_vae_combo") and self.gguf_vae_combo is not None:
                data["gguf_vae_path"] = str(self.gguf_vae_combo.currentData() or "")
        except Exception:
            pass
        try:
            if hasattr(self, "sd_cli_path") and self.sd_cli_path is not None:
                data["sd_cli_path"] = str(self.sd_cli_path.text() or "").strip()
        except Exception:
            pass
        return data

    def _open_zimage_sel_edit_ui(self):
        """Show the Z-Image GGUF selective-edit helper embedded as a txt2img tab."""
        try:
            host = getattr(self, "_zimg_sel_edit_tab", None)
            lay = getattr(self, "_zimg_sel_edit_tab_layout", None)
            if host is None or lay is None:
                return None

            self._zimg_sel_edit_open_pending = False
            win = getattr(self, "_zimg_sel_edit_window", None)
            if win is None:
                try:
                    loading = getattr(self, "_zimg_sel_edit_loading_label", None)
                    if loading is not None:
                        loading.setVisible(True)
                        loading.setText("Loading Z-Image selective edit UI...")
                    QApplication.processEvents()
                except Exception:
                    pass
                try:
                    from helpers.zimage_sel_edit import ZImageSelectiveEditWidget  # type: ignore
                except Exception:
                    from zimage_sel_edit import ZImageSelectiveEditWidget  # type: ignore

                try:
                    win = ZImageSelectiveEditWidget(parent=host, embedded=True, gguf_provider=self._zimg_sel_edit_current_gguf_settings)
                except TypeError:
                    win = ZImageSelectiveEditWidget(host)
                    try:
                        win.gguf_provider = self._zimg_sel_edit_current_gguf_settings
                    except Exception:
                        pass
                self._zimg_sel_edit_window = win
                try:
                    win.setParent(host)
                except Exception:
                    pass
                ...
                try:
                    ...
                    pass
                try:
                    if hasattr(win, "output_path") and hasattr(self, "output_path"):
                        _out = self.output_path.text().strip()
                        if _out:
                            win.output_path.setText(_out)
                except Exception:
                    pass
                try:
                    if hasattr(win, "fileReady"):
                        win.fileReady.connect(self._on_zimg_sel_edit_file_ready)
                except Exception:
                    pass
                try:
                    loading = getattr(self, "_zimg_sel_edit_loading_label", None)
                ...
                try:
                    ...
                    pass
                try:
                    win.destroyed.connect(lambda *_: setattr(self, "_zimg_sel_edit_window", None))
                except Exception:
                    pass
            ...
            try:
                if hasattr(self, "status"):
                    self.status.setText("Z-Image GGUF selective-edit tab ready")
            except Exception:
                pass
            return win
        except Exception as exc:
            ...
            try:
                ...
                if loading is not None:
                    loading.setVisible(True)
                    loading.setText(f"Could not load Z-Image selective edit UI:\n{exc}")
            except Exception:
                pass
            try:
                QMessageBox.warning(self, "Z-Image selective edit", f"Could not show Z-Image selective edit UI:\n{exc}")
            except Exception:
                pass
            try:
                if hasattr(self, "status"):
                    self.status.setText(f"Could not show Z-Image selective edit UI: {exc}")
            except Exception:
                pass
            return None

    def _set_zimg_sel_edit_tab_visible(self, visible: bool):
        """Add/remove the Selective Edit tab for Z-Image GGUF only."""
        try:
            tabs = getattr(self, "txt2img_tabs", None)
            tab = getattr(self, "_zimg_sel_edit_tab", None)
            ...
            if bool(visible):
                if idx < 0:
                    try:
                        tabs.insertTab(1, tab, "Selective Edit")
                    except Exception:
                        tabs.addTab(tab, "Selective Edit")
                if getattr(self, "_zimg_sel_edit_window", None) is None and not bool(getattr(self, "_zimg_sel_edit_open_pending", False)):
                    self._zimg_sel_edit_open_pending = True
                    try:
                        QTimer.singleShot(40, self._open_zimage_sel_edit_ui)
                    except Exception:
                        self._open_zimage_sel_edit_ui()
            else:
                self._zimg_sel_edit_open_pending = False
                if idx >= 0:
                    try:
                    ...
                    try:
                        ...
                        pass
            try:
                self._sync_zimg_sel_edit_footer_visibility()
            except Exception:
                pass
        except Exception:
            pass

    def _on_zimg_sel_edit_file_ready(self, path: str):
        try:
            self.fileReady.emit(str(path))
        except Exception:
            pass
        try:
            if hasattr(self, "status"):
                self.status.setText(f"Selective edit created: {path}")
        except Exception:
            pass

    def _is_lens_engine_selected(self) -> bool:
        try:
            return self._engine_key_selected() == "lens_turbo_u4"
        except Exception:
            try:
                cb = getattr(self, "engine_combo", None)
                text = str(cb.currentText() if cb is not None else "").lower()
                data = str(cb.currentData() if cb is not None else "").lower()
                return "lens" in text or "lens" in data
            except Exception:
                return False
    ...
    def _on_engine_changed(self, *_):
        ...
        is_chroma = (key == "chroma")
        is_lens = (key == "lens_turbo_u4")
        ...
        try:
            self._set_zimg_sel_edit_tab_visible(bool(is_gguf))
        except Exception:
            pass
        ...
        try:
            ...
            if hasattr(self, "banner") and self.banner is not None:
                if is_zimage:
                    ...
                    self.banner.setText("Lens Turbo U4")
                elif is_chroma:
                    self.banner.setText("Text to image with Chroma")
                elif is_ideogram4:
                    self.banner.setText("Text to image with Ideogram 4 GGUF")
                elif is_krea2:
        ...
        # Lens and Chroma use their existing helper UIs, embedded inside this tab.
        # Do not route those through the generic txt2img controls.
        try:
            self._set_lens_embedded_visible(bool(is_lens))
        except Exception:
            pass
        try:
            self._set_chroma_embedded_visible(bool(is_chroma))
        except Exception:
            pass
        try:
            self._set_ideogram4_embedded_visible(bool(is_ideogram4))
        except Exception:
            pass
        try:
            self._set_krea2_embedded_visible(bool(is_krea2))
        except Exception:
            pass
        try:
        ...
        if is_ideogram4:
            try:
                self._last_engine_key = "ideogram4_gguf"
            except Exception:
                pass
            try:
                if saved_fmt is not None and hasattr(self, "format_combo"):
        ...
        if is_lens:
            try:
                self._last_engine_key = "lens_turbo_u4"
            except Exception:
                pass
            try:
                if saved_fmt is not None and hasattr(self, "format_combo"):
        ...
        if fname_edit is not None:
            ...
            try:
                if is_zimage:
                    ...
                    if prev_engine != "chroma":
                        try:
                            self._filename_template_before_chroma = fname_edit.text()
                        except Exception:
                            pass
                    try:
                        fname_edit.blockSignals(True)
                        ...
                        pass
                    fname_edit.setText(f"chroma_{{seed}}_{{idx:03d}}.{fmt}")
                    try:
                        fname_edit.blockSignals(False)
                    except Exception:
                        pass
                    ...
                    try:
                        ...
                        elif prev_engine == "chroma":
                            orig = getattr(self, "_filename_template_before_chroma", "") or ""
                        elif prev_engine == "ideogram4_gguf":
                            orig = getattr(self, "_filename_template_before_ideogram4", "") or ""
                        else:
        ...
        try:
            ...
            if cs is not None:
                if is_zimage:
                    # Z-Image: CFG 0.0–5.0. Set range but do NOT hard-force value.
                    try:
                        _cur = float(cs.value())
                    except Exception:
                        _cur = 0.0
                    ...
                    # Only apply default if current value is out of range
                    try:
                        if _cur < 0.0 or _cur > 5.0:
                            _cur = 0.0
                            cs.setValue(_cur)
                    except Exception:
                        try:
        ...
        # Show the Chroma CPU offload toggle only when Chroma is active.
        try:
            if hasattr(self, "chroma_offload_cpu") and self.chroma_offload_cpu is not None:
                self.chroma_offload_cpu.setVisible(bool(is_chroma))
        except Exception:
            pass
    ...
    def _collect_job(self) -> dict:
        ...
        try:
            engine_key = self._engine_key_selected()
        except Exception:
        ...
        if not fname:
            ...
            elif engine_key == "qwen2512":
                fname = f"qwen_{{seed}}_{{idx:03d}}.{fmt}"
            elif engine_key == "chroma":
                fname = f"chroma_{{seed}}_{{idx:03d}}.{fmt}"
            else:
        ...
        job = {
            ...
            # Compatibility only: old queue/result readers may still expect this key.
            "vram_profile": getattr(self, "_vram_profile_compat", "Auto"),
            ...
            "chroma_offload_cpu": (bool(self.chroma_offload_cpu.isChecked()) if hasattr(self, "chroma_offload_cpu") else True),
        ...
        try:
            ...
            if ek.startswith("zimage"):
                ...
                try:
                    ...
                    pass
                job.setdefault("model_name", "SPARK.Chroma")
                job.setdefault("model", "SPARK.Chroma")
    ...
    def _load_settings(self):
        ...
        try:
            ...
            if hasattr(self, 'chroma_offload_cpu') and 'chroma_offload_cpu' in s: self.chroma_offload_cpu.setChecked(bool(s.get('chroma_offload_cpu')))
        except Exception:
            pass
        try:
            if hasattr(self, 'model_combo'):
    ...
    def _snapshot_ui(self) -> dict:
        ...
        try: d["init_image_enabled"] = False
        except Exception: pass
        try: d["init_image"] = ""
        except Exception: pass
        try: d["img2img_strength"] = 0.35
        except Exception: pass
        ...
        try:
            if hasattr(self, "chroma_offload_cpu"):
                d["chroma_offload_cpu"] = bool(self.chroma_offload_cpu.isChecked())
        except Exception: pass
        try:
    ...
    def _connect_autosave(self):
        ...
        for obj, sig in [
            ...
            # Save engine changes immediately so the selected backend survives restart.
            (getattr(self, 'engine_combo', None), 'currentIndexChanged'),
            ...
            (getattr(self,'chroma_offload_cpu', None), 'toggled'),
    ...
    def _enqueue(self, run_now: bool):
            ...
            if ok:
                ...
                self.status.setText("Enqueue failed — see logs/txt2img_queue_debug.log")
                try:
                    print('[txt2img] enqueue failed for engine:', job.get('engine'), 'prompt_len:', len(str(job.get('prompt') or '')))
                except Exception:
                    pass

    def _on_generate_clicked(self):
        if self._is_lens_engine_selected():
            self._open_lens_turbo_ui()
            try:
                self.status.setText("Use the embedded Lens Turbo U4 UI to generate Lens images")
            except Exception:
                pass
            return
...
def _gen_via_diffusers(job: dict, out_dir: Path, progress_cb=None, cancel_event=None):
    """
...
def _gen_via_zimage(job: dict, out_dir: Path, progress_cb=None, cancel_event=None):
    """
    Z-Image Turbo backend that runs in the shared image-model environment
    as an external process.

    It calls helpers/zimage_cli.py using the python.exe from:
        <root>/environments/.images_models/python.exe        (shared conda env)
        <root>/environments/.image_models/python.exe         (legacy spelling fallback)
        <root>/environments/.zimage_env/...                  (old private fallback only)
    ...
    # Locate the shared image-model Python first.
    # Z-Image now shares the image-model environment with HiDream/Lens:
    #     <root>/environments/.images_models/python.exe
    # Keep old private Z-Image env paths only as migration fallbacks.
    env_root = root_dir / "environments"
    shared_env = env_root / ".images_models"
    alt_shared_env = env_root / ".image_models"
    old_zenv_dir = env_root / ".zimage_env"
    candidates = [
        # Conda env layout used by the shared image-model environment.
        shared_env / "python.exe",
        shared_env / "python",
        alt_shared_env / "python.exe",
        alt_shared_env / "python",

        # Fallbacks for venv-style shared envs, in case a user created it that way.
        shared_env / "Scripts" / "python.exe",
        shared_env / "scripts" / "python.exe",
        shared_env / "bin" / "python",
        shared_env / "bin" / "python3",
        alt_shared_env / "Scripts" / "python.exe",
        alt_shared_env / "scripts" / "python.exe",
        alt_shared_env / "bin" / "python",
        alt_shared_env / "bin" / "python3",

        # Old private Z-Image env paths: fallback only during migration.
        old_zenv_dir / "Scripts" / "python.exe",
        old_zenv_dir / "scripts" / "python.exe",
        old_zenv_dir / "python.exe",
        old_zenv_dir / "bin" / "python",
        old_zenv_dir / "bin" / "python3",
        root_dir / ".zimage_env" / "Scripts" / "python.exe",
        root_dir / ".zimage_env" / "scripts" / "python.exe",
        root_dir / ".zimage_env" / "python.exe",
        root_dir / ".zimage_env" / "bin" / "python",
        root_dir / ".zimage_env" / "bin" / "python3",
    ]
    pyexe = None
    for c in candidates:
        try:
    ...
    if pyexe is None:
        try:
            print("[txt2img] Z-Image: python.exe not found. Expected shared env at:", shared_env)
            print("[txt2img] Z-Image: tried:", [str(c) for c in candidates])
        except Exception:
            pass
        return None

    try:
        py_s = str(pyexe).lower().replace("\\", "/")
        if "/environments/.images_models/" in py_s or "/environments/.image_models/" in py_s:
            print("[txt2img] Z-Image: using shared image-model env:", pyexe)
        elif ".zimage_env" in py_s:
            print("[txt2img] Z-Image: WARNING using old private .zimage_env fallback:", pyexe)
        else:
            print("[txt2img] Z-Image: using python:", pyexe)
    except Exception:
        pass
    ...
    # Optional img2img / selective edit support for Z-Image.
    # If a mask image is also present, pass it through to the helper CLI as --mask-image.
    try:
        init_img = (job.get("init_image") or job.get("source_image") or "").strip()
    except Exception:
        init_img = ""
    try:
        mask_img = (job.get("mask_image") or "").strip()
    except Exception:
        mask_img = ""
    try:
    ...
    if init_enabled and init_img:
        try:
            strength = float(job.get("img2img_strength", job.get("strength", 0.35)))
        except Exception:
            strength = 0.35
        # Clamp to sane range
        try:
        ...
        if mask_img:
            args += ["--mask-image", str(mask_img)]
    try:
        if str(job.get("gguf_model_path") or "").strip():
            args += ["--gguf-model", str(job.get("gguf_model_path") or "").strip()]
        if str(job.get("gguf_instruct_path") or "").strip():
            args += ["--gguf-instruct", str(job.get("gguf_instruct_path") or "").strip()]
        if str(job.get("gguf_vae_path") or "").strip():
            args += ["--gguf-vae", str(job.get("gguf_vae_path") or "").strip()]
    except Exception:
        pass
...
def _gen_via_chroma(job: dict, out_dir: Path, progress_cb=None, cancel_event: Optional[threading.Event] = None):
    """SPARK.Chroma backend that runs via helpers/chroma.py inside the shared image-model environment."""
    try:
        _aggressive_free_cuda_vram()
    except Exception:
        pass
    ...
    env_dir = root_dir / "environments" / ".images_models"
    candidates = [
        env_dir / "python.exe", env_dir / "python",
        env_dir / "Scripts" / "python.exe", env_dir / "scripts" / "python.exe",
        env_dir / "bin" / "python", env_dir / "bin" / "python3",
    ]
    pyexe = next((c for c in candidates if c.exists() and c.is_file()), None)
    if pyexe is None:
        try:
            print("[txt2img] Chroma: shared image-model python not found at", env_dir)
        except Exception:
            pass
        return None

    cli = root_dir / "helpers" / "chroma.py"
    if not cli.exists():
        try:
            print("[txt2img] Chroma: helpers/chroma.py not found at", cli)
        except Exception:
            pass
        return None
    ...
    prompt = str(job.get("prompt") or "")
    neg = str(job.get("negative") or "")
    batch = max(1, int(job.get("batch", 1) or 1))
    steps = int(job.get("steps", 35) or 35)
    cfg = float(job.get("cfg_scale", 3.0) or 3.0)
    width = int(job.get("width", 1024) or 1024)
    height = int(job.get("height", 1024) or 1024)
    seed = int(job.get("seed", 0) or 0)
    seed_policy = str(job.get("seed_policy") or "fixed").strip().lower()
    fmt = (job.get("format", "png") or "png").lower().strip()
    if fmt == "jpg":
        fmt = "jpeg"
    if fmt not in ("png", "jpeg", "webp", "bmp"):
        fmt = "png"
    fname_tmpl = job.get("filename_template") or "chroma_{seed}_{idx:03d}.png"
    max_seq = int(job.get("max_sequence_length", 512) or 512)
    offload_cpu = bool(job.get("chroma_offload_cpu", True))
    ...
    try:
        ...
        log_dir.mkdir(parents=True, exist_ok=True)
        stamp = time.strftime("%Y%m%d_%H%M%S")
        log_file = log_dir / f"chroma_{stamp}.log"
    except Exception:
        log_file = None

    files = []
    total_steps_all = max(1, steps * batch)
    step_re = _re.compile(r"\b(\d{1,6})\s*/\s*(\d{1,6})\b")

    for i in range(batch):
        if cancel_event and cancel_event.is_set():
            break
        s = int(seeds_list[i] if i < len(seeds_list) else seed)
        fname = str(fname_tmpl).format(seed=s, idx=i)
        if not fname.lower().endswith((".png", ".jpg", ".jpeg", ".webp", ".bmp")):
            fname += "." + ("jpg" if fmt == "jpeg" else fmt)
        out_path = _unique_path(out_dir / fname)

        args = [
            str(pyexe), str(cli), "--generate",
            "--prompt", prompt,
            "--negative", neg,
            "--width", str(width),
            "--height", str(height),
            ...
            "--guidance", str(cfg),
            "--seed", str(s),
            "--max-sequence-length", str(max_seq),
            "--output", str(out_path),
        ...
        payload = None
        code = -1
        current_base = i * max(1, steps)
        try:
            if progress_cb:
                progress_cb({"step": current_base, "total": total_steps_all, "status": "chroma"})
        except Exception:
            pass

        try:
            with (open(log_file, "a", encoding="utf-8", errors="replace") if log_file else None) as lf:
                if lf:
                    try:
                        lf.write("CMD: " + " ".join([str(x) for x in args]) + "\n")
                        lf.flush()
                ...
                if proc.stdout:
                    ...
                    for line in proc.stdout:
                        ...
                        if not sline:
                            continue
                        if sline.startswith("CHROMA_RESULT_JSON="):
                            try:
                                payload = json.loads(sline.split("=", 1)[1])
                            except Exception:
                                payload = None
                            continue
                        try:
                            sm = step_re.search(sline)
                            if sm and ":" not in sm.group(0):
                                step = int(sm.group(1))
                                total = int(sm.group(2))
                                if total > 0 and step != last_step:
                                    last_step = step
                                    if progress_cb:
                                        progress_cb({"step": current_base + step, "total": total_steps_all, "status": "chroma"})
                        except Exception:
                            pass
                code = proc.wait()
                if lf:
                    try:
                        lf.write(f"EXIT CODE: {code}\n\n")
                        lf.flush()
                    except Exception:
                        pass
        except Exception as e:
            try:
                print("[txt2img] Chroma: failed to launch/stream CLI:", e)
            except Exception:
                pass
            return None
        ...
        if int(code) != 0:
            try:
                print("[txt2img] Chroma CLI failed (see log):", str(log_file) if log_file else "(no log)")
            except Exception:
                pass
            return None

        out_file = (payload or {}).get("output") if isinstance(payload, dict) else None
        files.append(str(out_file or out_path))
        try:
            if progress_cb:
                progress_cb({"step": (i + 1) * max(1, steps), "total": total_steps_all, "status": "chroma"})
        except Exception:
            pass

    return {"files": files, "backend": "chroma", "model": "SPARK.Chroma", "log_file": (str(log_file) if log_file else None)}
...
def generate_one_from_job(job: dict, out_dir: str | Path, progress_cb=None, cancel_event: Optional[threading.Event] = None):
    ...
    if ek == "qwen2512":
        ...
        return res
    if ek == "chroma":
        res = _gen_via_chroma(job, od, progress_cb=progress_cb, cancel_event=cancel_event)
        return res
    if ek.startswith("zimage"):
...
def generate_qwen_images(job: dict, progress_cb: Optional[Callable[[float], None]] = None, cancel_event: Optional[threading.Event] = None):
    ...
    try:
        if engine in ("zimage","zimage_gguf"):
            ...
            diff = _gen_via_chroma(job, out_dir, progress_cb, cancel_event)
...
def _t2i_apply_from_dict(self, s: dict):
    ...
    widgets = [
        ...
        'chroma_offload_cpu','use_queue','show_in_player','output_path'
    ]
    blocked = []
    for name in widgets:
