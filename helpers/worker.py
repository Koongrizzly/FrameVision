...
def handle_job(jpath: Path):
    ...
    try:
        if t=="upscale_video": code = upscale_video(job, cfg, mani)
            ...
            # LTX 2.4 is queued as a normal external command. No special worker
...
def main():
    print("FrameVision Worker V2.4 Waiting for jobs in", JOBS["pending"])
    while True:
        try:
