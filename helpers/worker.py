...
def tools_ffmpeg(job, cfg, mani):
    # Execute an arbitrary ffmpeg command produced by Tools tab.
    try:
        args = job.get("args", {})
        cmd = args.get("ffmpeg_cmd") or []
        ...
        # If last token looks like a path, ensure its parent exists
        try:
            out_path = Path(args.get("outfile") or cmd[-1])
            out_path.parent.mkdir(parents=True, exist_ok=True)
        except Exception:
            pass
        return run([str(x) for x in cmd])
    except Exception as e:
        print("ERROR tools_ffmpeg:", e)
        return 1
