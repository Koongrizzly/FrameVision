...
def _create_backup_zip(app_root: Path, target_dir: Path) -> Path:
    """
    Create a backup ZIP of the app using *no compression* (ZIP_STORED) to avoid UI freezes.
    Exclude heavy/generated folders:
      - tools/ (top-level)
      - models/ (top-level)
      - output/ (top-level)
      - .venv (top-level)
      - presets/bin/ (specific subfolder)
    Also excludes __pycache__ directories anywhere.
    """
    import zipfile as _zipfile
    from datetime import datetime as _dt

    # Top-level folders to exclude (do NOT exclude entire presets anymore)
    EXCLUDE_TOP = {"tools", "models", "output", ".venv"}
    # Specific subfolder exclusion under presets/
    EXCLUDE_PRESETS_SUB = ("bin",)

    ts = _dt.now().strftime("%Y%m%d_%H%M%S")
    out = Path(target_dir) / f"{app_root.name}_backup_{ts}.zip"

    # Open ZIP with ZIP_STORED => store files without compression
    with _zipfile.ZipFile(out, "w", compression=_zipfile.ZIP_STORED, allowZip64=True) as zf:
        for p in app_root.rglob("*"):
            ...
            # Skip top-level excluded folders
            if rel.parts:
                top = rel.parts[0]
...
# --- Auto-run GitHub update check on startup (non-blocking) ---
try:
