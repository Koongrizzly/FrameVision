...
BACKUP_EXCLUDE_TOP = {"models", "output", "environments", ".venv"}
BACKUP_EXCLUDE_PRESETS_SUB = {"bin"}
BACKUP_EXCLUDE_EXTS = {".zip", ".7z", ".rar", ".tar", ".gz", ".bz2", ".xz"}


def _backup_is_hidden_folder_path(rel: Path) -> bool:
    """True when any folder component in a relative path starts with a dot."""
    return any(part.startswith(".") for part in rel.parts if part not in ("", "."))


def _backup_should_skip_dir(rel_dir: Path) -> bool:
    if not rel_dir.parts:
        return False
    parts_lower = tuple(part.lower() for part in rel_dir.parts)
    if _backup_is_hidden_folder_path(rel_dir):
        return True
    top = parts_lower[0]
    if top in BACKUP_EXCLUDE_TOP:
        return True
    if top == "presets" and len(parts_lower) >= 2 and parts_lower[1] in BACKUP_EXCLUDE_PRESETS_SUB:
        return True
    return False


def _backup_should_skip_file(rel_file: Path, app_prefix: str) -> bool:
    parent_rel = Path(*rel_file.parts[:-1]) if len(rel_file.parts) > 1 else Path()
    if _backup_should_skip_dir(parent_rel):
        return True
    if rel_file.suffix.lower() in BACKUP_EXCLUDE_EXTS:
        return True
    if rel_file.name.lower().startswith(app_prefix) and rel_file.suffix.lower() == ".zip":
        return True
    return False
...
def _create_backup_zip(app_root: Path, target_dir: Path) -> Path:
    ...
    with _zipfile.ZipFile(out, "w", compression=_zipfile.ZIP_STORED, allowZip64=True) as zf:
        for root, dirs, files in os.walk(app_root):
            ...
            if _backup_should_skip_dir(rel_root):
                dirs[:] = []
                continue
            dirs[:] = [
                d for d in dirs
                if d != "__pycache__" and not _backup_should_skip_dir(rel_root / d)
            ]
            for fname in files:
                p = Path(root) / fname
                rel = p.relative_to(app_root)
                if _backup_should_skip_file(rel, app_prefix):
                    continue
...
class UpdateDialog(QDialog):
    def __init__(self, parent: QWidget | None = None):
        ...
        update_hdr = QLabel("<b>Update all files</b>")
        ...
        warn = QLabel(
            ...
            "or <code>presets/viz/</code>). Consider creating a backup ZIP first.<br>"
            "Update all files to get newest model installs, update python files to update models and FrameVision files only."
        )
        warn.setWordWrap(True)

        v = QVBoxLayout(self)
        v.addWidget(update_hdr)
        row2 = QHBoxLayout()
    ...
    def _on_manual_check(self):
        ...
        def should_skip(rel: str) -> bool:
            ...
            # tools/ used to be mostly local/debug files, so it is still skipped by default.
            # tools/vram_lab is now a shipped runtime component and must be updated.
    ...
    def _on_update(self, source: str, mode: str):
        ...
        try:
            if source == "release":
                ...
                self._append("Fetching latest GitHub default-branch ZIP…")
