...
BETA_INCLUDE_PREFIXES = ("helpers/", "presets/viz/", "presets/extra_env/", "tools/vram_lab/")  # include any file types here
BETA_EXCLUDE_TOP = {"tools", "models", "output", "presets", ".venv", ".git"}  # when scanning whole tree
# NOTE: tools/vram_lab and presets/extra_env are intentionally included above before the top-folder exclude.
...
def _auto_update_should_skip(rel: str) -> bool:
    ...
    try:
        ...
        if rl in AUTO_UPDATE_SKIP_EXACT:
            return True
        # Shipped runtime/updateable folders.
        if rl.startswith("tools/vram_lab/"):
            return False
        if rl.startswith("presets/extra_env/"):
            return False

        # Other tools can still be skipped because they may contain local/debug files.
