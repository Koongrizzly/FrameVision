  ...
  #      )
   #     v_main.addWidget(self.banner)
    #    v_main.addSpacing(4)
        ...
        for _am in ("auto", "sage", "flash_attn", "sdpa", "none"):
            self.combo_seedvr2_attn.addItem(_am)
        self.combo_seedvr2_attn.setCurrentText("auto")
        self.combo_seedvr2_attn.setToolTip("auto = SageAttention if installed, else FlashAttention if installed, else SDPA/no optional attention.")
        adv.addWidget(self.combo_seedvr2_attn, 4, 1)

        self.lbl_seedvr2_attn_warn = QtWidgets.QLabel("Auto uses SageAttention first, then FlashAttention, then SDPA/no optional attention. Sage/Flash must be installed in the SeedVR2 environment.", self)
...
# Sections collapsed state
    ...
    def _update_seedvr2_warnings(self):
        ...
        try:
            ...
            self.lbl_seedvr2_attn_warn.setVisible(am in ("auto", "sage", "flash_attn"))
        except Exception:
            pass
