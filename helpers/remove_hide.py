...
- Shared env groups:
  - Lens and Z-image entries share '/environments/.images_models'
  - Qwen 2511 & Qwen 2512 do not use a Python env; keep their model folders separate from the shared Qwen sd-cli bin.
...
class Entry:
    ...
    # Optional subpaths that belong to another entry and should be ignored when
    # calculating size/status or deleting this entry.
...
def build_registry() -> List[Entry]:
    return [
        Entry(
            ...
            exclude_paths=["/models/wan22/wan_turbo/"],
            shared_env_group="wan22_shared",
            shared_env_path="/.wan_venv/",
            shared_helper_note="Shares the WAN env with WAN 2.2 Turbo. The env is only removed after both WAN entries are removed.",
        ),
        Entry(
            id="wan22_turbo",
            title="WAN 2.2 Turbo 5B",
            env_paths=["/.wan_venv/"],
            model_paths=["/models/wan22/wan_turbo/Wan2.2-TI2V-5B-Turbo/"],
            repo_paths=["/models/wan22/wan_turbo/Wan2.2-TI2V-5B-Turbo-main/"],
            helper_paths=["/helpers/wan22_turbo_vramlab_cli.py"],
            shared_env_group="wan22_shared",
            shared_env_path="/.wan_venv/",
            shared_helper_note="Shares the WAN env with WAN 2.2. The env is only removed after both WAN entries are removed.",
        ),
        Entry(
            id="hunyuan15",
            title="HunyuanVideo 1.5",
            env_paths=["/environments/.hunyuan15_official/"],
        ...
        Entry(
            ...
            env_paths=["/environments/.bernini_r/"],
            model_paths=["/models/bernini_r_1p3b/"],
            helper_paths=["/helpers/bernini_small.py"],
            shared_helper_note=(
                "Hide only removes the Bernini-R 1.3B selection from the WAN/Video engine selector. "
                "Delete removes the Bernini env, repo/model folder, and helper file."
            ),
        ),
        Entry(
            id="hiar",
        ...
        Entry(
            id="ltx23",
            title="LTX 2.3",
            env_paths=["/environments/.ltx23/"],
            model_paths=["/models/ltx23/"],
            helper_paths=[
                "/helpers/ltx23_vram_lab_cli.py",
                "/helpers/ltx23_ui.py",
            ],
        ),
        Entry(
        ...
        Entry(
            id="sdxl_inpaint",
            title="SDXL Inpaint",
            env_paths=["/environments/.sdxl_inpaint/"],
        ...
        Entry(
            ...
            shared_helper_note="Uses the shared Qwen sd-cli bin; this entry only removes the 2512 model files.",
        ),
        Entry(
            id="lens_turbo_u4",
            title="Lens Turbo U4",
            env_paths=["/environments/.images_models/"],
            model_paths=["/models/lens/"],
            helper_paths=["/helpers/lens_turbo_u4_ui.py"],
            shared_env_group="image_models_shared",
            shared_env_path="/environments/.images_models/",
            shared_helper_note="Shared image-model env. It is only removed after all image_models_shared model folders are removed.",
        ),
        Entry(
            id="chroma",
            title="SPARK.Chroma Text to Image",
            env_paths=["/environments/.images_models/"],
            model_paths=["/models/chroma/"],
            helper_paths=["/helpers/chroma.py"],
            shared_env_group="image_models_shared",
            shared_env_path="/environments/.images_models/",
            shared_helper_note="Shared image-model env. It is only removed after all image_models_shared model folders are removed.",
        ),
        ...
        Entry(
            ...
            # Klein GGUF uses sd-cli and model files; no private Python env is required.
            model_paths=["/models/klein4b_gguf/"],
            helper_paths=["/helpers/flux_klein_editor_ui.py"],
            shared_helper_note="Uses sd-cli + GGUF model files; no private Klein Python env is removed.",
        ),
        Entry(
            id="flux_klein_9b",
        ...
        Entry(
            ...
            env_paths=["/environments/.dots_tts/"],
            model_paths=["/models/dots_tts/"],
            repo_paths=["/models/dots_tts/repo/"],
            helper_paths=["/helpers/dotstts.py"],
        ),
        Entry(
            id="whisper",
            title="Whisper Lab",
...
def _path_has_non_excluded_content(base_path: str, exclude_paths: List[str]) -> bool:
    """True if base_path exists and contains anything outside excluded descendants."""
    ...
    for ep in exclude_paths or []:
        if not ep:
            continue
        en = _norm_path(ep)
        if en == base_n or en.startswith(base_n + os.sep):
            excludes.append(en)
    ...
    try:
        if os.path.isfile(base_path) or os.path.islink(base_path):
            return not is_excluded(base_path)
    except Exception:
        return False
            ...
            for entry in it:
                try:
                    ...
                    if entry.is_dir(follow_symlinks=False):
                        if _path_has_non_excluded_content(entry.path, excludes):
                            return True
                except Exception:
                    pass
    except Exception:
...
def path_exists_any_excluding(paths: List[str], exclude_paths: List[str]) -> bool:
    return any(_path_has_non_excluded_content(p, exclude_paths) for p in paths)
...
def entry_paths_resolved(app_root: str, e: Entry) -> Dict[str, List[str]]:
    ...
    excludes = [resolve_app_path(app_root, s) for s in getattr(e, "exclude_paths", [])]
...
def installed_status(app_root: str, e: Entry) -> Tuple[bool, bool, bool]:
    ...
    models_ok = path_exists_any_excluding(r["models"], excludes) if r["models"] else False
    repos_ok = path_exists_any_excluding(r["repos"], excludes) if r["repos"] else False
    return env_ok, models_ok, repos_ok
...
def entry_has_non_shared_install(app_root: str, e: Entry) -> bool:
    ...
    r = entry_paths_resolved(app_root, e)
    excludes = r.get("excludes", [])
    return path_exists_any_excluding(r.get("models", []), excludes) or path_exists_any_excluding(r.get("repos", []), excludes)
...
def remove_path_excluding(p: str, exclude_paths: List[str]) -> Tuple[bool, str]:
    """Delete a path while preserving excluded descendants.

    Used for parent folders like /models/wan22 when a nested child such as
    /models/wan22/wan_turbo belongs to a separate entry.
    """
    if not os.path.exists(p):
        return True, f"Already missing: {p}"
    ...
    try:
        if os.path.isfile(p) or os.path.islink(p):
            if is_excluded(p):
                return True, f"Skipped excluded path: {p}"
            return remove_path(p)
        ...
        # remove now-empty directories up the tree only when safe
        try:
            if not any(True for _ in os.scandir(p)):
                os.rmdir(p)
                return True, f"Deleted: {p}"
        except Exception:
            pass
        if kept:
            return True, f"Deleted contents of: {p} (kept excluded subpaths: {', '.join(kept)})"
        return True, f"Deleted contents of: {p}"
    except Exception as ex:
        return False, f"Failed: {p} ({ex})"
...
def remove_entry(app_root: str, registry: List[Entry], e: Entry) -> List[Tuple[bool, str]]:
    ...
    # 1) Remove models and repos (always safe). When a parent path contains
    # another entry's nested folders, preserve those excluded descendants.
    for p in r["models"]:
        if _is_helpers_path(app_root, p):
            results.append((True, f"Skipped protected helpers path: {p}"))
            continue
        ok, msg = remove_path_excluding(p, excludes)
